// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: replicant/service.proto

package replicant

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// TargetAction is the action to take on a replication target.
type TargetAction int32

const (
	TargetAction_TARGET_ACTION_UNSPECIFIED TargetAction = 0
	TargetAction_TARGET_ACTION_START       TargetAction = 1
	TargetAction_TARGET_ACTION_STOP        TargetAction = 2
)

// Enum value maps for TargetAction.
var (
	TargetAction_name = map[int32]string{
		0: "TARGET_ACTION_UNSPECIFIED",
		1: "TARGET_ACTION_START",
		2: "TARGET_ACTION_STOP",
	}
	TargetAction_value = map[string]int32{
		"TARGET_ACTION_UNSPECIFIED": 0,
		"TARGET_ACTION_START":       1,
		"TARGET_ACTION_STOP":        2,
	}
)

func (x TargetAction) Enum() *TargetAction {
	p := new(TargetAction)
	*p = x
	return p
}

func (x TargetAction) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TargetAction) Descriptor() protoreflect.EnumDescriptor {
	return file_replicant_service_proto_enumTypes[0].Descriptor()
}

func (TargetAction) Type() protoreflect.EnumType {
	return &file_replicant_service_proto_enumTypes[0]
}

func (x TargetAction) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TargetAction.Descriptor instead.
func (TargetAction) EnumDescriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{0}
}

// TargetState describes the current runtime state of a replication target.
type TargetState int32

const (
	TargetState_TARGET_STATE_UNSPECIFIED TargetState = 0
	TargetState_TARGET_STATE_RUNNING     TargetState = 1
	TargetState_TARGET_STATE_STARTING    TargetState = 2
	TargetState_TARGET_STATE_STOPPING    TargetState = 3
	TargetState_TARGET_STATE_STOPPED     TargetState = 4
)

// Enum value maps for TargetState.
var (
	TargetState_name = map[int32]string{
		0: "TARGET_STATE_UNSPECIFIED",
		1: "TARGET_STATE_RUNNING",
		2: "TARGET_STATE_STARTING",
		3: "TARGET_STATE_STOPPING",
		4: "TARGET_STATE_STOPPED",
	}
	TargetState_value = map[string]int32{
		"TARGET_STATE_UNSPECIFIED": 0,
		"TARGET_STATE_RUNNING":     1,
		"TARGET_STATE_STARTING":    2,
		"TARGET_STATE_STOPPING":    3,
		"TARGET_STATE_STOPPED":     4,
	}
)

func (x TargetState) Enum() *TargetState {
	p := new(TargetState)
	*p = x
	return p
}

func (x TargetState) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TargetState) Descriptor() protoreflect.EnumDescriptor {
	return file_replicant_service_proto_enumTypes[1].Descriptor()
}

func (TargetState) Type() protoreflect.EnumType {
	return &file_replicant_service_proto_enumTypes[1]
}

func (x TargetState) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TargetState.Descriptor instead.
func (TargetState) EnumDescriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{1}
}

type SendDocumentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The UUID of the document to replicate.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Force replication even if the document has already been replicated.
	Force         bool `protobuf:"varint,2,opt,name=force,proto3" json:"force,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendDocumentRequest) Reset() {
	*x = SendDocumentRequest{}
	mi := &file_replicant_service_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendDocumentRequest) ProtoMessage() {}

func (x *SendDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendDocumentRequest.ProtoReflect.Descriptor instead.
func (*SendDocumentRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{0}
}

func (x *SendDocumentRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *SendDocumentRequest) GetForce() bool {
	if x != nil {
		return x.Force
	}
	return false
}

type SendDocumentResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The version of the document that was replicated to the target.
	TargetVersion int64 `protobuf:"varint,1,opt,name=target_version,json=targetVersion,proto3" json:"target_version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendDocumentResponse) Reset() {
	*x = SendDocumentResponse{}
	mi := &file_replicant_service_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendDocumentResponse) ProtoMessage() {}

func (x *SendDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendDocumentResponse.ProtoReflect.Descriptor instead.
func (*SendDocumentResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{1}
}

func (x *SendDocumentResponse) GetTargetVersion() int64 {
	if x != nil {
		return x.TargetVersion
	}
	return 0
}

// ConfigureTargetRequest creates or updates a replication target.
type ConfigureTargetRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name is the identifier for the replication target.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The URL of the target repository to replicate to.
	RepositoryUrl string `protobuf:"bytes,2,opt,name=repository_url,json=repositoryUrl,proto3" json:"repository_url,omitempty"`
	// The OIDC discovery configuration URL for target authentication.
	OidcConfig string `protobuf:"bytes,3,opt,name=oidc_config,json=oidcConfig,proto3" json:"oidc_config,omitempty"`
	// The OAuth2 client ID for target authentication.
	ClientId string `protobuf:"bytes,4,opt,name=client_id,json=clientId,proto3" json:"client_id,omitempty"`
	// The OAuth2 client secret for target authentication.
	ClientSecret string `protobuf:"bytes,5,opt,name=client_secret,json=clientSecret,proto3" json:"client_secret,omitempty"`
	// The event log position to start replication from.
	StartFrom int64 `protobuf:"varint,6,opt,name=start_from,json=startFrom,proto3" json:"start_from,omitempty"`
	// Sync configuration controlling what gets replicated.
	Config        *SyncConfig `protobuf:"bytes,7,opt,name=config,proto3" json:"config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigureTargetRequest) Reset() {
	*x = ConfigureTargetRequest{}
	mi := &file_replicant_service_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigureTargetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigureTargetRequest) ProtoMessage() {}

func (x *ConfigureTargetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigureTargetRequest.ProtoReflect.Descriptor instead.
func (*ConfigureTargetRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{2}
}

func (x *ConfigureTargetRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ConfigureTargetRequest) GetRepositoryUrl() string {
	if x != nil {
		return x.RepositoryUrl
	}
	return ""
}

func (x *ConfigureTargetRequest) GetOidcConfig() string {
	if x != nil {
		return x.OidcConfig
	}
	return ""
}

func (x *ConfigureTargetRequest) GetClientId() string {
	if x != nil {
		return x.ClientId
	}
	return ""
}

func (x *ConfigureTargetRequest) GetClientSecret() string {
	if x != nil {
		return x.ClientSecret
	}
	return ""
}

func (x *ConfigureTargetRequest) GetStartFrom() int64 {
	if x != nil {
		return x.StartFrom
	}
	return 0
}

func (x *ConfigureTargetRequest) GetConfig() *SyncConfig {
	if x != nil {
		return x.Config
	}
	return nil
}

// SyncConfig controls what documents and content are replicated.
type SyncConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Accept and skip documents that fail to replicate instead of blocking.
	AcceptErrors bool `protobuf:"varint,1,opt,name=accept_errors,json=acceptErrors,proto3" json:"accept_errors,omitempty"`
	// Replicate all attachments for documents.
	AllAttachments bool `protobuf:"varint,2,opt,name=all_attachments,json=allAttachments,proto3" json:"all_attachments,omitempty"`
	// Document types to exclude from replication.
	IgnoreTypes []string `protobuf:"bytes,3,rep,name=ignore_types,json=ignoreTypes,proto3" json:"ignore_types,omitempty"`
	// Sections to exclude from replication, identified by document type and
	// section UUID.
	IgnoreSections []*SectionForType `protobuf:"bytes,4,rep,name=ignore_sections,json=ignoreSections,proto3" json:"ignore_sections,omitempty"`
	// Specific attachments to include when all_attachments is false.
	IncludeAttachments []*AttachmentForType `protobuf:"bytes,5,rep,name=include_attachments,json=includeAttachments,proto3" json:"include_attachments,omitempty"`
	// Ignore events generated by these client subs.
	IgnoreSubs    []string `protobuf:"bytes,6,rep,name=ignore_subs,json=ignoreSubs,proto3" json:"ignore_subs,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncConfig) Reset() {
	*x = SyncConfig{}
	mi := &file_replicant_service_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncConfig) ProtoMessage() {}

func (x *SyncConfig) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncConfig.ProtoReflect.Descriptor instead.
func (*SyncConfig) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{3}
}

func (x *SyncConfig) GetAcceptErrors() bool {
	if x != nil {
		return x.AcceptErrors
	}
	return false
}

func (x *SyncConfig) GetAllAttachments() bool {
	if x != nil {
		return x.AllAttachments
	}
	return false
}

func (x *SyncConfig) GetIgnoreTypes() []string {
	if x != nil {
		return x.IgnoreTypes
	}
	return nil
}

func (x *SyncConfig) GetIgnoreSections() []*SectionForType {
	if x != nil {
		return x.IgnoreSections
	}
	return nil
}

func (x *SyncConfig) GetIncludeAttachments() []*AttachmentForType {
	if x != nil {
		return x.IncludeAttachments
	}
	return nil
}

func (x *SyncConfig) GetIgnoreSubs() []string {
	if x != nil {
		return x.IgnoreSubs
	}
	return nil
}

// SectionForType identifies a section to ignore for a specific document type.
type SectionForType struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The document type, e.g. "core/event".
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// The UUID of the section to ignore.
	SectionUuid   string `protobuf:"bytes,2,opt,name=section_uuid,json=sectionUuid,proto3" json:"section_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SectionForType) Reset() {
	*x = SectionForType{}
	mi := &file_replicant_service_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SectionForType) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SectionForType) ProtoMessage() {}

func (x *SectionForType) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SectionForType.ProtoReflect.Descriptor instead.
func (*SectionForType) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{4}
}

func (x *SectionForType) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *SectionForType) GetSectionUuid() string {
	if x != nil {
		return x.SectionUuid
	}
	return ""
}

// AttachmentForType identifies an attachment to include for a specific document
// type. Format in environment variable: "name.type", e.g. "image.core/image".
type AttachmentForType struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The attachment name, e.g. "image".
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// The document type, e.g. "core/image".
	Name          string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AttachmentForType) Reset() {
	*x = AttachmentForType{}
	mi := &file_replicant_service_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AttachmentForType) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AttachmentForType) ProtoMessage() {}

func (x *AttachmentForType) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AttachmentForType.ProtoReflect.Descriptor instead.
func (*AttachmentForType) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{5}
}

func (x *AttachmentForType) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *AttachmentForType) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type ConfigureTargetResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigureTargetResponse) Reset() {
	*x = ConfigureTargetResponse{}
	mi := &file_replicant_service_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigureTargetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigureTargetResponse) ProtoMessage() {}

func (x *ConfigureTargetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigureTargetResponse.ProtoReflect.Descriptor instead.
func (*ConfigureTargetResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{6}
}

type RemoveTargetRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the replication target to remove.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveTargetRequest) Reset() {
	*x = RemoveTargetRequest{}
	mi := &file_replicant_service_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveTargetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveTargetRequest) ProtoMessage() {}

func (x *RemoveTargetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveTargetRequest.ProtoReflect.Descriptor instead.
func (*RemoveTargetRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{7}
}

func (x *RemoveTargetRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type RemoveTargetResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveTargetResponse) Reset() {
	*x = RemoveTargetResponse{}
	mi := &file_replicant_service_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveTargetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveTargetResponse) ProtoMessage() {}

func (x *RemoveTargetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveTargetResponse.ProtoReflect.Descriptor instead.
func (*RemoveTargetResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{8}
}

type ChangeTargetStateRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the replication target.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The action to perform on the target.
	Action        TargetAction `protobuf:"varint,2,opt,name=action,proto3,enum=elephant.replicant.TargetAction" json:"action,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeTargetStateRequest) Reset() {
	*x = ChangeTargetStateRequest{}
	mi := &file_replicant_service_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeTargetStateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeTargetStateRequest) ProtoMessage() {}

func (x *ChangeTargetStateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeTargetStateRequest.ProtoReflect.Descriptor instead.
func (*ChangeTargetStateRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{9}
}

func (x *ChangeTargetStateRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ChangeTargetStateRequest) GetAction() TargetAction {
	if x != nil {
		return x.Action
	}
	return TargetAction_TARGET_ACTION_UNSPECIFIED
}

type ChangeTargetStateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeTargetStateResponse) Reset() {
	*x = ChangeTargetStateResponse{}
	mi := &file_replicant_service_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeTargetStateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeTargetStateResponse) ProtoMessage() {}

func (x *ChangeTargetStateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeTargetStateResponse.ProtoReflect.Descriptor instead.
func (*ChangeTargetStateResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{10}
}

type GetTargetStateRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the replication target.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTargetStateRequest) Reset() {
	*x = GetTargetStateRequest{}
	mi := &file_replicant_service_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTargetStateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTargetStateRequest) ProtoMessage() {}

func (x *GetTargetStateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTargetStateRequest.ProtoReflect.Descriptor instead.
func (*GetTargetStateRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{11}
}

func (x *GetTargetStateRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetTargetStateResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The current state of the replication target.
	State         TargetState `protobuf:"varint,1,opt,name=state,proto3,enum=elephant.replicant.TargetState" json:"state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTargetStateResponse) Reset() {
	*x = GetTargetStateResponse{}
	mi := &file_replicant_service_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTargetStateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTargetStateResponse) ProtoMessage() {}

func (x *GetTargetStateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTargetStateResponse.ProtoReflect.Descriptor instead.
func (*GetTargetStateResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{12}
}

func (x *GetTargetStateResponse) GetState() TargetState {
	if x != nil {
		return x.State
	}
	return TargetState_TARGET_STATE_UNSPECIFIED
}

type ListTargetsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTargetsRequest) Reset() {
	*x = ListTargetsRequest{}
	mi := &file_replicant_service_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTargetsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTargetsRequest) ProtoMessage() {}

func (x *ListTargetsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTargetsRequest.ProtoReflect.Descriptor instead.
func (*ListTargetsRequest) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{13}
}

type ListTargetsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The configured replication targets.
	Targets       []*TargetInfo `protobuf:"bytes,1,rep,name=targets,proto3" json:"targets,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTargetsResponse) Reset() {
	*x = ListTargetsResponse{}
	mi := &file_replicant_service_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTargetsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTargetsResponse) ProtoMessage() {}

func (x *ListTargetsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTargetsResponse.ProtoReflect.Descriptor instead.
func (*ListTargetsResponse) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{14}
}

func (x *ListTargetsResponse) GetTargets() []*TargetInfo {
	if x != nil {
		return x.Targets
	}
	return nil
}

// TargetInfo is a summary of a configured replication target.
type TargetInfo struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the replication target.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The URL of the target repository.
	RepositoryUrl string `protobuf:"bytes,2,opt,name=repository_url,json=repositoryUrl,proto3" json:"repository_url,omitempty"`
	// Enabled is true if the target is configured to run.
	Enabled       bool `protobuf:"varint,3,opt,name=enabled,proto3" json:"enabled,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TargetInfo) Reset() {
	*x = TargetInfo{}
	mi := &file_replicant_service_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TargetInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TargetInfo) ProtoMessage() {}

func (x *TargetInfo) ProtoReflect() protoreflect.Message {
	mi := &file_replicant_service_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TargetInfo.ProtoReflect.Descriptor instead.
func (*TargetInfo) Descriptor() ([]byte, []int) {
	return file_replicant_service_proto_rawDescGZIP(), []int{15}
}

func (x *TargetInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *TargetInfo) GetRepositoryUrl() string {
	if x != nil {
		return x.RepositoryUrl
	}
	return ""
}

func (x *TargetInfo) GetEnabled() bool {
	if x != nil {
		return x.Enabled
	}
	return false
}

var File_replicant_service_proto protoreflect.FileDescriptor

const file_replicant_service_proto_rawDesc = "" +
	"\n" +
	"\x17replicant/service.proto\x12\x12elephant.replicant\"?\n" +
	"\x13SendDocumentRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x14\n" +
	"\x05force\x18\x02 \x01(\bR\x05force\"=\n" +
	"\x14SendDocumentResponse\x12%\n" +
	"\x0etarget_version\x18\x01 \x01(\x03R\rtargetVersion\"\x8d\x02\n" +
	"\x16ConfigureTargetRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12%\n" +
	"\x0erepository_url\x18\x02 \x01(\tR\rrepositoryUrl\x12\x1f\n" +
	"\voidc_config\x18\x03 \x01(\tR\n" +
	"oidcConfig\x12\x1b\n" +
	"\tclient_id\x18\x04 \x01(\tR\bclientId\x12#\n" +
	"\rclient_secret\x18\x05 \x01(\tR\fclientSecret\x12\x1d\n" +
	"\n" +
	"start_from\x18\x06 \x01(\x03R\tstartFrom\x126\n" +
	"\x06config\x18\a \x01(\v2\x1e.elephant.replicant.SyncConfigR\x06config\"\xc3\x02\n" +
	"\n" +
	"SyncConfig\x12#\n" +
	"\raccept_errors\x18\x01 \x01(\bR\facceptErrors\x12'\n" +
	"\x0fall_attachments\x18\x02 \x01(\bR\x0eallAttachments\x12!\n" +
	"\fignore_types\x18\x03 \x03(\tR\vignoreTypes\x12K\n" +
	"\x0fignore_sections\x18\x04 \x03(\v2\".elephant.replicant.SectionForTypeR\x0eignoreSections\x12V\n" +
	"\x13include_attachments\x18\x05 \x03(\v2%.elephant.replicant.AttachmentForTypeR\x12includeAttachments\x12\x1f\n" +
	"\vignore_subs\x18\x06 \x03(\tR\n" +
	"ignoreSubs\"G\n" +
	"\x0eSectionForType\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12!\n" +
	"\fsection_uuid\x18\x02 \x01(\tR\vsectionUuid\";\n" +
	"\x11AttachmentForType\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\"\x19\n" +
	"\x17ConfigureTargetResponse\")\n" +
	"\x13RemoveTargetRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"\x16\n" +
	"\x14RemoveTargetResponse\"h\n" +
	"\x18ChangeTargetStateRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x128\n" +
	"\x06action\x18\x02 \x01(\x0e2 .elephant.replicant.TargetActionR\x06action\"\x1b\n" +
	"\x19ChangeTargetStateResponse\"+\n" +
	"\x15GetTargetStateRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"O\n" +
	"\x16GetTargetStateResponse\x125\n" +
	"\x05state\x18\x01 \x01(\x0e2\x1f.elephant.replicant.TargetStateR\x05state\"\x14\n" +
	"\x12ListTargetsRequest\"O\n" +
	"\x13ListTargetsResponse\x128\n" +
	"\atargets\x18\x01 \x03(\v2\x1e.elephant.replicant.TargetInfoR\atargets\"a\n" +
	"\n" +
	"TargetInfo\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12%\n" +
	"\x0erepository_url\x18\x02 \x01(\tR\rrepositoryUrl\x12\x18\n" +
	"\aenabled\x18\x03 \x01(\bR\aenabled*^\n" +
	"\fTargetAction\x12\x1d\n" +
	"\x19TARGET_ACTION_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13TARGET_ACTION_START\x10\x01\x12\x16\n" +
	"\x12TARGET_ACTION_STOP\x10\x02*\x95\x01\n" +
	"\vTargetState\x12\x1c\n" +
	"\x18TARGET_STATE_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14TARGET_STATE_RUNNING\x10\x01\x12\x19\n" +
	"\x15TARGET_STATE_STARTING\x10\x02\x12\x19\n" +
	"\x15TARGET_STATE_STOPPING\x10\x03\x12\x18\n" +
	"\x14TARGET_STATE_STOPPED\x10\x042\xfa\x04\n" +
	"\vReplication\x12a\n" +
	"\fSendDocument\x12'.elephant.replicant.SendDocumentRequest\x1a(.elephant.replicant.SendDocumentResponse\x12j\n" +
	"\x0fConfigureTarget\x12*.elephant.replicant.ConfigureTargetRequest\x1a+.elephant.replicant.ConfigureTargetResponse\x12a\n" +
	"\fRemoveTarget\x12'.elephant.replicant.RemoveTargetRequest\x1a(.elephant.replicant.RemoveTargetResponse\x12p\n" +
	"\x11ChangeTargetState\x12,.elephant.replicant.ChangeTargetStateRequest\x1a-.elephant.replicant.ChangeTargetStateResponse\x12g\n" +
	"\x0eGetTargetState\x12).elephant.replicant.GetTargetStateRequest\x1a*.elephant.replicant.GetTargetStateResponse\x12^\n" +
	"\vListTargets\x12&.elephant.replicant.ListTargetsRequest\x1a'.elephant.replicant.ListTargetsResponseB(Z&github.com/ttab/elephant-api/replicantb\x06proto3"

var (
	file_replicant_service_proto_rawDescOnce sync.Once
	file_replicant_service_proto_rawDescData []byte
)

func file_replicant_service_proto_rawDescGZIP() []byte {
	file_replicant_service_proto_rawDescOnce.Do(func() {
		file_replicant_service_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_replicant_service_proto_rawDesc), len(file_replicant_service_proto_rawDesc)))
	})
	return file_replicant_service_proto_rawDescData
}

var file_replicant_service_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_replicant_service_proto_msgTypes = make([]protoimpl.MessageInfo, 16)
var file_replicant_service_proto_goTypes = []any{
	(TargetAction)(0),                 // 0: elephant.replicant.TargetAction
	(TargetState)(0),                  // 1: elephant.replicant.TargetState
	(*SendDocumentRequest)(nil),       // 2: elephant.replicant.SendDocumentRequest
	(*SendDocumentResponse)(nil),      // 3: elephant.replicant.SendDocumentResponse
	(*ConfigureTargetRequest)(nil),    // 4: elephant.replicant.ConfigureTargetRequest
	(*SyncConfig)(nil),                // 5: elephant.replicant.SyncConfig
	(*SectionForType)(nil),            // 6: elephant.replicant.SectionForType
	(*AttachmentForType)(nil),         // 7: elephant.replicant.AttachmentForType
	(*ConfigureTargetResponse)(nil),   // 8: elephant.replicant.ConfigureTargetResponse
	(*RemoveTargetRequest)(nil),       // 9: elephant.replicant.RemoveTargetRequest
	(*RemoveTargetResponse)(nil),      // 10: elephant.replicant.RemoveTargetResponse
	(*ChangeTargetStateRequest)(nil),  // 11: elephant.replicant.ChangeTargetStateRequest
	(*ChangeTargetStateResponse)(nil), // 12: elephant.replicant.ChangeTargetStateResponse
	(*GetTargetStateRequest)(nil),     // 13: elephant.replicant.GetTargetStateRequest
	(*GetTargetStateResponse)(nil),    // 14: elephant.replicant.GetTargetStateResponse
	(*ListTargetsRequest)(nil),        // 15: elephant.replicant.ListTargetsRequest
	(*ListTargetsResponse)(nil),       // 16: elephant.replicant.ListTargetsResponse
	(*TargetInfo)(nil),                // 17: elephant.replicant.TargetInfo
}
var file_replicant_service_proto_depIdxs = []int32{
	5,  // 0: elephant.replicant.ConfigureTargetRequest.config:type_name -> elephant.replicant.SyncConfig
	6,  // 1: elephant.replicant.SyncConfig.ignore_sections:type_name -> elephant.replicant.SectionForType
	7,  // 2: elephant.replicant.SyncConfig.include_attachments:type_name -> elephant.replicant.AttachmentForType
	0,  // 3: elephant.replicant.ChangeTargetStateRequest.action:type_name -> elephant.replicant.TargetAction
	1,  // 4: elephant.replicant.GetTargetStateResponse.state:type_name -> elephant.replicant.TargetState
	17, // 5: elephant.replicant.ListTargetsResponse.targets:type_name -> elephant.replicant.TargetInfo
	2,  // 6: elephant.replicant.Replication.SendDocument:input_type -> elephant.replicant.SendDocumentRequest
	4,  // 7: elephant.replicant.Replication.ConfigureTarget:input_type -> elephant.replicant.ConfigureTargetRequest
	9,  // 8: elephant.replicant.Replication.RemoveTarget:input_type -> elephant.replicant.RemoveTargetRequest
	11, // 9: elephant.replicant.Replication.ChangeTargetState:input_type -> elephant.replicant.ChangeTargetStateRequest
	13, // 10: elephant.replicant.Replication.GetTargetState:input_type -> elephant.replicant.GetTargetStateRequest
	15, // 11: elephant.replicant.Replication.ListTargets:input_type -> elephant.replicant.ListTargetsRequest
	3,  // 12: elephant.replicant.Replication.SendDocument:output_type -> elephant.replicant.SendDocumentResponse
	8,  // 13: elephant.replicant.Replication.ConfigureTarget:output_type -> elephant.replicant.ConfigureTargetResponse
	10, // 14: elephant.replicant.Replication.RemoveTarget:output_type -> elephant.replicant.RemoveTargetResponse
	12, // 15: elephant.replicant.Replication.ChangeTargetState:output_type -> elephant.replicant.ChangeTargetStateResponse
	14, // 16: elephant.replicant.Replication.GetTargetState:output_type -> elephant.replicant.GetTargetStateResponse
	16, // 17: elephant.replicant.Replication.ListTargets:output_type -> elephant.replicant.ListTargetsResponse
	12, // [12:18] is the sub-list for method output_type
	6,  // [6:12] is the sub-list for method input_type
	6,  // [6:6] is the sub-list for extension type_name
	6,  // [6:6] is the sub-list for extension extendee
	0,  // [0:6] is the sub-list for field type_name
}

func init() { file_replicant_service_proto_init() }
func file_replicant_service_proto_init() {
	if File_replicant_service_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_replicant_service_proto_rawDesc), len(file_replicant_service_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   16,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_replicant_service_proto_goTypes,
		DependencyIndexes: file_replicant_service_proto_depIdxs,
		EnumInfos:         file_replicant_service_proto_enumTypes,
		MessageInfos:      file_replicant_service_proto_msgTypes,
	}.Build()
	File_replicant_service_proto = out.File
	file_replicant_service_proto_goTypes = nil
	file_replicant_service_proto_depIdxs = nil
}
