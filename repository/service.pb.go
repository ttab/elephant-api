// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: repository/service.proto

package repository

import (
	newsdoc "github.com/ttab/elephant-api/newsdoc"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetMetaDoc int32

const (
	GetMetaDoc_META_NONE    GetMetaDoc = 0
	GetMetaDoc_META_INCLUDE GetMetaDoc = 1
	GetMetaDoc_META_ONLY    GetMetaDoc = 2
)

// Enum value maps for GetMetaDoc.
var (
	GetMetaDoc_name = map[int32]string{
		0: "META_NONE",
		1: "META_INCLUDE",
		2: "META_ONLY",
	}
	GetMetaDoc_value = map[string]int32{
		"META_NONE":    0,
		"META_INCLUDE": 1,
		"META_ONLY":    2,
	}
)

func (x GetMetaDoc) Enum() *GetMetaDoc {
	p := new(GetMetaDoc)
	*p = x
	return p
}

func (x GetMetaDoc) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (GetMetaDoc) Descriptor() protoreflect.EnumDescriptor {
	return file_repository_service_proto_enumTypes[0].Descriptor()
}

func (GetMetaDoc) Type() protoreflect.EnumType {
	return &file_repository_service_proto_enumTypes[0]
}

func (x GetMetaDoc) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use GetMetaDoc.Descriptor instead.
func (GetMetaDoc) EnumDescriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{0}
}

// LockExclusivity controls which operations a document lock blocks for
// callers that don't hold the lock token. Document updates (new versions,
// deletes) are always blocked by a lock; the exclusivity level can extend
// the lock to also cover status and ACL updates.
type LockExclusivity int32

const (
	// Block document updates only. Statuses and ACLs can still be updated
	// by others. This is the default.
	LockExclusivity_LOCK_DOCUMENT LockExclusivity = 0
	// Block document and status updates.
	LockExclusivity_LOCK_STATUS LockExclusivity = 1
	// Block document and ACL updates.
	LockExclusivity_LOCK_ACL LockExclusivity = 2
	// Block document, status, and ACL updates.
	LockExclusivity_LOCK_EXCLUSIVE LockExclusivity = 3
)

// Enum value maps for LockExclusivity.
var (
	LockExclusivity_name = map[int32]string{
		0: "LOCK_DOCUMENT",
		1: "LOCK_STATUS",
		2: "LOCK_ACL",
		3: "LOCK_EXCLUSIVE",
	}
	LockExclusivity_value = map[string]int32{
		"LOCK_DOCUMENT":  0,
		"LOCK_STATUS":    1,
		"LOCK_ACL":       2,
		"LOCK_EXCLUSIVE": 3,
	}
)

func (x LockExclusivity) Enum() *LockExclusivity {
	p := new(LockExclusivity)
	*p = x
	return p
}

func (x LockExclusivity) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (LockExclusivity) Descriptor() protoreflect.EnumDescriptor {
	return file_repository_service_proto_enumTypes[1].Descriptor()
}

func (LockExclusivity) Type() protoreflect.EnumType {
	return &file_repository_service_proto_enumTypes[1]
}

func (x LockExclusivity) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use LockExclusivity.Descriptor instead.
func (LockExclusivity) EnumDescriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{1}
}

type SchemaActivation int32

const (
	// No activation status.
	SchemaActivation_ACTIVATION_UNKNOWN SchemaActivation = 0
	// Active schema generations are used to determine whether documents are valid.
	SchemaActivation_ACTIVATION_ACTIVE SchemaActivation = 1
	// Pending schema generations are used for soft validation and we log
	// validation errors for it. The currently active schema generation still has
	// the last say in whether a document is valid or not.
	SchemaActivation_ACTIVATION_PENDING SchemaActivation = 2
	// Deactivated schema generation.
	SchemaActivation_ACTIVATION_DEACTIVATED SchemaActivation = 3
)

// Enum value maps for SchemaActivation.
var (
	SchemaActivation_name = map[int32]string{
		0: "ACTIVATION_UNKNOWN",
		1: "ACTIVATION_ACTIVE",
		2: "ACTIVATION_PENDING",
		3: "ACTIVATION_DEACTIVATED",
	}
	SchemaActivation_value = map[string]int32{
		"ACTIVATION_UNKNOWN":     0,
		"ACTIVATION_ACTIVE":      1,
		"ACTIVATION_PENDING":     2,
		"ACTIVATION_DEACTIVATED": 3,
	}
)

func (x SchemaActivation) Enum() *SchemaActivation {
	p := new(SchemaActivation)
	*p = x
	return p
}

func (x SchemaActivation) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SchemaActivation) Descriptor() protoreflect.EnumDescriptor {
	return file_repository_service_proto_enumTypes[2].Descriptor()
}

func (SchemaActivation) Type() protoreflect.EnumType {
	return &file_repository_service_proto_enumTypes[2]
}

func (x SchemaActivation) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SchemaActivation.Descriptor instead.
func (SchemaActivation) EnumDescriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{2}
}

type MetricAggregation int32

const (
	MetricAggregation_NONE      MetricAggregation = 0
	MetricAggregation_REPLACE   MetricAggregation = 1
	MetricAggregation_INCREMENT MetricAggregation = 2
)

// Enum value maps for MetricAggregation.
var (
	MetricAggregation_name = map[int32]string{
		0: "NONE",
		1: "REPLACE",
		2: "INCREMENT",
	}
	MetricAggregation_value = map[string]int32{
		"NONE":      0,
		"REPLACE":   1,
		"INCREMENT": 2,
	}
)

func (x MetricAggregation) Enum() *MetricAggregation {
	p := new(MetricAggregation)
	*p = x
	return p
}

func (x MetricAggregation) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MetricAggregation) Descriptor() protoreflect.EnumDescriptor {
	return file_repository_service_proto_enumTypes[3].Descriptor()
}

func (MetricAggregation) Type() protoreflect.EnumType {
	return &file_repository_service_proto_enumTypes[3]
}

func (x MetricAggregation) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MetricAggregation.Descriptor instead.
func (MetricAggregation) EnumDescriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{3}
}

type FilterOperator int32

const (
	FilterOperator_FILTER_OP_UNKNOWN FilterOperator = 0
	FilterOperator_FILTER_OP_ANY     FilterOperator = 1
	FilterOperator_FILTER_OP_ALL     FilterOperator = 2
	FilterOperator_FILTER_OP_NONE    FilterOperator = 3
)

// Enum value maps for FilterOperator.
var (
	FilterOperator_name = map[int32]string{
		0: "FILTER_OP_UNKNOWN",
		1: "FILTER_OP_ANY",
		2: "FILTER_OP_ALL",
		3: "FILTER_OP_NONE",
	}
	FilterOperator_value = map[string]int32{
		"FILTER_OP_UNKNOWN": 0,
		"FILTER_OP_ANY":     1,
		"FILTER_OP_ALL":     2,
		"FILTER_OP_NONE":    3,
	}
)

func (x FilterOperator) Enum() *FilterOperator {
	p := new(FilterOperator)
	*p = x
	return p
}

func (x FilterOperator) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FilterOperator) Descriptor() protoreflect.EnumDescriptor {
	return file_repository_service_proto_enumTypes[4].Descriptor()
}

func (FilterOperator) Type() protoreflect.EnumType {
	return &file_repository_service_proto_enumTypes[4]
}

func (x FilterOperator) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FilterOperator.Descriptor instead.
func (FilterOperator) EnumDescriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{4}
}

type EvictRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to evict versions of.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Version can be set to a specific version to evict, optional.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// UntilVersion can be set to evict all versions up to (inclusive) the
	// specified version, optional. A negative value will be interpreted as
	// relative to the current version, set to -1 to evict all non-current
	// versions.
	UntilVersion  int64 `protobuf:"varint,3,opt,name=until_version,json=untilVersion,proto3" json:"until_version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EvictRequest) Reset() {
	*x = EvictRequest{}
	mi := &file_repository_service_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvictRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvictRequest) ProtoMessage() {}

func (x *EvictRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvictRequest.ProtoReflect.Descriptor instead.
func (*EvictRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{0}
}

func (x *EvictRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *EvictRequest) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *EvictRequest) GetUntilVersion() int64 {
	if x != nil {
		return x.UntilVersion
	}
	return 0
}

type EvictResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// EvictCount is the number of versions that were evicted.
	EvictCount    int64 `protobuf:"varint,1,opt,name=evict_count,json=evictCount,proto3" json:"evict_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EvictResponse) Reset() {
	*x = EvictResponse{}
	mi := &file_repository_service_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EvictResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EvictResponse) ProtoMessage() {}

func (x *EvictResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EvictResponse.ProtoReflect.Descriptor instead.
func (*EvictResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{1}
}

func (x *EvictResponse) GetEvictCount() int64 {
	if x != nil {
		return x.EvictCount
	}
	return 0
}

type GetStatusRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get the status history for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Name of the status to get.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// ID of the status to get. Optional, will default to the latest status.
	Id            int64 `protobuf:"varint,3,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusRequest) Reset() {
	*x = GetStatusRequest{}
	mi := &file_repository_service_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusRequest) ProtoMessage() {}

func (x *GetStatusRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusRequest.ProtoReflect.Descriptor instead.
func (*GetStatusRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{2}
}

func (x *GetStatusRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *GetStatusRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetStatusRequest) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

type GetStatusResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Status        *Status                `protobuf:"bytes,1,opt,name=status,proto3" json:"status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusResponse) Reset() {
	*x = GetStatusResponse{}
	mi := &file_repository_service_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusResponse) ProtoMessage() {}

func (x *GetStatusResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusResponse.ProtoReflect.Descriptor instead.
func (*GetStatusResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{3}
}

func (x *GetStatusResponse) GetStatus() *Status {
	if x != nil {
		return x.Status
	}
	return nil
}

type GetStatusHistoryRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get the status history for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Name of the status to get.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Before is a status ID used to page backwards through the status history. If
	// no ID or a zero ID is provided the list will start with the lastest status.
	Before        int64 `protobuf:"varint,3,opt,name=before,proto3" json:"before,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusHistoryRequest) Reset() {
	*x = GetStatusHistoryRequest{}
	mi := &file_repository_service_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusHistoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusHistoryRequest) ProtoMessage() {}

func (x *GetStatusHistoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusHistoryRequest.ProtoReflect.Descriptor instead.
func (*GetStatusHistoryRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{4}
}

func (x *GetStatusHistoryRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *GetStatusHistoryRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetStatusHistoryRequest) GetBefore() int64 {
	if x != nil {
		return x.Before
	}
	return 0
}

type GetStatusHistoryReponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Statuses      []*Status              `protobuf:"bytes,1,rep,name=statuses,proto3" json:"statuses,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusHistoryReponse) Reset() {
	*x = GetStatusHistoryReponse{}
	mi := &file_repository_service_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusHistoryReponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusHistoryReponse) ProtoMessage() {}

func (x *GetStatusHistoryReponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusHistoryReponse.ProtoReflect.Descriptor instead.
func (*GetStatusHistoryReponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{5}
}

func (x *GetStatusHistoryReponse) GetStatuses() []*Status {
	if x != nil {
		return x.Statuses
	}
	return nil
}

type GetNilStatusesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get the nil statuses for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Names of statuses to get, optional. Defaults to all statuses.
	Names         []string `protobuf:"bytes,2,rep,name=names,proto3" json:"names,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNilStatusesRequest) Reset() {
	*x = GetNilStatusesRequest{}
	mi := &file_repository_service_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNilStatusesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNilStatusesRequest) ProtoMessage() {}

func (x *GetNilStatusesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNilStatusesRequest.ProtoReflect.Descriptor instead.
func (*GetNilStatusesRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{6}
}

func (x *GetNilStatusesRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *GetNilStatusesRequest) GetNames() []string {
	if x != nil {
		return x.Names
	}
	return nil
}

type GetNilStatusesResponse struct {
	state         protoimpl.MessageState       `protogen:"open.v1"`
	Statuses      map[string]*DocumentStatuses `protobuf:"bytes,1,rep,name=statuses,proto3" json:"statuses,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNilStatusesResponse) Reset() {
	*x = GetNilStatusesResponse{}
	mi := &file_repository_service_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNilStatusesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNilStatusesResponse) ProtoMessage() {}

func (x *GetNilStatusesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetNilStatusesResponse.ProtoReflect.Descriptor instead.
func (*GetNilStatusesResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{7}
}

func (x *GetNilStatusesResponse) GetStatuses() map[string]*DocumentStatuses {
	if x != nil {
		return x.Statuses
	}
	return nil
}

type GetStatusOverviewRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUIDs to get statuses for.
	Uuids []string `protobuf:"bytes,1,rep,name=uuids,proto3" json:"uuids,omitempty"`
	// Statuses to return information for. Optional, omit to only return workflow
	// state.
	Statuses []string `protobuf:"bytes,2,rep,name=statuses,proto3" json:"statuses,omitempty"`
	// GetMeta can be used to load any set status metadata.
	GetMeta       bool `protobuf:"varint,3,opt,name=get_meta,json=getMeta,proto3" json:"get_meta,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusOverviewRequest) Reset() {
	*x = GetStatusOverviewRequest{}
	mi := &file_repository_service_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusOverviewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusOverviewRequest) ProtoMessage() {}

func (x *GetStatusOverviewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusOverviewRequest.ProtoReflect.Descriptor instead.
func (*GetStatusOverviewRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{8}
}

func (x *GetStatusOverviewRequest) GetUuids() []string {
	if x != nil {
		return x.Uuids
	}
	return nil
}

func (x *GetStatusOverviewRequest) GetStatuses() []string {
	if x != nil {
		return x.Statuses
	}
	return nil
}

func (x *GetStatusOverviewRequest) GetGetMeta() bool {
	if x != nil {
		return x.GetMeta
	}
	return false
}

type GetStatusOverviewResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*StatusOverviewItem  `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusOverviewResponse) Reset() {
	*x = GetStatusOverviewResponse{}
	mi := &file_repository_service_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusOverviewResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusOverviewResponse) ProtoMessage() {}

func (x *GetStatusOverviewResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusOverviewResponse.ProtoReflect.Descriptor instead.
func (*GetStatusOverviewResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{9}
}

func (x *GetStatusOverviewResponse) GetItems() []*StatusOverviewItem {
	if x != nil {
		return x.Items
	}
	return nil
}

type StatusOverviewItem struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Version is the current version (last written version) of the document.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Modified timestamp is the RFC3339 timestamp for the last change that
	// affected the document.
	Modified string `protobuf:"bytes,3,opt,name=modified,proto3" json:"modified,omitempty"`
	// Heads are the last statuses.
	Heads map[string]*Status `protobuf:"bytes,4,rep,name=heads,proto3" json:"heads,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// WorkflowState that the document is in.
	WorkflowState string `protobuf:"bytes,5,opt,name=workflow_state,json=workflowState,proto3" json:"workflow_state,omitempty"`
	// WorkflowCheckpoint that the document reached most recently.
	WorkflowCheckpoint string `protobuf:"bytes,6,opt,name=workflow_checkpoint,json=workflowCheckpoint,proto3" json:"workflow_checkpoint,omitempty"`
	// CreatorURI is the URI of the subject that created the document.
	CreatorUri string `protobuf:"bytes,7,opt,name=creator_uri,json=creatorUri,proto3" json:"creator_uri,omitempty"`
	// UpdaterURI is the URI of the subject that last updated the document.
	UpdaterUri    string `protobuf:"bytes,8,opt,name=updater_uri,json=updaterUri,proto3" json:"updater_uri,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StatusOverviewItem) Reset() {
	*x = StatusOverviewItem{}
	mi := &file_repository_service_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StatusOverviewItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatusOverviewItem) ProtoMessage() {}

func (x *StatusOverviewItem) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StatusOverviewItem.ProtoReflect.Descriptor instead.
func (*StatusOverviewItem) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{10}
}

func (x *StatusOverviewItem) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *StatusOverviewItem) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *StatusOverviewItem) GetModified() string {
	if x != nil {
		return x.Modified
	}
	return ""
}

func (x *StatusOverviewItem) GetHeads() map[string]*Status {
	if x != nil {
		return x.Heads
	}
	return nil
}

func (x *StatusOverviewItem) GetWorkflowState() string {
	if x != nil {
		return x.WorkflowState
	}
	return ""
}

func (x *StatusOverviewItem) GetWorkflowCheckpoint() string {
	if x != nil {
		return x.WorkflowCheckpoint
	}
	return ""
}

func (x *StatusOverviewItem) GetCreatorUri() string {
	if x != nil {
		return x.CreatorUri
	}
	return ""
}

func (x *StatusOverviewItem) GetUpdaterUri() string {
	if x != nil {
		return x.UpdaterUri
	}
	return ""
}

type GetPermissionsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get the permissions for.
	Uuid          string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPermissionsRequest) Reset() {
	*x = GetPermissionsRequest{}
	mi := &file_repository_service_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPermissionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPermissionsRequest) ProtoMessage() {}

func (x *GetPermissionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPermissionsRequest.ProtoReflect.Descriptor instead.
func (*GetPermissionsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{11}
}

func (x *GetPermissionsRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

type GetPermissionsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Permissions that the client has for the document. The map is keyed by the
	// permission the client has and the value is either the client subject, the
	// unit URI if the permission was granted through a unit membership, or
	// "scope://[name]" if it was granted through elevated privileges.
	Permissions   map[string]string `protobuf:"bytes,1,rep,name=permissions,proto3" json:"permissions,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPermissionsResponse) Reset() {
	*x = GetPermissionsResponse{}
	mi := &file_repository_service_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPermissionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPermissionsResponse) ProtoMessage() {}

func (x *GetPermissionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPermissionsResponse.ProtoReflect.Descriptor instead.
func (*GetPermissionsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{12}
}

func (x *GetPermissionsResponse) GetPermissions() map[string]string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

type GetEventlogRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// After specifies the event ID after which to start returning events. A
	// negative value of -N will start from the N most recent events.
	After int64 `protobuf:"varint,1,opt,name=after,proto3" json:"after,omitempty"`
	// Wait is the maximum time to wait for new events. Defaults to 2000.
	WaitMs int32 `protobuf:"varint,2,opt,name=wait_ms,json=waitMs,proto3" json:"wait_ms,omitempty"`
	// BatchSize is the number of events we want in the response. Defaults to 10.
	BatchSize int32 `protobuf:"varint,3,opt,name=batch_size,json=batchSize,proto3" json:"batch_size,omitempty"`
	// BatchWaitMs is the maximum time we wait for a batch to be filled. Defaults to 200.
	BatchWaitMs   int32 `protobuf:"varint,4,opt,name=batch_wait_ms,json=batchWaitMs,proto3" json:"batch_wait_ms,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetEventlogRequest) Reset() {
	*x = GetEventlogRequest{}
	mi := &file_repository_service_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetEventlogRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetEventlogRequest) ProtoMessage() {}

func (x *GetEventlogRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetEventlogRequest.ProtoReflect.Descriptor instead.
func (*GetEventlogRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{13}
}

func (x *GetEventlogRequest) GetAfter() int64 {
	if x != nil {
		return x.After
	}
	return 0
}

func (x *GetEventlogRequest) GetWaitMs() int32 {
	if x != nil {
		return x.WaitMs
	}
	return 0
}

func (x *GetEventlogRequest) GetBatchSize() int32 {
	if x != nil {
		return x.BatchSize
	}
	return 0
}

func (x *GetEventlogRequest) GetBatchWaitMs() int32 {
	if x != nil {
		return x.BatchWaitMs
	}
	return 0
}

type GetEventlogResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*EventlogItem        `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetEventlogResponse) Reset() {
	*x = GetEventlogResponse{}
	mi := &file_repository_service_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetEventlogResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetEventlogResponse) ProtoMessage() {}

func (x *GetEventlogResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetEventlogResponse.ProtoReflect.Descriptor instead.
func (*GetEventlogResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{14}
}

func (x *GetEventlogResponse) GetItems() []*EventlogItem {
	if x != nil {
		return x.Items
	}
	return nil
}

type GetCompactedEventlogRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// After specifies the event ID after which to start returning events.
	After int64 `protobuf:"varint,1,opt,name=after,proto3" json:"after,omitempty"`
	// Until allows us to control the number of events we want to evaluate when
	// compacting. Must be between 1 and 10000 more than `after`, and cannot be
	// greater than the current last event.
	Until int64 `protobuf:"varint,3,opt,name=until,proto3" json:"until,omitempty"`
	// Limit restricts the number of events we want returned for each
	// call. Together with `offset` this allows us to page through the unique
	// document events between `after` and `until`. Defaults to 0, meaning no
	// limit.
	Limit int32 `protobuf:"varint,4,opt,name=limit,proto3" json:"limit,omitempty"`
	// Offset allows us to skip N number of events relative to `after`. So while
	// `after` and until allows us to paginate through the log as a whole,
	// `offset` and `limit` allows us to page through the unique document events
	// between `after` and `until`. Defaults to 0, meaning no offset.
	Offset int32 `protobuf:"varint,5,opt,name=offset,proto3" json:"offset,omitempty"`
	// Type is used to only return events for documents of the given
	// type. Defaults to empty, which will return events for all document types.
	Type          string `protobuf:"bytes,6,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCompactedEventlogRequest) Reset() {
	*x = GetCompactedEventlogRequest{}
	mi := &file_repository_service_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCompactedEventlogRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCompactedEventlogRequest) ProtoMessage() {}

func (x *GetCompactedEventlogRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCompactedEventlogRequest.ProtoReflect.Descriptor instead.
func (*GetCompactedEventlogRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{15}
}

func (x *GetCompactedEventlogRequest) GetAfter() int64 {
	if x != nil {
		return x.After
	}
	return 0
}

func (x *GetCompactedEventlogRequest) GetUntil() int64 {
	if x != nil {
		return x.Until
	}
	return 0
}

func (x *GetCompactedEventlogRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *GetCompactedEventlogRequest) GetOffset() int32 {
	if x != nil {
		return x.Offset
	}
	return 0
}

func (x *GetCompactedEventlogRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type GetCompactedEventlogResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*EventlogItem        `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetCompactedEventlogResponse) Reset() {
	*x = GetCompactedEventlogResponse{}
	mi := &file_repository_service_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetCompactedEventlogResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetCompactedEventlogResponse) ProtoMessage() {}

func (x *GetCompactedEventlogResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetCompactedEventlogResponse.ProtoReflect.Descriptor instead.
func (*GetCompactedEventlogResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{16}
}

func (x *GetCompactedEventlogResponse) GetItems() []*EventlogItem {
	if x != nil {
		return x.Items
	}
	return nil
}

type EventlogItem struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Id of the event.
	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	// Event type, one of: "document", "status", "acl", "delete_document",
	// "restore_finished", "workflow".
	Event string `protobuf:"bytes,2,opt,name=event,proto3" json:"event,omitempty"`
	// Uuid of the document that was affected.
	Uuid string `protobuf:"bytes,3,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Timestamp that the event occurred, as an RFC3339 timestamp.
	Timestamp string `protobuf:"bytes,4,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	// Version of the document.
	Version int64 `protobuf:"varint,5,opt,name=version,proto3" json:"version,omitempty"`
	// Status that was affected, if it was a "status" event.
	Status string `protobuf:"bytes,6,opt,name=status,proto3" json:"status,omitempty"`
	// StatusId is the ID of the created status.
	StatusId int64 `protobuf:"varint,7,opt,name=status_id,json=statusId,proto3" json:"status_id,omitempty"`
	// Acl entry, if it was an "acl" event.
	Acl []*ACLEntry `protobuf:"bytes,8,rep,name=acl,proto3" json:"acl,omitempty"`
	// UpdaterUri is the URI for the subject that performed the update.
	UpdaterUri string `protobuf:"bytes,9,opt,name=updater_uri,json=updaterUri,proto3" json:"updater_uri,omitempty"`
	// Type of the affected document.
	Type string `protobuf:"bytes,10,opt,name=type,proto3" json:"type,omitempty"`
	// Language of the affected document.
	Language string `protobuf:"bytes,11,opt,name=language,proto3" json:"language,omitempty"`
	// OldLanguage of the document if the document changed language.
	OldLanguage string `protobuf:"bytes,12,opt,name=old_language,json=oldLanguage,proto3" json:"old_language,omitempty"`
	// MainDocument UUID if the document is a metadata document.
	MainDocument string `protobuf:"bytes,13,opt,name=main_document,json=mainDocument,proto3" json:"main_document,omitempty"`
	// SystemState is set to "restoring" if this event was generated from a
	// restore operation. If the event wasn't generated by a special operation it
	// is left empty.
	SystemState string `protobuf:"bytes,14,opt,name=system_state,json=systemState,proto3" json:"system_state,omitempty"`
	// WorkflowState that the document is in.
	WorkflowState string `protobuf:"bytes,15,opt,name=workflow_state,json=workflowState,proto3" json:"workflow_state,omitempty"`
	// WorkflowCheckpoint that the document reached most recently.
	WorkflowCheckpoint string `protobuf:"bytes,16,opt,name=workflow_checkpoint,json=workflowCheckpoint,proto3" json:"workflow_checkpoint,omitempty"`
	// MainDocumentType is the type of the main document if the document is a
	// metadata document.
	MainDocumentType string `protobuf:"bytes,17,opt,name=main_document_type,json=mainDocumentType,proto3" json:"main_document_type,omitempty"`
	// AttachedObjects contains the object names, f.ex. "image" or "audio", when
	// when objects (files) were attached together with a document update.
	AttachedObjects []string `protobuf:"bytes,18,rep,name=attached_objects,json=attachedObjects,proto3" json:"attached_objects,omitempty"`
	// DetachedObjects contains the object names, f.ex. "image" or "audio", when
	// when objects (files) were detached in a document update.
	DetachedObjects []string `protobuf:"bytes,19,rep,name=detached_objects,json=detachedObjects,proto3" json:"detached_objects,omitempty"`
	// DeleteRecordId is the ID of the delete record if this was a delete event.
	DeleteRecordId int64 `protobuf:"varint,20,opt,name=delete_record_id,json=deleteRecordId,proto3" json:"delete_record_id,omitempty"`
	// DocumentNonce is the unique nonce for this document. If the document is
	// deleted and recreated with the same ID it will still have a unique nonce.
	DocumentNonce string `protobuf:"bytes,21,opt,name=document_nonce,json=documentNonce,proto3" json:"document_nonce,omitempty"`
	// Timespans are the timespans that were extracted from the last document
	// update and from the assignment if this document is a deliverable.
	Timespans []*Timespan `protobuf:"bytes,22,rep,name=timespans,proto3" json:"timespans,omitempty"`
	// Labels are the labels that were extracted from the last document update.
	Labels []string `protobuf:"bytes,23,rep,name=labels,proto3" json:"labels,omitempty"`
	// SchemaGeneration is the ID of the schema generation that was used to
	// validate the document.
	SchemaGeneration int64 `protobuf:"varint,24,opt,name=schema_generation,json=schemaGeneration,proto3" json:"schema_generation,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *EventlogItem) Reset() {
	*x = EventlogItem{}
	mi := &file_repository_service_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EventlogItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EventlogItem) ProtoMessage() {}

func (x *EventlogItem) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EventlogItem.ProtoReflect.Descriptor instead.
func (*EventlogItem) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{17}
}

func (x *EventlogItem) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *EventlogItem) GetEvent() string {
	if x != nil {
		return x.Event
	}
	return ""
}

func (x *EventlogItem) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *EventlogItem) GetTimestamp() string {
	if x != nil {
		return x.Timestamp
	}
	return ""
}

func (x *EventlogItem) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *EventlogItem) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *EventlogItem) GetStatusId() int64 {
	if x != nil {
		return x.StatusId
	}
	return 0
}

func (x *EventlogItem) GetAcl() []*ACLEntry {
	if x != nil {
		return x.Acl
	}
	return nil
}

func (x *EventlogItem) GetUpdaterUri() string {
	if x != nil {
		return x.UpdaterUri
	}
	return ""
}

func (x *EventlogItem) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *EventlogItem) GetLanguage() string {
	if x != nil {
		return x.Language
	}
	return ""
}

func (x *EventlogItem) GetOldLanguage() string {
	if x != nil {
		return x.OldLanguage
	}
	return ""
}

func (x *EventlogItem) GetMainDocument() string {
	if x != nil {
		return x.MainDocument
	}
	return ""
}

func (x *EventlogItem) GetSystemState() string {
	if x != nil {
		return x.SystemState
	}
	return ""
}

func (x *EventlogItem) GetWorkflowState() string {
	if x != nil {
		return x.WorkflowState
	}
	return ""
}

func (x *EventlogItem) GetWorkflowCheckpoint() string {
	if x != nil {
		return x.WorkflowCheckpoint
	}
	return ""
}

func (x *EventlogItem) GetMainDocumentType() string {
	if x != nil {
		return x.MainDocumentType
	}
	return ""
}

func (x *EventlogItem) GetAttachedObjects() []string {
	if x != nil {
		return x.AttachedObjects
	}
	return nil
}

func (x *EventlogItem) GetDetachedObjects() []string {
	if x != nil {
		return x.DetachedObjects
	}
	return nil
}

func (x *EventlogItem) GetDeleteRecordId() int64 {
	if x != nil {
		return x.DeleteRecordId
	}
	return 0
}

func (x *EventlogItem) GetDocumentNonce() string {
	if x != nil {
		return x.DocumentNonce
	}
	return ""
}

func (x *EventlogItem) GetTimespans() []*Timespan {
	if x != nil {
		return x.Timespans
	}
	return nil
}

func (x *EventlogItem) GetLabels() []string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *EventlogItem) GetSchemaGeneration() int64 {
	if x != nil {
		return x.SchemaGeneration
	}
	return 0
}

type Timespan struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	From          string                 `protobuf:"bytes,1,opt,name=from,proto3" json:"from,omitempty"`
	To            string                 `protobuf:"bytes,2,opt,name=to,proto3" json:"to,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Timespan) Reset() {
	*x = Timespan{}
	mi := &file_repository_service_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Timespan) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Timespan) ProtoMessage() {}

func (x *Timespan) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Timespan.ProtoReflect.Descriptor instead.
func (*Timespan) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{18}
}

func (x *Timespan) GetFrom() string {
	if x != nil {
		return x.From
	}
	return ""
}

func (x *Timespan) GetTo() string {
	if x != nil {
		return x.To
	}
	return ""
}

type GetStatusRulesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusRulesRequest) Reset() {
	*x = GetStatusRulesRequest{}
	mi := &file_repository_service_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusRulesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusRulesRequest) ProtoMessage() {}

func (x *GetStatusRulesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusRulesRequest.ProtoReflect.Descriptor instead.
func (*GetStatusRulesRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{19}
}

type GetStatusRulesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Rules         []*StatusRule          `protobuf:"bytes,1,rep,name=rules,proto3" json:"rules,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusRulesResponse) Reset() {
	*x = GetStatusRulesResponse{}
	mi := &file_repository_service_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusRulesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusRulesResponse) ProtoMessage() {}

func (x *GetStatusRulesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusRulesResponse.ProtoReflect.Descriptor instead.
func (*GetStatusRulesResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{20}
}

func (x *GetStatusRulesResponse) GetRules() []*StatusRule {
	if x != nil {
		return x.Rules
	}
	return nil
}

type GetStatusesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type of documents to get statuses from.
	Type          string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusesRequest) Reset() {
	*x = GetStatusesRequest{}
	mi := &file_repository_service_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusesRequest) ProtoMessage() {}

func (x *GetStatusesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusesRequest.ProtoReflect.Descriptor instead.
func (*GetStatusesRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{21}
}

func (x *GetStatusesRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type SetWorkflowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type of the document that this workflow is for.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Workflow declaration.
	Workflow      *DocumentWorkflow `protobuf:"bytes,2,opt,name=workflow,proto3" json:"workflow,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetWorkflowRequest) Reset() {
	*x = SetWorkflowRequest{}
	mi := &file_repository_service_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetWorkflowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetWorkflowRequest) ProtoMessage() {}

func (x *SetWorkflowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetWorkflowRequest.ProtoReflect.Descriptor instead.
func (*SetWorkflowRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{22}
}

func (x *SetWorkflowRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *SetWorkflowRequest) GetWorkflow() *DocumentWorkflow {
	if x != nil {
		return x.Workflow
	}
	return nil
}

type DocumentWorkflow struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// StepZero is the step that the state begins in, or reverts to after a
	// checkpoint.
	StepZero string `protobuf:"bytes,1,opt,name=step_zero,json=stepZero,proto3" json:"step_zero,omitempty"`
	// Checkpoint is the status that should be used as the state checkpoint.
	Checkpoint string `protobuf:"bytes,2,opt,name=checkpoint,proto3" json:"checkpoint,omitempty"`
	// NegativeCheckpoint is the state that should be used when the checkpoint
	// status is set using a negative version.
	NegativeCheckpoint string `protobuf:"bytes,3,opt,name=negative_checkpoint,json=negativeCheckpoint,proto3" json:"negative_checkpoint,omitempty"`
	// Steps are the names of statuses that should be used as steps between
	// checkpoints.
	Steps         []string `protobuf:"bytes,4,rep,name=steps,proto3" json:"steps,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentWorkflow) Reset() {
	*x = DocumentWorkflow{}
	mi := &file_repository_service_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentWorkflow) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentWorkflow) ProtoMessage() {}

func (x *DocumentWorkflow) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentWorkflow.ProtoReflect.Descriptor instead.
func (*DocumentWorkflow) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{23}
}

func (x *DocumentWorkflow) GetStepZero() string {
	if x != nil {
		return x.StepZero
	}
	return ""
}

func (x *DocumentWorkflow) GetCheckpoint() string {
	if x != nil {
		return x.Checkpoint
	}
	return ""
}

func (x *DocumentWorkflow) GetNegativeCheckpoint() string {
	if x != nil {
		return x.NegativeCheckpoint
	}
	return ""
}

func (x *DocumentWorkflow) GetSteps() []string {
	if x != nil {
		return x.Steps
	}
	return nil
}

type SetWorkflowResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetWorkflowResponse) Reset() {
	*x = SetWorkflowResponse{}
	mi := &file_repository_service_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetWorkflowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetWorkflowResponse) ProtoMessage() {}

func (x *SetWorkflowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetWorkflowResponse.ProtoReflect.Descriptor instead.
func (*SetWorkflowResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{24}
}

type GetWorkflowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type of the document to get workflow for.
	Type          string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetWorkflowRequest) Reset() {
	*x = GetWorkflowRequest{}
	mi := &file_repository_service_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetWorkflowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetWorkflowRequest) ProtoMessage() {}

func (x *GetWorkflowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetWorkflowRequest.ProtoReflect.Descriptor instead.
func (*GetWorkflowRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{25}
}

func (x *GetWorkflowRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type GetWorkflowResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Workflow declaration.
	Workflow *DocumentWorkflow `protobuf:"bytes,1,opt,name=workflow,proto3" json:"workflow,omitempty"`
	// UpdaterURI identifies the user that last updated the workflow.
	UpdaterUri string `protobuf:"bytes,2,opt,name=updater_uri,json=updaterUri,proto3" json:"updater_uri,omitempty"`
	// Updated is the update time as a RFC3339 timestamp.
	Updated       string `protobuf:"bytes,3,opt,name=updated,proto3" json:"updated,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetWorkflowResponse) Reset() {
	*x = GetWorkflowResponse{}
	mi := &file_repository_service_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetWorkflowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetWorkflowResponse) ProtoMessage() {}

func (x *GetWorkflowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetWorkflowResponse.ProtoReflect.Descriptor instead.
func (*GetWorkflowResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{26}
}

func (x *GetWorkflowResponse) GetWorkflow() *DocumentWorkflow {
	if x != nil {
		return x.Workflow
	}
	return nil
}

func (x *GetWorkflowResponse) GetUpdaterUri() string {
	if x != nil {
		return x.UpdaterUri
	}
	return ""
}

func (x *GetWorkflowResponse) GetUpdated() string {
	if x != nil {
		return x.Updated
	}
	return ""
}

type DeleteWorkflowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type of the document to remove the workflow for.
	Type          string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteWorkflowRequest) Reset() {
	*x = DeleteWorkflowRequest{}
	mi := &file_repository_service_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteWorkflowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteWorkflowRequest) ProtoMessage() {}

func (x *DeleteWorkflowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteWorkflowRequest.ProtoReflect.Descriptor instead.
func (*DeleteWorkflowRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{27}
}

func (x *DeleteWorkflowRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type DeleteWorkflowResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteWorkflowResponse) Reset() {
	*x = DeleteWorkflowResponse{}
	mi := &file_repository_service_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteWorkflowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteWorkflowResponse) ProtoMessage() {}

func (x *DeleteWorkflowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteWorkflowResponse.ProtoReflect.Descriptor instead.
func (*DeleteWorkflowResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{28}
}

type WorkflowStatus struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Type          string                 `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WorkflowStatus) Reset() {
	*x = WorkflowStatus{}
	mi := &file_repository_service_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WorkflowStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorkflowStatus) ProtoMessage() {}

func (x *WorkflowStatus) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorkflowStatus.ProtoReflect.Descriptor instead.
func (*WorkflowStatus) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{29}
}

func (x *WorkflowStatus) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *WorkflowStatus) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetStatusesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Statuses      []*WorkflowStatus      `protobuf:"bytes,1,rep,name=statuses,proto3" json:"statuses,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStatusesResponse) Reset() {
	*x = GetStatusesResponse{}
	mi := &file_repository_service_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStatusesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStatusesResponse) ProtoMessage() {}

func (x *GetStatusesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStatusesResponse.ProtoReflect.Descriptor instead.
func (*GetStatusesResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{30}
}

func (x *GetStatusesResponse) GetStatuses() []*WorkflowStatus {
	if x != nil {
		return x.Statuses
	}
	return nil
}

type UpdateStatusRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type that the status is valid for.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Name of the status.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Disabled disables the status so that it cannot be used when setting new
	// statuses.
	Disabled      bool `protobuf:"varint,3,opt,name=disabled,proto3" json:"disabled,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStatusRequest) Reset() {
	*x = UpdateStatusRequest{}
	mi := &file_repository_service_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStatusRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStatusRequest) ProtoMessage() {}

func (x *UpdateStatusRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStatusRequest.ProtoReflect.Descriptor instead.
func (*UpdateStatusRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{31}
}

func (x *UpdateStatusRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *UpdateStatusRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateStatusRequest) GetDisabled() bool {
	if x != nil {
		return x.Disabled
	}
	return false
}

type UpdateStatusResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStatusResponse) Reset() {
	*x = UpdateStatusResponse{}
	mi := &file_repository_service_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStatusResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStatusResponse) ProtoMessage() {}

func (x *UpdateStatusResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStatusResponse.ProtoReflect.Descriptor instead.
func (*UpdateStatusResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{32}
}

type CreateStatusRuleRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Rule          *StatusRule            `protobuf:"bytes,1,opt,name=rule,proto3" json:"rule,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateStatusRuleRequest) Reset() {
	*x = CreateStatusRuleRequest{}
	mi := &file_repository_service_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateStatusRuleRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateStatusRuleRequest) ProtoMessage() {}

func (x *CreateStatusRuleRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateStatusRuleRequest.ProtoReflect.Descriptor instead.
func (*CreateStatusRuleRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{33}
}

func (x *CreateStatusRuleRequest) GetRule() *StatusRule {
	if x != nil {
		return x.Rule
	}
	return nil
}

type StatusRule struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type that the status rule applies to.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Name of the rule.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Description of the rule.
	Description string `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	// AccessRule whether this rule acts as access control.
	AccessRule bool `protobuf:"varint,4,opt,name=access_rule,json=accessRule,proto3" json:"access_rule,omitempty"`
	// AppliesTo is a list of statuses that this rule applies to.
	AppliesTo []string `protobuf:"bytes,5,rep,name=applies_to,json=appliesTo,proto3" json:"applies_to,omitempty"`
	// Expression that is evaluated for the rule.
	Expression    string `protobuf:"bytes,7,opt,name=expression,proto3" json:"expression,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StatusRule) Reset() {
	*x = StatusRule{}
	mi := &file_repository_service_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StatusRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatusRule) ProtoMessage() {}

func (x *StatusRule) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StatusRule.ProtoReflect.Descriptor instead.
func (*StatusRule) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{34}
}

func (x *StatusRule) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *StatusRule) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *StatusRule) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *StatusRule) GetAccessRule() bool {
	if x != nil {
		return x.AccessRule
	}
	return false
}

func (x *StatusRule) GetAppliesTo() []string {
	if x != nil {
		return x.AppliesTo
	}
	return nil
}

func (x *StatusRule) GetExpression() string {
	if x != nil {
		return x.Expression
	}
	return ""
}

type CreateStatusRuleResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateStatusRuleResponse) Reset() {
	*x = CreateStatusRuleResponse{}
	mi := &file_repository_service_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateStatusRuleResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateStatusRuleResponse) ProtoMessage() {}

func (x *CreateStatusRuleResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateStatusRuleResponse.ProtoReflect.Descriptor instead.
func (*CreateStatusRuleResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{35}
}

type DeleteStatusRuleRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Type          string                 `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteStatusRuleRequest) Reset() {
	*x = DeleteStatusRuleRequest{}
	mi := &file_repository_service_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteStatusRuleRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteStatusRuleRequest) ProtoMessage() {}

func (x *DeleteStatusRuleRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteStatusRuleRequest.ProtoReflect.Descriptor instead.
func (*DeleteStatusRuleRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{36}
}

func (x *DeleteStatusRuleRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *DeleteStatusRuleRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteStatusRuleResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteStatusRuleResponse) Reset() {
	*x = DeleteStatusRuleResponse{}
	mi := &file_repository_service_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteStatusRuleResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteStatusRuleResponse) ProtoMessage() {}

func (x *DeleteStatusRuleResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteStatusRuleResponse.ProtoReflect.Descriptor instead.
func (*DeleteStatusRuleResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{37}
}

type GetDocumentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Version to get, omit to get latest (or use status).
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Status is used to fetch the version of the document references by the last
	// status update. Can be used instead of specifying a version.
	Status string `protobuf:"bytes,3,opt,name=status,proto3" json:"status,omitempty"`
	// Lock, when set, acquires a write lock on the document as part of the Get
	// request. By default the lock only affects the creation of new versions;
	// statuses and ACLs can still be updated. Set the lock exclusivity to also
	// block status and/or ACL updates. The granted lock is returned in the
	// response on success.
	Lock *AcquireLock `protobuf:"bytes,4,opt,name=lock,proto3" json:"lock,omitempty"`
	// MetaDocument controls the inclusion of associated meta
	// documents. "META_INCLUDE" will include the document if it
	// exists. "META_ONLY" will only return the meta document, and will return a
	// not found error if it doesn't exist.
	MetaDocument GetMetaDoc `protobuf:"varint,5,opt,name=meta_document,json=metaDocument,proto3,enum=elephant.repository.GetMetaDoc" json:"meta_document,omitempty"`
	// MetaDocumentVersion is used to fetch a specific version of meta document.
	// Can not be used with status. If status is specified,
	// the version the meta document had at the time the status was set is returned.
	// If ommited and status is not used, the latest meta document version is returned.
	MetaDocumentVersion int64 `protobuf:"varint,6,opt,name=meta_document_version,json=metaDocumentVersion,proto3" json:"meta_document_version,omitempty"`
	// Subset returns a subset of the document as specified by a list of newsdoc
	// value extractors.
	Subset        []string `protobuf:"bytes,7,rep,name=subset,proto3" json:"subset,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentRequest) Reset() {
	*x = GetDocumentRequest{}
	mi := &file_repository_service_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentRequest) ProtoMessage() {}

func (x *GetDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{38}
}

func (x *GetDocumentRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *GetDocumentRequest) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *GetDocumentRequest) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *GetDocumentRequest) GetLock() *AcquireLock {
	if x != nil {
		return x.Lock
	}
	return nil
}

func (x *GetDocumentRequest) GetMetaDocument() GetMetaDoc {
	if x != nil {
		return x.MetaDocument
	}
	return GetMetaDoc_META_NONE
}

func (x *GetDocumentRequest) GetMetaDocumentVersion() int64 {
	if x != nil {
		return x.MetaDocumentVersion
	}
	return 0
}

func (x *GetDocumentRequest) GetSubset() []string {
	if x != nil {
		return x.Subset
	}
	return nil
}

type GetDocumentResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Document is the requested document.
	Document *newsdoc.Document `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	// Version is the version of the returned document.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Status is returned if the document was requested by status name.
	Status *Status `protobuf:"bytes,3,opt,name=status,proto3" json:"status,omitempty"`
	// Meta contains the meta document, if requested, and if it exists.
	Meta *MetaDocument `protobuf:"bytes,4,opt,name=meta,proto3" json:"meta,omitempty"`
	// IsMetaDocument is true if the document is a meta document.
	IsMetaDocument bool `protobuf:"varint,5,opt,name=is_meta_document,json=isMetaDocument,proto3" json:"is_meta_document,omitempty"`
	// MainDocument is the UUID of the main document if this is a meta document.
	MainDocument string `protobuf:"bytes,6,opt,name=main_document,json=mainDocument,proto3" json:"main_document,omitempty"`
	// Subset is the extracted subset of the document.
	Subset []*ExtractedValues `protobuf:"bytes,7,rep,name=subset,proto3" json:"subset,omitempty"`
	// Lock is the granted lock when the request set lock.
	Lock          *LockGrant `protobuf:"bytes,8,opt,name=lock,proto3" json:"lock,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentResponse) Reset() {
	*x = GetDocumentResponse{}
	mi := &file_repository_service_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentResponse) ProtoMessage() {}

func (x *GetDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{39}
}

func (x *GetDocumentResponse) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *GetDocumentResponse) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *GetDocumentResponse) GetStatus() *Status {
	if x != nil {
		return x.Status
	}
	return nil
}

func (x *GetDocumentResponse) GetMeta() *MetaDocument {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *GetDocumentResponse) GetIsMetaDocument() bool {
	if x != nil {
		return x.IsMetaDocument
	}
	return false
}

func (x *GetDocumentResponse) GetMainDocument() string {
	if x != nil {
		return x.MainDocument
	}
	return ""
}

func (x *GetDocumentResponse) GetSubset() []*ExtractedValues {
	if x != nil {
		return x.Subset
	}
	return nil
}

func (x *GetDocumentResponse) GetLock() *LockGrant {
	if x != nil {
		return x.Lock
	}
	return nil
}

type ExtractedValues struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Extractor index for the the extractor that produced the values.
	Extractor int64 `protobuf:"varint,1,opt,name=extractor,proto3" json:"extractor,omitempty"`
	// Values that were extracted.
	Values        map[string]*ExtractedValue `protobuf:"bytes,2,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExtractedValues) Reset() {
	*x = ExtractedValues{}
	mi := &file_repository_service_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExtractedValues) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExtractedValues) ProtoMessage() {}

func (x *ExtractedValues) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExtractedValues.ProtoReflect.Descriptor instead.
func (*ExtractedValues) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{40}
}

func (x *ExtractedValues) GetExtractor() int64 {
	if x != nil {
		return x.Extractor
	}
	return 0
}

func (x *ExtractedValues) GetValues() map[string]*ExtractedValue {
	if x != nil {
		return x.Values
	}
	return nil
}

type ExtractedValue struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Value is used for extracted attributes and data values.
	Value string `protobuf:"bytes,1,opt,name=value,proto3" json:"value,omitempty"`
	// Block is used for block extractors.
	Block *newsdoc.Block `protobuf:"bytes,2,opt,name=block,proto3" json:"block,omitempty"`
	// Annotations can be used to differentiate between different extracted values.
	Annotation string `protobuf:"bytes,3,opt,name=annotation,proto3" json:"annotation,omitempty"`
	// Role can be used to differentiate between different extracted values.
	Role          string `protobuf:"bytes,4,opt,name=role,proto3" json:"role,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExtractedValue) Reset() {
	*x = ExtractedValue{}
	mi := &file_repository_service_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExtractedValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExtractedValue) ProtoMessage() {}

func (x *ExtractedValue) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExtractedValue.ProtoReflect.Descriptor instead.
func (*ExtractedValue) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{41}
}

func (x *ExtractedValue) GetValue() string {
	if x != nil {
		return x.Value
	}
	return ""
}

func (x *ExtractedValue) GetBlock() *newsdoc.Block {
	if x != nil {
		return x.Block
	}
	return nil
}

func (x *ExtractedValue) GetAnnotation() string {
	if x != nil {
		return x.Annotation
	}
	return ""
}

func (x *ExtractedValue) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

type BulkGetRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Documents to get.
	Documents []*BulkGetReference `protobuf:"bytes,1,rep,name=documents,proto3" json:"documents,omitempty"`
	// Subset returns a subset of the documents as specified by a list of newsdoc
	// value extractors.
	Subset        []string `protobuf:"bytes,2,rep,name=subset,proto3" json:"subset,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetRequest) Reset() {
	*x = BulkGetRequest{}
	mi := &file_repository_service_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetRequest) ProtoMessage() {}

func (x *BulkGetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetRequest.ProtoReflect.Descriptor instead.
func (*BulkGetRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{42}
}

func (x *BulkGetRequest) GetDocuments() []*BulkGetReference {
	if x != nil {
		return x.Documents
	}
	return nil
}

func (x *BulkGetRequest) GetSubset() []string {
	if x != nil {
		return x.Subset
	}
	return nil
}

type BulkGetReference struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Version to get, omit to get latest.
	Version       int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetReference) Reset() {
	*x = BulkGetReference{}
	mi := &file_repository_service_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetReference) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetReference) ProtoMessage() {}

func (x *BulkGetReference) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetReference.ProtoReflect.Descriptor instead.
func (*BulkGetReference) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{43}
}

func (x *BulkGetReference) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *BulkGetReference) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

type BulkGetResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*BulkGetItem         `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetResponse) Reset() {
	*x = BulkGetResponse{}
	mi := &file_repository_service_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetResponse) ProtoMessage() {}

func (x *BulkGetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetResponse.ProtoReflect.Descriptor instead.
func (*BulkGetResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{44}
}

func (x *BulkGetResponse) GetItems() []*BulkGetItem {
	if x != nil {
		return x.Items
	}
	return nil
}

type BulkGetItem struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Document is the requested document.
	Document *newsdoc.Document `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	// Version is the version of the returned document.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// UUID of the loaded document.
	Uuid string `protobuf:"bytes,3,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Subset is the extracted subset of the document.
	Subset        []*ExtractedValues `protobuf:"bytes,4,rep,name=subset,proto3" json:"subset,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetItem) Reset() {
	*x = BulkGetItem{}
	mi := &file_repository_service_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetItem) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetItem) ProtoMessage() {}

func (x *BulkGetItem) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetItem.ProtoReflect.Descriptor instead.
func (*BulkGetItem) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{45}
}

func (x *BulkGetItem) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *BulkGetItem) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *BulkGetItem) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *BulkGetItem) GetSubset() []*ExtractedValues {
	if x != nil {
		return x.Subset
	}
	return nil
}

type MetaDocument struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Document is the requested document.
	Document *newsdoc.Document `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	// Version is the version of the returned document.
	Version       int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MetaDocument) Reset() {
	*x = MetaDocument{}
	mi := &file_repository_service_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MetaDocument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MetaDocument) ProtoMessage() {}

func (x *MetaDocument) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MetaDocument.ProtoReflect.Descriptor instead.
func (*MetaDocument) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{46}
}

func (x *MetaDocument) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *MetaDocument) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

type GetHistoryRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get the history for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Before is which version number we should start fetching history from, omit
	// to start with the latest version.
	Before int64 `protobuf:"varint,2,opt,name=before,proto3" json:"before,omitempty"`
	// LoadStatuses loads any statuses set on the document versions.
	LoadStatuses  bool `protobuf:"varint,3,opt,name=load_statuses,json=loadStatuses,proto3" json:"load_statuses,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetHistoryRequest) Reset() {
	*x = GetHistoryRequest{}
	mi := &file_repository_service_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetHistoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetHistoryRequest) ProtoMessage() {}

func (x *GetHistoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetHistoryRequest.ProtoReflect.Descriptor instead.
func (*GetHistoryRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{47}
}

func (x *GetHistoryRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *GetHistoryRequest) GetBefore() int64 {
	if x != nil {
		return x.Before
	}
	return 0
}

func (x *GetHistoryRequest) GetLoadStatuses() bool {
	if x != nil {
		return x.LoadStatuses
	}
	return false
}

type GetHistoryResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Versions      []*DocumentVersion     `protobuf:"bytes,1,rep,name=versions,proto3" json:"versions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetHistoryResponse) Reset() {
	*x = GetHistoryResponse{}
	mi := &file_repository_service_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetHistoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetHistoryResponse) ProtoMessage() {}

func (x *GetHistoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetHistoryResponse.ProtoReflect.Descriptor instead.
func (*GetHistoryResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{48}
}

func (x *GetHistoryResponse) GetVersions() []*DocumentVersion {
	if x != nil {
		return x.Versions
	}
	return nil
}

type DocumentVersion struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Version of the document.
	Version int64 `protobuf:"varint,1,opt,name=version,proto3" json:"version,omitempty"`
	// Created timestamp is the RFC3339 timestamp for when the version was
	// created.
	Created string `protobuf:"bytes,2,opt,name=created,proto3" json:"created,omitempty"`
	// Creator of the version.
	Creator string `protobuf:"bytes,3,opt,name=creator,proto3" json:"creator,omitempty"`
	// Meta data to for the document version.
	Meta map[string]string `protobuf:"bytes,4,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Statuses that have been set for the document version.
	Statuses      map[string]*DocumentStatuses `protobuf:"bytes,5,rep,name=statuses,proto3" json:"statuses,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentVersion) Reset() {
	*x = DocumentVersion{}
	mi := &file_repository_service_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentVersion) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentVersion) ProtoMessage() {}

func (x *DocumentVersion) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentVersion.ProtoReflect.Descriptor instead.
func (*DocumentVersion) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{49}
}

func (x *DocumentVersion) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *DocumentVersion) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *DocumentVersion) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *DocumentVersion) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *DocumentVersion) GetStatuses() map[string]*DocumentStatuses {
	if x != nil {
		return x.Statuses
	}
	return nil
}

type DocumentStatuses struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*Status              `protobuf:"bytes,5,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentStatuses) Reset() {
	*x = DocumentStatuses{}
	mi := &file_repository_service_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentStatuses) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentStatuses) ProtoMessage() {}

func (x *DocumentStatuses) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentStatuses.ProtoReflect.Descriptor instead.
func (*DocumentStatuses) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{50}
}

func (x *DocumentStatuses) GetItems() []*Status {
	if x != nil {
		return x.Items
	}
	return nil
}

// UpdateRequest creates a new document version and/or sets statuses and ACLs
// for a document. The update is transactional will fail or succed as a whole.
//
// If the document fails validation an "invalid_argument" error will be returned
// with the number of errors as meta "err_count" and the individual errors as
// "0", "1", "2"... Use the Validate method to get th errors as structured data.
type UpdateRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to update.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Document version to create.
	Document *newsdoc.Document `protobuf:"bytes,2,opt,name=document,proto3" json:"document,omitempty"`
	// Meta data to associate with the document version.
	Meta map[string]string `protobuf:"bytes,3,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// IfMatch is used for optimistic locks. Set to the version that you require
	// to be the current one for the update to be performed, or -1 to only perform
	// the update if the document doesn't already exist.
	IfMatch int64 `protobuf:"varint,4,opt,name=if_match,json=ifMatch,proto3" json:"if_match,omitempty"`
	// Status updates to perform.
	Status []*StatusUpdate `protobuf:"bytes,5,rep,name=status,proto3" json:"status,omitempty"`
	// ACL is an ACL list controlling access to the document.
	Acl []*ACLEntry `protobuf:"bytes,6,rep,name=acl,proto3" json:"acl,omitempty"`
	// ImportDirective can be used to preserve timestamps and authorship
	// information from originating systems, but requires the "import_directive"
	// scope for use.
	ImportDirective *ImportDirective `protobuf:"bytes,7,opt,name=import_directive,json=importDirective,proto3" json:"import_directive,omitempty"`
	// LockToken is used for explicit pessimistic locking.
	LockToken string `protobuf:"bytes,8,opt,name=lockToken,proto3" json:"lockToken,omitempty"`
	// UpdateMetaDocument can be set to true to update the meta document instead
	// of the document identified by the UUID.
	UpdateMetaDocument bool `protobuf:"varint,9,opt,name=update_meta_document,json=updateMetaDocument,proto3" json:"update_meta_document,omitempty"`
	// IfWorkflowState is used to only perform the update if it's in the specified
	// workflow state.
	IfWorkflowState string `protobuf:"bytes,10,opt,name=if_workflow_state,json=ifWorkflowState,proto3" json:"if_workflow_state,omitempty"`
	// IfStatusHeads is used to only perform the update if the status heads match.
	IfStatusHeads map[string]int64 `protobuf:"bytes,11,rep,name=if_status_heads,json=ifStatusHeads,proto3" json:"if_status_heads,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"varint,2,opt,name=value"`
	// AttachObjects is used to attach uploaded objects to a document. This must
	// be done in combination with a document create or update. The key is the
	// object name, f.ex. "image" or "audio", and the value is an upload ID. See
	// CreateUpload().
	AttachObjects map[string]string `protobuf:"bytes,12,rep,name=attach_objects,json=attachObjects,proto3" json:"attach_objects,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// DetatchObjects is used to detach uploaded objects from a document. This
	// must be done in combination with a document create or update.
	DetachObjects []string `protobuf:"bytes,13,rep,name=detach_objects,json=detachObjects,proto3" json:"detach_objects,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateRequest) Reset() {
	*x = UpdateRequest{}
	mi := &file_repository_service_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRequest) ProtoMessage() {}

func (x *UpdateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRequest.ProtoReflect.Descriptor instead.
func (*UpdateRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{51}
}

func (x *UpdateRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *UpdateRequest) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *UpdateRequest) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *UpdateRequest) GetIfMatch() int64 {
	if x != nil {
		return x.IfMatch
	}
	return 0
}

func (x *UpdateRequest) GetStatus() []*StatusUpdate {
	if x != nil {
		return x.Status
	}
	return nil
}

func (x *UpdateRequest) GetAcl() []*ACLEntry {
	if x != nil {
		return x.Acl
	}
	return nil
}

func (x *UpdateRequest) GetImportDirective() *ImportDirective {
	if x != nil {
		return x.ImportDirective
	}
	return nil
}

func (x *UpdateRequest) GetLockToken() string {
	if x != nil {
		return x.LockToken
	}
	return ""
}

func (x *UpdateRequest) GetUpdateMetaDocument() bool {
	if x != nil {
		return x.UpdateMetaDocument
	}
	return false
}

func (x *UpdateRequest) GetIfWorkflowState() string {
	if x != nil {
		return x.IfWorkflowState
	}
	return ""
}

func (x *UpdateRequest) GetIfStatusHeads() map[string]int64 {
	if x != nil {
		return x.IfStatusHeads
	}
	return nil
}

func (x *UpdateRequest) GetAttachObjects() map[string]string {
	if x != nil {
		return x.AttachObjects
	}
	return nil
}

func (x *UpdateRequest) GetDetachObjects() []string {
	if x != nil {
		return x.DetachObjects
	}
	return nil
}

// ImportDirective can be used to preserve timestamps and authorship information
// from originating systems.
type ImportDirective struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	OriginallyCreated string                 `protobuf:"bytes,1,opt,name=originally_created,json=originallyCreated,proto3" json:"originally_created,omitempty"`
	OriginalCreator   string                 `protobuf:"bytes,2,opt,name=original_creator,json=originalCreator,proto3" json:"original_creator,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ImportDirective) Reset() {
	*x = ImportDirective{}
	mi := &file_repository_service_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ImportDirective) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ImportDirective) ProtoMessage() {}

func (x *ImportDirective) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ImportDirective.ProtoReflect.Descriptor instead.
func (*ImportDirective) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{52}
}

func (x *ImportDirective) GetOriginallyCreated() string {
	if x != nil {
		return x.OriginallyCreated
	}
	return ""
}

func (x *ImportDirective) GetOriginalCreator() string {
	if x != nil {
		return x.OriginalCreator
	}
	return ""
}

// UpdateResponse describes the result of an update request.
type UpdateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       int64                  `protobuf:"varint,1,opt,name=version,proto3" json:"version,omitempty"`
	Uuid          string                 `protobuf:"bytes,2,opt,name=uuid,proto3" json:"uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateResponse) Reset() {
	*x = UpdateResponse{}
	mi := &file_repository_service_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateResponse) ProtoMessage() {}

func (x *UpdateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateResponse.ProtoReflect.Descriptor instead.
func (*UpdateResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{53}
}

func (x *UpdateResponse) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *UpdateResponse) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

type BulkUpdateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Updates       []*UpdateRequest       `protobuf:"bytes,1,rep,name=updates,proto3" json:"updates,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkUpdateRequest) Reset() {
	*x = BulkUpdateRequest{}
	mi := &file_repository_service_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkUpdateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkUpdateRequest) ProtoMessage() {}

func (x *BulkUpdateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkUpdateRequest.ProtoReflect.Descriptor instead.
func (*BulkUpdateRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{54}
}

func (x *BulkUpdateRequest) GetUpdates() []*UpdateRequest {
	if x != nil {
		return x.Updates
	}
	return nil
}

type BulkUpdateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Updates       []*UpdateResponse      `protobuf:"bytes,1,rep,name=updates,proto3" json:"updates,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkUpdateResponse) Reset() {
	*x = BulkUpdateResponse{}
	mi := &file_repository_service_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkUpdateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkUpdateResponse) ProtoMessage() {}

func (x *BulkUpdateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkUpdateResponse.ProtoReflect.Descriptor instead.
func (*BulkUpdateResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{55}
}

func (x *BulkUpdateResponse) GetUpdates() []*UpdateResponse {
	if x != nil {
		return x.Updates
	}
	return nil
}

type ValidateRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Document      *newsdoc.Document      `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ValidateRequest) Reset() {
	*x = ValidateRequest{}
	mi := &file_repository_service_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ValidateRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ValidateRequest) ProtoMessage() {}

func (x *ValidateRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ValidateRequest.ProtoReflect.Descriptor instead.
func (*ValidateRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{56}
}

func (x *ValidateRequest) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

type ValidateResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Errors        []*ValidationResult    `protobuf:"bytes,1,rep,name=errors,proto3" json:"errors,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ValidateResponse) Reset() {
	*x = ValidateResponse{}
	mi := &file_repository_service_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ValidateResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ValidateResponse) ProtoMessage() {}

func (x *ValidateResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ValidateResponse.ProtoReflect.Descriptor instead.
func (*ValidateResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{57}
}

func (x *ValidateResponse) GetErrors() []*ValidationResult {
	if x != nil {
		return x.Errors
	}
	return nil
}

type PruneRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Document      *newsdoc.Document      `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PruneRequest) Reset() {
	*x = PruneRequest{}
	mi := &file_repository_service_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PruneRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PruneRequest) ProtoMessage() {}

func (x *PruneRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PruneRequest.ProtoReflect.Descriptor instead.
func (*PruneRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{58}
}

func (x *PruneRequest) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

type PruneResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Document is the pruned document.
	Document *newsdoc.Document `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	// Errors are validation issues that could not be resolved by pruning.
	Errors        []*ValidationResult `protobuf:"bytes,2,rep,name=errors,proto3" json:"errors,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PruneResponse) Reset() {
	*x = PruneResponse{}
	mi := &file_repository_service_proto_msgTypes[59]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PruneResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PruneResponse) ProtoMessage() {}

func (x *PruneResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[59]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PruneResponse.ProtoReflect.Descriptor instead.
func (*PruneResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{59}
}

func (x *PruneResponse) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *PruneResponse) GetErrors() []*ValidationResult {
	if x != nil {
		return x.Errors
	}
	return nil
}

type ValidationResult struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Entity        []*EntityRef           `protobuf:"bytes,1,rep,name=entity,proto3" json:"entity,omitempty"`
	Error         string                 `protobuf:"bytes,2,opt,name=error,proto3" json:"error,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ValidationResult) Reset() {
	*x = ValidationResult{}
	mi := &file_repository_service_proto_msgTypes[60]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ValidationResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ValidationResult) ProtoMessage() {}

func (x *ValidationResult) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[60]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ValidationResult.ProtoReflect.Descriptor instead.
func (*ValidationResult) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{60}
}

func (x *ValidationResult) GetEntity() []*EntityRef {
	if x != nil {
		return x.Entity
	}
	return nil
}

func (x *ValidationResult) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

type EntityRef struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// RefType is type of entity that's referenced, one of "block", "property",
	// "attribute", "data attribute", or "parameter."
	RefType string `protobuf:"bytes,1,opt,name=ref_type,json=refType,proto3" json:"ref_type,omitempty"`
	// Kind is the block type, one of "link", "meta", or "content".
	Kind string `protobuf:"bytes,2,opt,name=kind,proto3" json:"kind,omitempty"`
	// Index is the position of the entity in an array, where applicable.
	Index int64 `protobuf:"varint,3,opt,name=index,proto3" json:"index,omitempty"`
	// Name is the name of the entity, where applicable.
	Name string `protobuf:"bytes,4,opt,name=name,proto3" json:"name,omitempty"`
	// Type is the type of the entity, where applicable.
	Type string `protobuf:"bytes,5,opt,name=type,proto3" json:"type,omitempty"`
	// Rel is the relationship for the entity, where applicable.
	Rel           string `protobuf:"bytes,6,opt,name=rel,proto3" json:"rel,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EntityRef) Reset() {
	*x = EntityRef{}
	mi := &file_repository_service_proto_msgTypes[61]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EntityRef) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EntityRef) ProtoMessage() {}

func (x *EntityRef) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[61]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EntityRef.ProtoReflect.Descriptor instead.
func (*EntityRef) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{61}
}

func (x *EntityRef) GetRefType() string {
	if x != nil {
		return x.RefType
	}
	return ""
}

func (x *EntityRef) GetKind() string {
	if x != nil {
		return x.Kind
	}
	return ""
}

func (x *EntityRef) GetIndex() int64 {
	if x != nil {
		return x.Index
	}
	return 0
}

func (x *EntityRef) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *EntityRef) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *EntityRef) GetRel() string {
	if x != nil {
		return x.Rel
	}
	return ""
}

type StatusUpdate struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the status to set.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Version to set the status for. Set to -1 to signal that the status no
	// longer is valid for the document. Optional if part of a document update
	// request as it then will default to the version being written.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Meta data to include with the status update.
	Meta map[string]string `protobuf:"bytes,3,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// IfMatch is used for optimistic locks. Set to the status id that you require
	// to be the current one for the status to be set, or -1 to only perform the
	// update if the status hasn't previously been set.
	IfMatch       int64 `protobuf:"varint,4,opt,name=if_match,json=ifMatch,proto3" json:"if_match,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StatusUpdate) Reset() {
	*x = StatusUpdate{}
	mi := &file_repository_service_proto_msgTypes[62]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StatusUpdate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StatusUpdate) ProtoMessage() {}

func (x *StatusUpdate) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[62]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StatusUpdate.ProtoReflect.Descriptor instead.
func (*StatusUpdate) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{62}
}

func (x *StatusUpdate) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *StatusUpdate) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *StatusUpdate) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *StatusUpdate) GetIfMatch() int64 {
	if x != nil {
		return x.IfMatch
	}
	return 0
}

type UpdatePermissionsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to update permissions for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Set (upsert based on URI) this list of ACL entries.
	List          []*ACLEntry `protobuf:"bytes,2,rep,name=list,proto3" json:"list,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdatePermissionsRequest) Reset() {
	*x = UpdatePermissionsRequest{}
	mi := &file_repository_service_proto_msgTypes[63]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdatePermissionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdatePermissionsRequest) ProtoMessage() {}

func (x *UpdatePermissionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[63]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdatePermissionsRequest.ProtoReflect.Descriptor instead.
func (*UpdatePermissionsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{63}
}

func (x *UpdatePermissionsRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *UpdatePermissionsRequest) GetList() []*ACLEntry {
	if x != nil {
		return x.List
	}
	return nil
}

type UpdatePermissionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdatePermissionsResponse) Reset() {
	*x = UpdatePermissionsResponse{}
	mi := &file_repository_service_proto_msgTypes[64]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdatePermissionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdatePermissionsResponse) ProtoMessage() {}

func (x *UpdatePermissionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdatePermissionsResponse.ProtoReflect.Descriptor instead.
func (*UpdatePermissionsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{64}
}

type DeleteDocumentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to delete.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Meta data to include with the delete record.
	Meta map[string]string `protobuf:"bytes,2,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// IfMatch is used for optimistic locks. Set to the version that you require
	// to be the current one for the delete to be performed.
	IfMatch int64 `protobuf:"varint,3,opt,name=if_match,json=ifMatch,proto3" json:"if_match,omitempty"`
	// LockToken is used for explicit pessimistic locking.
	LockToken     string `protobuf:"bytes,8,opt,name=lockToken,proto3" json:"lockToken,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteDocumentRequest) Reset() {
	*x = DeleteDocumentRequest{}
	mi := &file_repository_service_proto_msgTypes[65]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDocumentRequest) ProtoMessage() {}

func (x *DeleteDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[65]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDocumentRequest.ProtoReflect.Descriptor instead.
func (*DeleteDocumentRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{65}
}

func (x *DeleteDocumentRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *DeleteDocumentRequest) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *DeleteDocumentRequest) GetIfMatch() int64 {
	if x != nil {
		return x.IfMatch
	}
	return 0
}

func (x *DeleteDocumentRequest) GetLockToken() string {
	if x != nil {
		return x.LockToken
	}
	return ""
}

type DeleteDocumentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteDocumentResponse) Reset() {
	*x = DeleteDocumentResponse{}
	mi := &file_repository_service_proto_msgTypes[66]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDocumentResponse) ProtoMessage() {}

func (x *DeleteDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[66]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDocumentResponse.ProtoReflect.Descriptor instead.
func (*DeleteDocumentResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{66}
}

type RestoreRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to restore.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// DeleteRecordID for the specific instance of the document that was deleted.
	DeleteRecordId int64 `protobuf:"varint,2,opt,name=delete_record_id,json=deleteRecordId,proto3" json:"delete_record_id,omitempty"`
	// ACL that should be set for the restored document.
	Acl           []*ACLEntry `protobuf:"bytes,3,rep,name=acl,proto3" json:"acl,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RestoreRequest) Reset() {
	*x = RestoreRequest{}
	mi := &file_repository_service_proto_msgTypes[67]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RestoreRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RestoreRequest) ProtoMessage() {}

func (x *RestoreRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[67]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RestoreRequest.ProtoReflect.Descriptor instead.
func (*RestoreRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{67}
}

func (x *RestoreRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *RestoreRequest) GetDeleteRecordId() int64 {
	if x != nil {
		return x.DeleteRecordId
	}
	return 0
}

func (x *RestoreRequest) GetAcl() []*ACLEntry {
	if x != nil {
		return x.Acl
	}
	return nil
}

type RestoreResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RestoreResponse) Reset() {
	*x = RestoreResponse{}
	mi := &file_repository_service_proto_msgTypes[68]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RestoreResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RestoreResponse) ProtoMessage() {}

func (x *RestoreResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[68]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RestoreResponse.ProtoReflect.Descriptor instead.
func (*RestoreResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{68}
}

type PurgeRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to purge.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// DeleteRecordID for the specific instance of the document that should be
	// purged.
	DeleteRecordId int64 `protobuf:"varint,2,opt,name=delete_record_id,json=deleteRecordId,proto3" json:"delete_record_id,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *PurgeRequest) Reset() {
	*x = PurgeRequest{}
	mi := &file_repository_service_proto_msgTypes[69]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PurgeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PurgeRequest) ProtoMessage() {}

func (x *PurgeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[69]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PurgeRequest.ProtoReflect.Descriptor instead.
func (*PurgeRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{69}
}

func (x *PurgeRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *PurgeRequest) GetDeleteRecordId() int64 {
	if x != nil {
		return x.DeleteRecordId
	}
	return 0
}

type PurgeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PurgeResponse) Reset() {
	*x = PurgeResponse{}
	mi := &file_repository_service_proto_msgTypes[70]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PurgeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PurgeResponse) ProtoMessage() {}

func (x *PurgeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[70]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PurgeResponse.ProtoReflect.Descriptor instead.
func (*PurgeResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{70}
}

type ListDeletedRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document list deletions of. Optional.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// BeforeID is used to paginate backwards in deletions. Optional.
	BeforeId int64 `protobuf:"varint,2,opt,name=before_id,json=beforeId,proto3" json:"before_id,omitempty"`
	// BeforeDate is used to get deletions going back from before the specified
	// date YYYY-MM-DD. Optional, will be ignored if before_id is specified.
	BeforeDate string `protobuf:"bytes,3,opt,name=before_date,json=beforeDate,proto3" json:"before_date,omitempty"`
	// Timezone that the date should be interpreted in.
	Timezone      string `protobuf:"bytes,4,opt,name=timezone,proto3" json:"timezone,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeletedRequest) Reset() {
	*x = ListDeletedRequest{}
	mi := &file_repository_service_proto_msgTypes[71]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeletedRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeletedRequest) ProtoMessage() {}

func (x *ListDeletedRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[71]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeletedRequest.ProtoReflect.Descriptor instead.
func (*ListDeletedRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{71}
}

func (x *ListDeletedRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *ListDeletedRequest) GetBeforeId() int64 {
	if x != nil {
		return x.BeforeId
	}
	return 0
}

func (x *ListDeletedRequest) GetBeforeDate() string {
	if x != nil {
		return x.BeforeDate
	}
	return ""
}

func (x *ListDeletedRequest) GetTimezone() string {
	if x != nil {
		return x.Timezone
	}
	return ""
}

type ListDeletedResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Deletes that have been performed on document instances with the given UUID.
	Deletes       []*DeleteRecord `protobuf:"bytes,1,rep,name=deletes,proto3" json:"deletes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeletedResponse) Reset() {
	*x = ListDeletedResponse{}
	mi := &file_repository_service_proto_msgTypes[72]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeletedResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeletedResponse) ProtoMessage() {}

func (x *ListDeletedResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[72]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeletedResponse.ProtoReflect.Descriptor instead.
func (*ListDeletedResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{72}
}

func (x *ListDeletedResponse) GetDeletes() []*DeleteRecord {
	if x != nil {
		return x.Deletes
	}
	return nil
}

type DeleteRecord struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the delete record.
	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	// UUID of the document.
	Uuid string `protobuf:"bytes,2,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// URI of the document.
	Uri string `protobuf:"bytes,3,opt,name=uri,proto3" json:"uri,omitempty"`
	// Type of the document.
	Type string `protobuf:"bytes,4,opt,name=type,proto3" json:"type,omitempty"`
	// Version that the document had at the time of restore.
	Version int64 `protobuf:"varint,5,opt,name=version,proto3" json:"version,omitempty"`
	// Creation time for the delete record, as an RFC3339 timestamp.
	Created string `protobuf:"bytes,6,opt,name=created,proto3" json:"created,omitempty"`
	// Creator of the delete record.
	Creator string `protobuf:"bytes,7,opt,name=creator,proto3" json:"creator,omitempty"`
	// Meta data included with the delete record.
	Meta map[string]string `protobuf:"bytes,8,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Language that the document was in.
	Language string `protobuf:"bytes,9,opt,name=language,proto3" json:"language,omitempty"`
	// Finalised is the time, as an RFC3339 timestamp, that the document finished
	// deleting. Empty if the delete is in progress.
	Finalised string `protobuf:"bytes,10,opt,name=finalised,proto3" json:"finalised,omitempty"`
	// Purged is the time, as an RFC3339 timestamp, all archived data for the
	// document was purged. Empty if the document hasn't been purged.
	Purged        string `protobuf:"bytes,11,opt,name=purged,proto3" json:"purged,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRecord) Reset() {
	*x = DeleteRecord{}
	mi := &file_repository_service_proto_msgTypes[73]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRecord) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRecord) ProtoMessage() {}

func (x *DeleteRecord) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[73]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRecord.ProtoReflect.Descriptor instead.
func (*DeleteRecord) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{73}
}

func (x *DeleteRecord) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *DeleteRecord) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *DeleteRecord) GetUri() string {
	if x != nil {
		return x.Uri
	}
	return ""
}

func (x *DeleteRecord) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *DeleteRecord) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *DeleteRecord) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *DeleteRecord) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *DeleteRecord) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *DeleteRecord) GetLanguage() string {
	if x != nil {
		return x.Language
	}
	return ""
}

func (x *DeleteRecord) GetFinalised() string {
	if x != nil {
		return x.Finalised
	}
	return ""
}

func (x *DeleteRecord) GetPurged() string {
	if x != nil {
		return x.Purged
	}
	return ""
}

type GetMetaRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to get metadata for
	Uuid          string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetaRequest) Reset() {
	*x = GetMetaRequest{}
	mi := &file_repository_service_proto_msgTypes[74]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetaRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetaRequest) ProtoMessage() {}

func (x *GetMetaRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[74]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetaRequest.ProtoReflect.Descriptor instead.
func (*GetMetaRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{74}
}

func (x *GetMetaRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

type GetMetaResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Meta          *DocumentMeta          `protobuf:"bytes,1,opt,name=meta,proto3" json:"meta,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetaResponse) Reset() {
	*x = GetMetaResponse{}
	mi := &file_repository_service_proto_msgTypes[75]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetaResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetaResponse) ProtoMessage() {}

func (x *GetMetaResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[75]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetaResponse.ProtoReflect.Descriptor instead.
func (*GetMetaResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{75}
}

func (x *GetMetaResponse) GetMeta() *DocumentMeta {
	if x != nil {
		return x.Meta
	}
	return nil
}

type DocumentMeta struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Created timestamp is the RFC3339 timestamp for when the document was
	// created.
	Created string `protobuf:"bytes,1,opt,name=created,proto3" json:"created,omitempty"`
	// Modified timestamp is the RFC3339 timestamp for the last change that
	// affected the document.
	Modified string `protobuf:"bytes,2,opt,name=modified,proto3" json:"modified,omitempty"`
	// CurrentVersion is the last written version of the document.
	CurrentVersion int64 `protobuf:"varint,3,opt,name=current_version,json=currentVersion,proto3" json:"current_version,omitempty"`
	// Heads are the last statuses.
	Heads map[string]*Status `protobuf:"bytes,4,rep,name=heads,proto3" json:"heads,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// ACL is an ACL list controlling access to the document.
	Acl []*ACLEntry `protobuf:"bytes,5,rep,name=acl,proto3" json:"acl,omitempty"`
	// Document lock, if any
	Lock *Lock `protobuf:"bytes,6,opt,name=lock,proto3" json:"lock,omitempty"`
	// IsMetaDocument is true if the document is a meta document.
	IsMetaDocument bool `protobuf:"varint,7,opt,name=is_meta_document,json=isMetaDocument,proto3" json:"is_meta_document,omitempty"`
	// MainDocument is the UUID of the main document if this is a meta document.
	MainDocument string `protobuf:"bytes,8,opt,name=main_document,json=mainDocument,proto3" json:"main_document,omitempty"`
	// WorkflowState that the document is in.
	WorkflowState string `protobuf:"bytes,9,opt,name=workflow_state,json=workflowState,proto3" json:"workflow_state,omitempty"`
	// WorkflowCheckpoint that the document reached most recently.
	WorkflowCheckpoint string `protobuf:"bytes,10,opt,name=workflow_checkpoint,json=workflowCheckpoint,proto3" json:"workflow_checkpoint,omitempty"`
	// CreatorURI is the identity of the party that created the document.
	CreatorUri string `protobuf:"bytes,11,opt,name=creator_uri,json=creatorUri,proto3" json:"creator_uri,omitempty"`
	// UpdaterURI is the identity of the party that last updated the document.
	UpdaterUri string `protobuf:"bytes,12,opt,name=updater_uri,json=updaterUri,proto3" json:"updater_uri,omitempty"`
	// Attachments are the names of the attached objects (files) for a document.
	Attachments []*AttachmentRef `protobuf:"bytes,13,rep,name=attachments,proto3" json:"attachments,omitempty"`
	// Nonce is the unique nonce for this document. If the document is deleted and
	// recreated with the same ID it will still have a unique nonce.
	Nonce string `protobuf:"bytes,14,opt,name=nonce,proto3" json:"nonce,omitempty"`
	// Type is the type of the document.
	Type          string `protobuf:"bytes,15,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentMeta) Reset() {
	*x = DocumentMeta{}
	mi := &file_repository_service_proto_msgTypes[76]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentMeta) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentMeta) ProtoMessage() {}

func (x *DocumentMeta) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[76]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentMeta.ProtoReflect.Descriptor instead.
func (*DocumentMeta) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{76}
}

func (x *DocumentMeta) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *DocumentMeta) GetModified() string {
	if x != nil {
		return x.Modified
	}
	return ""
}

func (x *DocumentMeta) GetCurrentVersion() int64 {
	if x != nil {
		return x.CurrentVersion
	}
	return 0
}

func (x *DocumentMeta) GetHeads() map[string]*Status {
	if x != nil {
		return x.Heads
	}
	return nil
}

func (x *DocumentMeta) GetAcl() []*ACLEntry {
	if x != nil {
		return x.Acl
	}
	return nil
}

func (x *DocumentMeta) GetLock() *Lock {
	if x != nil {
		return x.Lock
	}
	return nil
}

func (x *DocumentMeta) GetIsMetaDocument() bool {
	if x != nil {
		return x.IsMetaDocument
	}
	return false
}

func (x *DocumentMeta) GetMainDocument() string {
	if x != nil {
		return x.MainDocument
	}
	return ""
}

func (x *DocumentMeta) GetWorkflowState() string {
	if x != nil {
		return x.WorkflowState
	}
	return ""
}

func (x *DocumentMeta) GetWorkflowCheckpoint() string {
	if x != nil {
		return x.WorkflowCheckpoint
	}
	return ""
}

func (x *DocumentMeta) GetCreatorUri() string {
	if x != nil {
		return x.CreatorUri
	}
	return ""
}

func (x *DocumentMeta) GetUpdaterUri() string {
	if x != nil {
		return x.UpdaterUri
	}
	return ""
}

func (x *DocumentMeta) GetAttachments() []*AttachmentRef {
	if x != nil {
		return x.Attachments
	}
	return nil
}

func (x *DocumentMeta) GetNonce() string {
	if x != nil {
		return x.Nonce
	}
	return ""
}

func (x *DocumentMeta) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type AttachmentRef struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the attachment.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Version is the current version of the attachment.
	Version       int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AttachmentRef) Reset() {
	*x = AttachmentRef{}
	mi := &file_repository_service_proto_msgTypes[77]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AttachmentRef) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AttachmentRef) ProtoMessage() {}

func (x *AttachmentRef) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[77]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AttachmentRef.ProtoReflect.Descriptor instead.
func (*AttachmentRef) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{77}
}

func (x *AttachmentRef) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *AttachmentRef) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

type Status struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the status.
	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	// Version of the document that the status refers to.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Creator of the status.
	Creator string `protobuf:"bytes,3,opt,name=creator,proto3" json:"creator,omitempty"`
	// Created timestamp is the RFC3339 timestamp for when the status was created.
	Created string `protobuf:"bytes,4,opt,name=created,proto3" json:"created,omitempty"`
	// Meta data for the status.
	Meta map[string]string `protobuf:"bytes,5,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// MetaDocumentVersion is the version the meta document had at the time the
	// status was set.
	MetaDocVersion int64 `protobuf:"varint,6,opt,name=meta_doc_version,json=metaDocVersion,proto3" json:"meta_doc_version,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *Status) Reset() {
	*x = Status{}
	mi := &file_repository_service_proto_msgTypes[78]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Status) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Status) ProtoMessage() {}

func (x *Status) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[78]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Status.ProtoReflect.Descriptor instead.
func (*Status) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{78}
}

func (x *Status) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *Status) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *Status) GetCreator() string {
	if x != nil {
		return x.Creator
	}
	return ""
}

func (x *Status) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *Status) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

func (x *Status) GetMetaDocVersion() int64 {
	if x != nil {
		return x.MetaDocVersion
	}
	return 0
}

type ACLEntry struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// URI that identifies the party that's granted access.
	Uri string `protobuf:"bytes,1,opt,name=uri,proto3" json:"uri,omitempty"`
	// Permissions given to the grantee.
	Permissions   []string `protobuf:"bytes,2,rep,name=permissions,proto3" json:"permissions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ACLEntry) Reset() {
	*x = ACLEntry{}
	mi := &file_repository_service_proto_msgTypes[79]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ACLEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ACLEntry) ProtoMessage() {}

func (x *ACLEntry) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[79]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ACLEntry.ProtoReflect.Descriptor instead.
func (*ACLEntry) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{79}
}

func (x *ACLEntry) GetUri() string {
	if x != nil {
		return x.Uri
	}
	return ""
}

func (x *ACLEntry) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

type Lock struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Lock token string
	Token string `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	// URI of the lock owner
	Uri string `protobuf:"bytes,2,opt,name=uri,proto3" json:"uri,omitempty"`
	// Created timestamp is the RFC3339 timestamp for when the lock was created.
	Created string `protobuf:"bytes,3,opt,name=created,proto3" json:"created,omitempty"`
	// Expires timestamp is the RFC3339 timestamp for when the lock will expire.
	Expires string `protobuf:"bytes,4,opt,name=expires,proto3" json:"expires,omitempty"`
	// URI identifying the locking application.
	App string `protobuf:"bytes,5,opt,name=app,proto3" json:"app,omitempty"`
	// Optional free-form comment.
	Comment string `protobuf:"bytes,6,opt,name=comment,proto3" json:"comment,omitempty"`
	// Exclusivity level of the lock.
	Exclusivity   LockExclusivity `protobuf:"varint,7,opt,name=exclusivity,proto3,enum=elephant.repository.LockExclusivity" json:"exclusivity,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Lock) Reset() {
	*x = Lock{}
	mi := &file_repository_service_proto_msgTypes[80]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Lock) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Lock) ProtoMessage() {}

func (x *Lock) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[80]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Lock.ProtoReflect.Descriptor instead.
func (*Lock) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{80}
}

func (x *Lock) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *Lock) GetUri() string {
	if x != nil {
		return x.Uri
	}
	return ""
}

func (x *Lock) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *Lock) GetExpires() string {
	if x != nil {
		return x.Expires
	}
	return ""
}

func (x *Lock) GetApp() string {
	if x != nil {
		return x.App
	}
	return ""
}

func (x *Lock) GetComment() string {
	if x != nil {
		return x.Comment
	}
	return ""
}

func (x *Lock) GetExclusivity() LockExclusivity {
	if x != nil {
		return x.Exclusivity
	}
	return LockExclusivity_LOCK_DOCUMENT
}

type RegisterMetaTypeRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type to use as a meta type.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Exclusive - set to true to only allow the type to be used as a meta type.
	Exclusive     bool `protobuf:"varint,2,opt,name=exclusive,proto3" json:"exclusive,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetaTypeRequest) Reset() {
	*x = RegisterMetaTypeRequest{}
	mi := &file_repository_service_proto_msgTypes[81]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetaTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetaTypeRequest) ProtoMessage() {}

func (x *RegisterMetaTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[81]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetaTypeRequest.ProtoReflect.Descriptor instead.
func (*RegisterMetaTypeRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{81}
}

func (x *RegisterMetaTypeRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *RegisterMetaTypeRequest) GetExclusive() bool {
	if x != nil {
		return x.Exclusive
	}
	return false
}

type RegisterMetaTypeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetaTypeResponse) Reset() {
	*x = RegisterMetaTypeResponse{}
	mi := &file_repository_service_proto_msgTypes[82]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetaTypeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetaTypeResponse) ProtoMessage() {}

func (x *RegisterMetaTypeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[82]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetaTypeResponse.ProtoReflect.Descriptor instead.
func (*RegisterMetaTypeResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{82}
}

type RegisterMetaTypeUseRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// MainType to use the meta type for.
	MainType string `protobuf:"bytes,1,opt,name=main_type,json=mainType,proto3" json:"main_type,omitempty"`
	// MetaType to use for the main document.
	MetaType      string `protobuf:"bytes,2,opt,name=meta_type,json=metaType,proto3" json:"meta_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetaTypeUseRequest) Reset() {
	*x = RegisterMetaTypeUseRequest{}
	mi := &file_repository_service_proto_msgTypes[83]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetaTypeUseRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetaTypeUseRequest) ProtoMessage() {}

func (x *RegisterMetaTypeUseRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[83]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetaTypeUseRequest.ProtoReflect.Descriptor instead.
func (*RegisterMetaTypeUseRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{83}
}

func (x *RegisterMetaTypeUseRequest) GetMainType() string {
	if x != nil {
		return x.MainType
	}
	return ""
}

func (x *RegisterMetaTypeUseRequest) GetMetaType() string {
	if x != nil {
		return x.MetaType
	}
	return ""
}

type RegisterMetaTypeUseResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetaTypeUseResponse) Reset() {
	*x = RegisterMetaTypeUseResponse{}
	mi := &file_repository_service_proto_msgTypes[84]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetaTypeUseResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetaTypeUseResponse) ProtoMessage() {}

func (x *RegisterMetaTypeUseResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[84]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetaTypeUseResponse.ProtoReflect.Descriptor instead.
func (*RegisterMetaTypeUseResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{84}
}

type GetMetaTypesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetaTypesRequest) Reset() {
	*x = GetMetaTypesRequest{}
	mi := &file_repository_service_proto_msgTypes[85]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetaTypesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetaTypesRequest) ProtoMessage() {}

func (x *GetMetaTypesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[85]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetaTypesRequest.ProtoReflect.Descriptor instead.
func (*GetMetaTypesRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{85}
}

type GetMetaTypesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Types         []*MetaTypeInfo        `protobuf:"bytes,1,rep,name=types,proto3" json:"types,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetaTypesResponse) Reset() {
	*x = GetMetaTypesResponse{}
	mi := &file_repository_service_proto_msgTypes[86]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetaTypesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetaTypesResponse) ProtoMessage() {}

func (x *GetMetaTypesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[86]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetaTypesResponse.ProtoReflect.Descriptor instead.
func (*GetMetaTypesResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{86}
}

func (x *GetMetaTypesResponse) GetTypes() []*MetaTypeInfo {
	if x != nil {
		return x.Types
	}
	return nil
}

type GetDocumentTypesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentTypesRequest) Reset() {
	*x = GetDocumentTypesRequest{}
	mi := &file_repository_service_proto_msgTypes[87]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentTypesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentTypesRequest) ProtoMessage() {}

func (x *GetDocumentTypesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[87]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentTypesRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentTypesRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{87}
}

type GetDocumentTypesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that have been defined in the schema.
	Types         []string `protobuf:"bytes,1,rep,name=types,proto3" json:"types,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDocumentTypesResponse) Reset() {
	*x = GetDocumentTypesResponse{}
	mi := &file_repository_service_proto_msgTypes[88]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDocumentTypesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentTypesResponse) ProtoMessage() {}

func (x *GetDocumentTypesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[88]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentTypesResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentTypesResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{88}
}

func (x *GetDocumentTypesResponse) GetTypes() []string {
	if x != nil {
		return x.Types
	}
	return nil
}

type MetaTypeInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	UsedBy        []string               `protobuf:"bytes,2,rep,name=used_by,json=usedBy,proto3" json:"used_by,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MetaTypeInfo) Reset() {
	*x = MetaTypeInfo{}
	mi := &file_repository_service_proto_msgTypes[89]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MetaTypeInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MetaTypeInfo) ProtoMessage() {}

func (x *MetaTypeInfo) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[89]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MetaTypeInfo.ProtoReflect.Descriptor instead.
func (*MetaTypeInfo) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{89}
}

func (x *MetaTypeInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *MetaTypeInfo) GetUsedBy() []string {
	if x != nil {
		return x.UsedBy
	}
	return nil
}

type RegisterGenerationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Schemas to register.
	Schemas []*Schema `protobuf:"bytes,1,rep,name=schemas,proto3" json:"schemas,omitempty"`
	// Activation rule for the registered schema generation.
	Activation SchemaActivation `protobuf:"varint,2,opt,name=activation,proto3,enum=elephant.repository.SchemaActivation" json:"activation,omitempty"`
	// Examplars are documents that serve as examples of what can be expressed
	// with the schemas. If any of the exemplars are invalid the registration
	// request will be rejected.
	Exemplars     []*newsdoc.Document `protobuf:"bytes,3,rep,name=exemplars,proto3" json:"exemplars,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterGenerationRequest) Reset() {
	*x = RegisterGenerationRequest{}
	mi := &file_repository_service_proto_msgTypes[90]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterGenerationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterGenerationRequest) ProtoMessage() {}

func (x *RegisterGenerationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[90]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterGenerationRequest.ProtoReflect.Descriptor instead.
func (*RegisterGenerationRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{90}
}

func (x *RegisterGenerationRequest) GetSchemas() []*Schema {
	if x != nil {
		return x.Schemas
	}
	return nil
}

func (x *RegisterGenerationRequest) GetActivation() SchemaActivation {
	if x != nil {
		return x.Activation
	}
	return SchemaActivation_ACTIVATION_UNKNOWN
}

func (x *RegisterGenerationRequest) GetExemplars() []*newsdoc.Document {
	if x != nil {
		return x.Exemplars
	}
	return nil
}

type RegisterGenerationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// GenerationID is the ID of the created schema generation.
	GenerationId  int64 `protobuf:"varint,1,opt,name=generation_id,json=generationId,proto3" json:"generation_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterGenerationResponse) Reset() {
	*x = RegisterGenerationResponse{}
	mi := &file_repository_service_proto_msgTypes[91]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterGenerationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterGenerationResponse) ProtoMessage() {}

func (x *RegisterGenerationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[91]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterGenerationResponse.ProtoReflect.Descriptor instead.
func (*RegisterGenerationResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{91}
}

func (x *RegisterGenerationResponse) GetGenerationId() int64 {
	if x != nil {
		return x.GenerationId
	}
	return 0
}

type SetActiveSchemasRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// GenerationID is the ID of the generation to activate.
	GenerationId int64 `protobuf:"varint,1,opt,name=generation_id,json=generationId,proto3" json:"generation_id,omitempty"`
	// Activation setting.
	Activation    SchemaActivation `protobuf:"varint,2,opt,name=activation,proto3,enum=elephant.repository.SchemaActivation" json:"activation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetActiveSchemasRequest) Reset() {
	*x = SetActiveSchemasRequest{}
	mi := &file_repository_service_proto_msgTypes[92]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetActiveSchemasRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetActiveSchemasRequest) ProtoMessage() {}

func (x *SetActiveSchemasRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[92]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetActiveSchemasRequest.ProtoReflect.Descriptor instead.
func (*SetActiveSchemasRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{92}
}

func (x *SetActiveSchemasRequest) GetGenerationId() int64 {
	if x != nil {
		return x.GenerationId
	}
	return 0
}

func (x *SetActiveSchemasRequest) GetActivation() SchemaActivation {
	if x != nil {
		return x.Activation
	}
	return SchemaActivation_ACTIVATION_UNKNOWN
}

type SetActiveSchemasResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetActiveSchemasResponse) Reset() {
	*x = SetActiveSchemasResponse{}
	mi := &file_repository_service_proto_msgTypes[93]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetActiveSchemasResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetActiveSchemasResponse) ProtoMessage() {}

func (x *SetActiveSchemasResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[93]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetActiveSchemasResponse.ProtoReflect.Descriptor instead.
func (*SetActiveSchemasResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{93}
}

type ListSchemaGenerationsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Before is used to paginate backwards based on the generation ID.
	Before        int64 `protobuf:"varint,1,opt,name=before,proto3" json:"before,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListSchemaGenerationsRequest) Reset() {
	*x = ListSchemaGenerationsRequest{}
	mi := &file_repository_service_proto_msgTypes[94]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListSchemaGenerationsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListSchemaGenerationsRequest) ProtoMessage() {}

func (x *ListSchemaGenerationsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[94]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListSchemaGenerationsRequest.ProtoReflect.Descriptor instead.
func (*ListSchemaGenerationsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{94}
}

func (x *ListSchemaGenerationsRequest) GetBefore() int64 {
	if x != nil {
		return x.Before
	}
	return 0
}

type ListSchemaGenerationsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*SchemaGeneration    `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListSchemaGenerationsResponse) Reset() {
	*x = ListSchemaGenerationsResponse{}
	mi := &file_repository_service_proto_msgTypes[95]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListSchemaGenerationsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListSchemaGenerationsResponse) ProtoMessage() {}

func (x *ListSchemaGenerationsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[95]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListSchemaGenerationsResponse.ProtoReflect.Descriptor instead.
func (*ListSchemaGenerationsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{95}
}

func (x *ListSchemaGenerationsResponse) GetItems() []*SchemaGeneration {
	if x != nil {
		return x.Items
	}
	return nil
}

type SchemaGeneration struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the schema generation.
	Id int64 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	// Status of the generation.
	Status SchemaActivation `protobuf:"varint,2,opt,name=status,proto3,enum=elephant.repository.SchemaActivation" json:"status,omitempty"`
	// Created is the time (RFC3339) the schema generation was first registered.
	Created string `protobuf:"bytes,3,opt,name=created,proto3" json:"created,omitempty"`
	// Activated is the time (RFC3339) the schema generation was last activated (if it has been activated).
	Activated string `protobuf:"bytes,4,opt,name=activated,proto3" json:"activated,omitempty"`
	// Deactivated is the time (RFC3339) the schema generation was last deactivated (if it has been deactivated).
	Deactivated string `protobuf:"bytes,5,opt,name=deactivated,proto3" json:"deactivated,omitempty"`
	// Schemas are the schemas that are included in the generation.
	Schemas       []*SchemaReference `protobuf:"bytes,6,rep,name=schemas,proto3" json:"schemas,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SchemaGeneration) Reset() {
	*x = SchemaGeneration{}
	mi := &file_repository_service_proto_msgTypes[96]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SchemaGeneration) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SchemaGeneration) ProtoMessage() {}

func (x *SchemaGeneration) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[96]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SchemaGeneration.ProtoReflect.Descriptor instead.
func (*SchemaGeneration) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{96}
}

func (x *SchemaGeneration) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *SchemaGeneration) GetStatus() SchemaActivation {
	if x != nil {
		return x.Status
	}
	return SchemaActivation_ACTIVATION_UNKNOWN
}

func (x *SchemaGeneration) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *SchemaGeneration) GetActivated() string {
	if x != nil {
		return x.Activated
	}
	return ""
}

func (x *SchemaGeneration) GetDeactivated() string {
	if x != nil {
		return x.Deactivated
	}
	return ""
}

func (x *SchemaGeneration) GetSchemas() []*SchemaReference {
	if x != nil {
		return x.Schemas
	}
	return nil
}

type GetExemplarsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// GenerationID is the ID of the schema generation to get exemplars for.
	GenerationId int64 `protobuf:"varint,1,opt,name=generation_id,json=generationId,proto3" json:"generation_id,omitempty"`
	// Known exemplar version hashes that the client already has, keyed by
	// exemplar name. Exemplars with matching version hashes will not be included
	// in the response.
	Known         map[string]string `protobuf:"bytes,2,rep,name=known,proto3" json:"known,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExemplarsRequest) Reset() {
	*x = GetExemplarsRequest{}
	mi := &file_repository_service_proto_msgTypes[97]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExemplarsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExemplarsRequest) ProtoMessage() {}

func (x *GetExemplarsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[97]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExemplarsRequest.ProtoReflect.Descriptor instead.
func (*GetExemplarsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{97}
}

func (x *GetExemplarsRequest) GetGenerationId() int64 {
	if x != nil {
		return x.GenerationId
	}
	return 0
}

func (x *GetExemplarsRequest) GetKnown() map[string]string {
	if x != nil {
		return x.Known
	}
	return nil
}

type GetExemplarsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Exemplars are the exemplar documents for the schema generation.
	Exemplars     []*Exemplar `protobuf:"bytes,1,rep,name=exemplars,proto3" json:"exemplars,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetExemplarsResponse) Reset() {
	*x = GetExemplarsResponse{}
	mi := &file_repository_service_proto_msgTypes[98]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetExemplarsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetExemplarsResponse) ProtoMessage() {}

func (x *GetExemplarsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[98]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetExemplarsResponse.ProtoReflect.Descriptor instead.
func (*GetExemplarsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{98}
}

func (x *GetExemplarsResponse) GetExemplars() []*Exemplar {
	if x != nil {
		return x.Exemplars
	}
	return nil
}

type Exemplar struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the exemplar.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Document is the exemplar newsdoc document.
	Document *newsdoc.Document `protobuf:"bytes,2,opt,name=document,proto3" json:"document,omitempty"`
	// VersionHash is the version hash of the exemplar document.
	VersionHash   string `protobuf:"bytes,3,opt,name=version_hash,json=versionHash,proto3" json:"version_hash,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Exemplar) Reset() {
	*x = Exemplar{}
	mi := &file_repository_service_proto_msgTypes[99]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Exemplar) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Exemplar) ProtoMessage() {}

func (x *Exemplar) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[99]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Exemplar.ProtoReflect.Descriptor instead.
func (*Exemplar) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{99}
}

func (x *Exemplar) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Exemplar) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *Exemplar) GetVersionHash() string {
	if x != nil {
		return x.VersionHash
	}
	return ""
}

type GetSchemaRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the schema to get.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Version of the schema to get, optional. The currently active version of the
	// schema will be returned if no version is specified.
	Version       string `protobuf:"bytes,2,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSchemaRequest) Reset() {
	*x = GetSchemaRequest{}
	mi := &file_repository_service_proto_msgTypes[100]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSchemaRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSchemaRequest) ProtoMessage() {}

func (x *GetSchemaRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[100]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSchemaRequest.ProtoReflect.Descriptor instead.
func (*GetSchemaRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{100}
}

func (x *GetSchemaRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetSchemaRequest) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

type GetSchemaResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       string                 `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	Spec          []byte                 `protobuf:"bytes,2,opt,name=spec,proto3" json:"spec,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSchemaResponse) Reset() {
	*x = GetSchemaResponse{}
	mi := &file_repository_service_proto_msgTypes[101]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSchemaResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSchemaResponse) ProtoMessage() {}

func (x *GetSchemaResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[101]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSchemaResponse.ProtoReflect.Descriptor instead.
func (*GetSchemaResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{101}
}

func (x *GetSchemaResponse) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *GetSchemaResponse) GetSpec() []byte {
	if x != nil {
		return x.Spec
	}
	return nil
}

type GetAllActiveSchemasRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// WaitSeconds is the maximum number of seconds to wait for changes, defaults
	// to 10 seconds. Optional.
	WaitSeconds int64 `protobuf:"varint,1,opt,name=wait_seconds,json=waitSeconds,proto3" json:"wait_seconds,omitempty"`
	// Known schema versions that the client already has. Optional.
	Known map[string]string `protobuf:"bytes,2,rep,name=known,proto3" json:"known,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// OnlyChanged only returns the schemas that have been changed.
	OnlyChanged   bool `protobuf:"varint,3,opt,name=only_changed,json=onlyChanged,proto3" json:"only_changed,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAllActiveSchemasRequest) Reset() {
	*x = GetAllActiveSchemasRequest{}
	mi := &file_repository_service_proto_msgTypes[102]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAllActiveSchemasRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAllActiveSchemasRequest) ProtoMessage() {}

func (x *GetAllActiveSchemasRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[102]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAllActiveSchemasRequest.ProtoReflect.Descriptor instead.
func (*GetAllActiveSchemasRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{102}
}

func (x *GetAllActiveSchemasRequest) GetWaitSeconds() int64 {
	if x != nil {
		return x.WaitSeconds
	}
	return 0
}

func (x *GetAllActiveSchemasRequest) GetKnown() map[string]string {
	if x != nil {
		return x.Known
	}
	return nil
}

func (x *GetAllActiveSchemasRequest) GetOnlyChanged() bool {
	if x != nil {
		return x.OnlyChanged
	}
	return false
}

type GetAllActiveSchemasResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Schemas that currently are active.
	Schemas []*Schema `protobuf:"bytes,1,rep,name=schemas,proto3" json:"schemas,omitempty"`
	// Unchanged is set to true if no changes were done compared to known schema
	// versions.
	Unchanged bool `protobuf:"varint,2,opt,name=unchanged,proto3" json:"unchanged,omitempty"`
	// Removed schemas.
	Removed []string `protobuf:"bytes,3,rep,name=removed,proto3" json:"removed,omitempty"`
	// GenerationID is the ID of the currently active schema generation.
	GenerationId  int64 `protobuf:"varint,4,opt,name=generation_id,json=generationId,proto3" json:"generation_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAllActiveSchemasResponse) Reset() {
	*x = GetAllActiveSchemasResponse{}
	mi := &file_repository_service_proto_msgTypes[103]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAllActiveSchemasResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAllActiveSchemasResponse) ProtoMessage() {}

func (x *GetAllActiveSchemasResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[103]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAllActiveSchemasResponse.ProtoReflect.Descriptor instead.
func (*GetAllActiveSchemasResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{103}
}

func (x *GetAllActiveSchemasResponse) GetSchemas() []*Schema {
	if x != nil {
		return x.Schemas
	}
	return nil
}

func (x *GetAllActiveSchemasResponse) GetUnchanged() bool {
	if x != nil {
		return x.Unchanged
	}
	return false
}

func (x *GetAllActiveSchemasResponse) GetRemoved() []string {
	if x != nil {
		return x.Removed
	}
	return nil
}

func (x *GetAllActiveSchemasResponse) GetGenerationId() int64 {
	if x != nil {
		return x.GenerationId
	}
	return 0
}

type ListActiveSchemasRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListActiveSchemasRequest) Reset() {
	*x = ListActiveSchemasRequest{}
	mi := &file_repository_service_proto_msgTypes[104]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListActiveSchemasRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListActiveSchemasRequest) ProtoMessage() {}

func (x *ListActiveSchemasRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[104]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListActiveSchemasRequest.ProtoReflect.Descriptor instead.
func (*ListActiveSchemasRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{104}
}

type ListActiveSchemasResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Schemas without the spec populated
	Schemas []*Schema `protobuf:"bytes,1,rep,name=schemas,proto3" json:"schemas,omitempty"`
	// GenerationID is the ID of the currently active schema generation.
	GenerationId  int64 `protobuf:"varint,2,opt,name=generation_id,json=generationId,proto3" json:"generation_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListActiveSchemasResponse) Reset() {
	*x = ListActiveSchemasResponse{}
	mi := &file_repository_service_proto_msgTypes[105]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListActiveSchemasResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListActiveSchemasResponse) ProtoMessage() {}

func (x *ListActiveSchemasResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[105]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListActiveSchemasResponse.ProtoReflect.Descriptor instead.
func (*ListActiveSchemasResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{105}
}

func (x *ListActiveSchemasResponse) GetSchemas() []*Schema {
	if x != nil {
		return x.Schemas
	}
	return nil
}

func (x *ListActiveSchemasResponse) GetGenerationId() int64 {
	if x != nil {
		return x.GenerationId
	}
	return 0
}

type SchemaReference struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Version       string                 `protobuf:"bytes,2,opt,name=version,proto3" json:"version,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SchemaReference) Reset() {
	*x = SchemaReference{}
	mi := &file_repository_service_proto_msgTypes[106]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SchemaReference) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SchemaReference) ProtoMessage() {}

func (x *SchemaReference) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[106]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SchemaReference.ProtoReflect.Descriptor instead.
func (*SchemaReference) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{106}
}

func (x *SchemaReference) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *SchemaReference) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

type Schema struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Version       string                 `protobuf:"bytes,2,opt,name=version,proto3" json:"version,omitempty"`
	Spec          string                 `protobuf:"bytes,3,opt,name=spec,proto3" json:"spec,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Schema) Reset() {
	*x = Schema{}
	mi := &file_repository_service_proto_msgTypes[107]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Schema) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Schema) ProtoMessage() {}

func (x *Schema) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[107]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Schema.ProtoReflect.Descriptor instead.
func (*Schema) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{107}
}

func (x *Schema) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Schema) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *Schema) GetSpec() string {
	if x != nil {
		return x.Spec
	}
	return ""
}

type GetDeprecationsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeprecationsRequest) Reset() {
	*x = GetDeprecationsRequest{}
	mi := &file_repository_service_proto_msgTypes[108]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeprecationsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeprecationsRequest) ProtoMessage() {}

func (x *GetDeprecationsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[108]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeprecationsRequest.ProtoReflect.Descriptor instead.
func (*GetDeprecationsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{108}
}

type Deprecation struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Label         string                 `protobuf:"bytes,1,opt,name=label,proto3" json:"label,omitempty"`
	Enforced      bool                   `protobuf:"varint,2,opt,name=enforced,proto3" json:"enforced,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deprecation) Reset() {
	*x = Deprecation{}
	mi := &file_repository_service_proto_msgTypes[109]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deprecation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deprecation) ProtoMessage() {}

func (x *Deprecation) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[109]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deprecation.ProtoReflect.Descriptor instead.
func (*Deprecation) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{109}
}

func (x *Deprecation) GetLabel() string {
	if x != nil {
		return x.Label
	}
	return ""
}

func (x *Deprecation) GetEnforced() bool {
	if x != nil {
		return x.Enforced
	}
	return false
}

type GetDeprecationsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Deprecations  []*Deprecation         `protobuf:"bytes,1,rep,name=deprecations,proto3" json:"deprecations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeprecationsResponse) Reset() {
	*x = GetDeprecationsResponse{}
	mi := &file_repository_service_proto_msgTypes[110]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeprecationsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeprecationsResponse) ProtoMessage() {}

func (x *GetDeprecationsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[110]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeprecationsResponse.ProtoReflect.Descriptor instead.
func (*GetDeprecationsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{110}
}

func (x *GetDeprecationsResponse) GetDeprecations() []*Deprecation {
	if x != nil {
		return x.Deprecations
	}
	return nil
}

type UpdateDeprecationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Deprecation   *Deprecation           `protobuf:"bytes,1,opt,name=deprecation,proto3" json:"deprecation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateDeprecationRequest) Reset() {
	*x = UpdateDeprecationRequest{}
	mi := &file_repository_service_proto_msgTypes[111]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateDeprecationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDeprecationRequest) ProtoMessage() {}

func (x *UpdateDeprecationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[111]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDeprecationRequest.ProtoReflect.Descriptor instead.
func (*UpdateDeprecationRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{111}
}

func (x *UpdateDeprecationRequest) GetDeprecation() *Deprecation {
	if x != nil {
		return x.Deprecation
	}
	return nil
}

type UpdateDeprecationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateDeprecationResponse) Reset() {
	*x = UpdateDeprecationResponse{}
	mi := &file_repository_service_proto_msgTypes[112]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateDeprecationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDeprecationResponse) ProtoMessage() {}

func (x *UpdateDeprecationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[112]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDeprecationResponse.ProtoReflect.Descriptor instead.
func (*UpdateDeprecationResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{112}
}

type RegisterMetricKindRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Aggregation   MetricAggregation      `protobuf:"varint,2,opt,name=aggregation,proto3,enum=elephant.repository.MetricAggregation" json:"aggregation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetricKindRequest) Reset() {
	*x = RegisterMetricKindRequest{}
	mi := &file_repository_service_proto_msgTypes[113]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetricKindRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetricKindRequest) ProtoMessage() {}

func (x *RegisterMetricKindRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[113]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetricKindRequest.ProtoReflect.Descriptor instead.
func (*RegisterMetricKindRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{113}
}

func (x *RegisterMetricKindRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *RegisterMetricKindRequest) GetAggregation() MetricAggregation {
	if x != nil {
		return x.Aggregation
	}
	return MetricAggregation_NONE
}

type RegisterMetricKindResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetricKindResponse) Reset() {
	*x = RegisterMetricKindResponse{}
	mi := &file_repository_service_proto_msgTypes[114]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetricKindResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetricKindResponse) ProtoMessage() {}

func (x *RegisterMetricKindResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[114]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetricKindResponse.ProtoReflect.Descriptor instead.
func (*RegisterMetricKindResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{114}
}

type DeleteMetricKindRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteMetricKindRequest) Reset() {
	*x = DeleteMetricKindRequest{}
	mi := &file_repository_service_proto_msgTypes[115]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteMetricKindRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteMetricKindRequest) ProtoMessage() {}

func (x *DeleteMetricKindRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[115]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteMetricKindRequest.ProtoReflect.Descriptor instead.
func (*DeleteMetricKindRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{115}
}

func (x *DeleteMetricKindRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteMetricKindResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteMetricKindResponse) Reset() {
	*x = DeleteMetricKindResponse{}
	mi := &file_repository_service_proto_msgTypes[116]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteMetricKindResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteMetricKindResponse) ProtoMessage() {}

func (x *DeleteMetricKindResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[116]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteMetricKindResponse.ProtoReflect.Descriptor instead.
func (*DeleteMetricKindResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{116}
}

type GetMetricKindsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetricKindsRequest) Reset() {
	*x = GetMetricKindsRequest{}
	mi := &file_repository_service_proto_msgTypes[117]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetricKindsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetricKindsRequest) ProtoMessage() {}

func (x *GetMetricKindsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[117]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetricKindsRequest.ProtoReflect.Descriptor instead.
func (*GetMetricKindsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{117}
}

type GetMetricKindsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Kinds         []*MetricKind          `protobuf:"bytes,1,rep,name=kinds,proto3" json:"kinds,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetricKindsResponse) Reset() {
	*x = GetMetricKindsResponse{}
	mi := &file_repository_service_proto_msgTypes[118]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetricKindsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetricKindsResponse) ProtoMessage() {}

func (x *GetMetricKindsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[118]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetricKindsResponse.ProtoReflect.Descriptor instead.
func (*GetMetricKindsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{118}
}

func (x *GetMetricKindsResponse) GetKinds() []*MetricKind {
	if x != nil {
		return x.Kinds
	}
	return nil
}

type MetricKind struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Aggregation   MetricAggregation      `protobuf:"varint,2,opt,name=aggregation,proto3,enum=elephant.repository.MetricAggregation" json:"aggregation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MetricKind) Reset() {
	*x = MetricKind{}
	mi := &file_repository_service_proto_msgTypes[119]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MetricKind) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MetricKind) ProtoMessage() {}

func (x *MetricKind) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[119]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MetricKind.ProtoReflect.Descriptor instead.
func (*MetricKind) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{119}
}

func (x *MetricKind) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *MetricKind) GetAggregation() MetricAggregation {
	if x != nil {
		return x.Aggregation
	}
	return MetricAggregation_NONE
}

type RegisterMetricRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Uuid          string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	Kind          string                 `protobuf:"bytes,2,opt,name=kind,proto3" json:"kind,omitempty"`
	Label         string                 `protobuf:"bytes,3,opt,name=label,proto3" json:"label,omitempty"`
	Value         int64                  `protobuf:"varint,5,opt,name=value,proto3" json:"value,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetricRequest) Reset() {
	*x = RegisterMetricRequest{}
	mi := &file_repository_service_proto_msgTypes[120]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetricRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetricRequest) ProtoMessage() {}

func (x *RegisterMetricRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[120]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetricRequest.ProtoReflect.Descriptor instead.
func (*RegisterMetricRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{120}
}

func (x *RegisterMetricRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *RegisterMetricRequest) GetKind() string {
	if x != nil {
		return x.Kind
	}
	return ""
}

func (x *RegisterMetricRequest) GetLabel() string {
	if x != nil {
		return x.Label
	}
	return ""
}

func (x *RegisterMetricRequest) GetValue() int64 {
	if x != nil {
		return x.Value
	}
	return 0
}

type RegisterMetricResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterMetricResponse) Reset() {
	*x = RegisterMetricResponse{}
	mi := &file_repository_service_proto_msgTypes[121]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterMetricResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterMetricResponse) ProtoMessage() {}

func (x *RegisterMetricResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[121]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterMetricResponse.ProtoReflect.Descriptor instead.
func (*RegisterMetricResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{121}
}

type GetMetricsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The documents to get metrics for.
	Uuids []string `protobuf:"bytes,1,rep,name=uuids,proto3" json:"uuids,omitempty"`
	// The metric kinds to get. Optional, defaults to all.
	Kinds         []string `protobuf:"bytes,2,rep,name=kinds,proto3" json:"kinds,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetricsRequest) Reset() {
	*x = GetMetricsRequest{}
	mi := &file_repository_service_proto_msgTypes[122]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetricsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetricsRequest) ProtoMessage() {}

func (x *GetMetricsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[122]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetricsRequest.ProtoReflect.Descriptor instead.
func (*GetMetricsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{122}
}

func (x *GetMetricsRequest) GetUuids() []string {
	if x != nil {
		return x.Uuids
	}
	return nil
}

func (x *GetMetricsRequest) GetKinds() []string {
	if x != nil {
		return x.Kinds
	}
	return nil
}

type GetMetricsResponse struct {
	state         protoimpl.MessageState      `protogen:"open.v1"`
	Documents     map[string]*DocumentMetrics `protobuf:"bytes,1,rep,name=documents,proto3" json:"documents,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMetricsResponse) Reset() {
	*x = GetMetricsResponse{}
	mi := &file_repository_service_proto_msgTypes[123]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMetricsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMetricsResponse) ProtoMessage() {}

func (x *GetMetricsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[123]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMetricsResponse.ProtoReflect.Descriptor instead.
func (*GetMetricsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{123}
}

func (x *GetMetricsResponse) GetDocuments() map[string]*DocumentMetrics {
	if x != nil {
		return x.Documents
	}
	return nil
}

type DocumentMetrics struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Uuid          string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	Metrics       []*Metric              `protobuf:"bytes,2,rep,name=metrics,proto3" json:"metrics,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentMetrics) Reset() {
	*x = DocumentMetrics{}
	mi := &file_repository_service_proto_msgTypes[124]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentMetrics) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentMetrics) ProtoMessage() {}

func (x *DocumentMetrics) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[124]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentMetrics.ProtoReflect.Descriptor instead.
func (*DocumentMetrics) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{124}
}

func (x *DocumentMetrics) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *DocumentMetrics) GetMetrics() []*Metric {
	if x != nil {
		return x.Metrics
	}
	return nil
}

type Metric struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Kind          string                 `protobuf:"bytes,1,opt,name=kind,proto3" json:"kind,omitempty"`
	Label         string                 `protobuf:"bytes,2,opt,name=label,proto3" json:"label,omitempty"`
	Value         int64                  `protobuf:"varint,3,opt,name=value,proto3" json:"value,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Metric) Reset() {
	*x = Metric{}
	mi := &file_repository_service_proto_msgTypes[125]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Metric) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Metric) ProtoMessage() {}

func (x *Metric) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[125]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Metric.ProtoReflect.Descriptor instead.
func (*Metric) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{125}
}

func (x *Metric) GetKind() string {
	if x != nil {
		return x.Kind
	}
	return ""
}

func (x *Metric) GetLabel() string {
	if x != nil {
		return x.Label
	}
	return ""
}

func (x *Metric) GetValue() int64 {
	if x != nil {
		return x.Value
	}
	return 0
}

type LockRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to lock
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Lock TTL in milliseconds
	Ttl int32 `protobuf:"varint,2,opt,name=ttl,proto3" json:"ttl,omitempty"`
	// Name of the locking application. Optional.
	App string `protobuf:"bytes,3,opt,name=app,proto3" json:"app,omitempty"`
	// Free-form comment string. Optional.
	Comment string `protobuf:"bytes,4,opt,name=comment,proto3" json:"comment,omitempty"`
	// Exclusivity controls which operations the lock blocks. Defaults to
	// LOCK_DOCUMENT, which only blocks document updates.
	Exclusivity   LockExclusivity `protobuf:"varint,5,opt,name=exclusivity,proto3,enum=elephant.repository.LockExclusivity" json:"exclusivity,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LockRequest) Reset() {
	*x = LockRequest{}
	mi := &file_repository_service_proto_msgTypes[126]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LockRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LockRequest) ProtoMessage() {}

func (x *LockRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[126]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LockRequest.ProtoReflect.Descriptor instead.
func (*LockRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{126}
}

func (x *LockRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *LockRequest) GetTtl() int32 {
	if x != nil {
		return x.Ttl
	}
	return 0
}

func (x *LockRequest) GetApp() string {
	if x != nil {
		return x.App
	}
	return ""
}

func (x *LockRequest) GetComment() string {
	if x != nil {
		return x.Comment
	}
	return ""
}

func (x *LockRequest) GetExclusivity() LockExclusivity {
	if x != nil {
		return x.Exclusivity
	}
	return LockExclusivity_LOCK_DOCUMENT
}

type LockResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Generated lock token in UUID format.
	Token string `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	// Expires is the RFC3339 timestamp when the lock will expire.
	Expires       string `protobuf:"bytes,2,opt,name=expires,proto3" json:"expires,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LockResponse) Reset() {
	*x = LockResponse{}
	mi := &file_repository_service_proto_msgTypes[127]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LockResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LockResponse) ProtoMessage() {}

func (x *LockResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[127]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LockResponse.ProtoReflect.Descriptor instead.
func (*LockResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{127}
}

func (x *LockResponse) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *LockResponse) GetExpires() string {
	if x != nil {
		return x.Expires
	}
	return ""
}

// AcquireLock describes a lock to acquire as part of a GetDocument request.
// It mirrors LockRequest, but omits the redundant document UUID.
type AcquireLock struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Lock TTL in milliseconds.
	Ttl int32 `protobuf:"varint,1,opt,name=ttl,proto3" json:"ttl,omitempty"`
	// Name of the locking application. Optional.
	App string `protobuf:"bytes,2,opt,name=app,proto3" json:"app,omitempty"`
	// Free-form comment string. Optional.
	Comment string `protobuf:"bytes,3,opt,name=comment,proto3" json:"comment,omitempty"`
	// Exclusivity controls which operations the lock blocks. Defaults to
	// LOCK_DOCUMENT, which only blocks document updates.
	Exclusivity   LockExclusivity `protobuf:"varint,4,opt,name=exclusivity,proto3,enum=elephant.repository.LockExclusivity" json:"exclusivity,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AcquireLock) Reset() {
	*x = AcquireLock{}
	mi := &file_repository_service_proto_msgTypes[128]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AcquireLock) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AcquireLock) ProtoMessage() {}

func (x *AcquireLock) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[128]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AcquireLock.ProtoReflect.Descriptor instead.
func (*AcquireLock) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{128}
}

func (x *AcquireLock) GetTtl() int32 {
	if x != nil {
		return x.Ttl
	}
	return 0
}

func (x *AcquireLock) GetApp() string {
	if x != nil {
		return x.App
	}
	return ""
}

func (x *AcquireLock) GetComment() string {
	if x != nil {
		return x.Comment
	}
	return ""
}

func (x *AcquireLock) GetExclusivity() LockExclusivity {
	if x != nil {
		return x.Exclusivity
	}
	return LockExclusivity_LOCK_DOCUMENT
}

// LockGrant is the lock returned when a GetDocument request successfully
// acquired a lock via its lock field. It mirrors LockResponse.
type LockGrant struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Generated lock token in UUID format.
	Token string `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	// Expires is the RFC3339 timestamp when the lock will expire.
	Expires       string `protobuf:"bytes,2,opt,name=expires,proto3" json:"expires,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LockGrant) Reset() {
	*x = LockGrant{}
	mi := &file_repository_service_proto_msgTypes[129]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LockGrant) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LockGrant) ProtoMessage() {}

func (x *LockGrant) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[129]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LockGrant.ProtoReflect.Descriptor instead.
func (*LockGrant) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{129}
}

func (x *LockGrant) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *LockGrant) GetExpires() string {
	if x != nil {
		return x.Expires
	}
	return ""
}

type ExtendLockRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to extend the lock for.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Lock TTL in milliseconds.
	Ttl int32 `protobuf:"varint,2,opt,name=ttl,proto3" json:"ttl,omitempty"`
	// Lock token in UUID format.
	Token         string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExtendLockRequest) Reset() {
	*x = ExtendLockRequest{}
	mi := &file_repository_service_proto_msgTypes[130]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExtendLockRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExtendLockRequest) ProtoMessage() {}

func (x *ExtendLockRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[130]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExtendLockRequest.ProtoReflect.Descriptor instead.
func (*ExtendLockRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{130}
}

func (x *ExtendLockRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *ExtendLockRequest) GetTtl() int32 {
	if x != nil {
		return x.Ttl
	}
	return 0
}

func (x *ExtendLockRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

type UnlockRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document to unlock
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Lock token in UUID format.
	Token         string `protobuf:"bytes,2,opt,name=token,proto3" json:"token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnlockRequest) Reset() {
	*x = UnlockRequest{}
	mi := &file_repository_service_proto_msgTypes[131]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnlockRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnlockRequest) ProtoMessage() {}

func (x *UnlockRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[131]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnlockRequest.ProtoReflect.Descriptor instead.
func (*UnlockRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{131}
}

func (x *UnlockRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *UnlockRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

type UnlockResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnlockResponse) Reset() {
	*x = UnlockResponse{}
	mi := &file_repository_service_proto_msgTypes[132]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnlockResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnlockResponse) ProtoMessage() {}

func (x *UnlockResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[132]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnlockResponse.ProtoReflect.Descriptor instead.
func (*UnlockResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{132}
}

type GetWithheldRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetWithheldRequest) Reset() {
	*x = GetWithheldRequest{}
	mi := &file_repository_service_proto_msgTypes[133]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetWithheldRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetWithheldRequest) ProtoMessage() {}

func (x *GetWithheldRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[133]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetWithheldRequest.ProtoReflect.Descriptor instead.
func (*GetWithheldRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{133}
}

type GetWithheldResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Items         []*ScheduledDocument   `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetWithheldResponse) Reset() {
	*x = GetWithheldResponse{}
	mi := &file_repository_service_proto_msgTypes[134]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetWithheldResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetWithheldResponse) ProtoMessage() {}

func (x *GetWithheldResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[134]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetWithheldResponse.ProtoReflect.Descriptor instead.
func (*GetWithheldResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{134}
}

func (x *GetWithheldResponse) GetItems() []*ScheduledDocument {
	if x != nil {
		return x.Items
	}
	return nil
}

type ScheduledDocument struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID is the ID of the scheduled document.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Type of the scheduled document.
	Type string `protobuf:"bytes,2,opt,name=type,proto3" json:"type,omitempty"`
	// StatusID is the last withheld status ID.
	StatusId int64 `protobuf:"varint,3,opt,name=status_id,json=statusId,proto3" json:"status_id,omitempty"`
	// DocumentVersion is the last version that was set as withheld.
	DocumentVersion int64 `protobuf:"varint,4,opt,name=document_version,json=documentVersion,proto3" json:"document_version,omitempty"`
	// PlanningItem UUID.
	PlanningItem string `protobuf:"bytes,5,opt,name=planning_item,json=planningItem,proto3" json:"planning_item,omitempty"`
	// Assignment ID.
	Assignment string `protobuf:"bytes,6,opt,name=assignment,proto3" json:"assignment,omitempty"`
	// Publish timestamp as RFC3339 set in the assignment.
	Publish string `protobuf:"bytes,7,opt,name=publish,proto3" json:"publish,omitempty"`
	// ScheduledBy is the sub of the user that set the withheld status.
	ScheduledBy   string `protobuf:"bytes,8,opt,name=scheduled_by,json=scheduledBy,proto3" json:"scheduled_by,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ScheduledDocument) Reset() {
	*x = ScheduledDocument{}
	mi := &file_repository_service_proto_msgTypes[135]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ScheduledDocument) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ScheduledDocument) ProtoMessage() {}

func (x *ScheduledDocument) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[135]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ScheduledDocument.ProtoReflect.Descriptor instead.
func (*ScheduledDocument) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{135}
}

func (x *ScheduledDocument) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *ScheduledDocument) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *ScheduledDocument) GetStatusId() int64 {
	if x != nil {
		return x.StatusId
	}
	return 0
}

func (x *ScheduledDocument) GetDocumentVersion() int64 {
	if x != nil {
		return x.DocumentVersion
	}
	return 0
}

func (x *ScheduledDocument) GetPlanningItem() string {
	if x != nil {
		return x.PlanningItem
	}
	return ""
}

func (x *ScheduledDocument) GetAssignment() string {
	if x != nil {
		return x.Assignment
	}
	return ""
}

func (x *ScheduledDocument) GetPublish() string {
	if x != nil {
		return x.Publish
	}
	return ""
}

func (x *ScheduledDocument) GetScheduledBy() string {
	if x != nil {
		return x.ScheduledBy
	}
	return ""
}

type GetDeliverableInfoRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the deliverable.
	Uuid          string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeliverableInfoRequest) Reset() {
	*x = GetDeliverableInfoRequest{}
	mi := &file_repository_service_proto_msgTypes[136]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeliverableInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeliverableInfoRequest) ProtoMessage() {}

func (x *GetDeliverableInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[136]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeliverableInfoRequest.ProtoReflect.Descriptor instead.
func (*GetDeliverableInfoRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{136}
}

func (x *GetDeliverableInfoRequest) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

type GetDeliverableInfoResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Indicates whether the deliverable has planning information.
	HasPlanningInfo bool `protobuf:"varint,1,opt,name=has_planning_info,json=hasPlanningInfo,proto3" json:"has_planning_info,omitempty"`
	// UUID of the related planning item.
	PlanningUuid string `protobuf:"bytes,2,opt,name=planning_uuid,json=planningUuid,proto3" json:"planning_uuid,omitempty"`
	// UUID of the related assignment.
	AssignmentUuid string `protobuf:"bytes,3,opt,name=assignment_uuid,json=assignmentUuid,proto3" json:"assignment_uuid,omitempty"`
	// UUID of the related event.
	EventUuid     string `protobuf:"bytes,4,opt,name=event_uuid,json=eventUuid,proto3" json:"event_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeliverableInfoResponse) Reset() {
	*x = GetDeliverableInfoResponse{}
	mi := &file_repository_service_proto_msgTypes[137]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeliverableInfoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeliverableInfoResponse) ProtoMessage() {}

func (x *GetDeliverableInfoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[137]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeliverableInfoResponse.ProtoReflect.Descriptor instead.
func (*GetDeliverableInfoResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{137}
}

func (x *GetDeliverableInfoResponse) GetHasPlanningInfo() bool {
	if x != nil {
		return x.HasPlanningInfo
	}
	return false
}

func (x *GetDeliverableInfoResponse) GetPlanningUuid() string {
	if x != nil {
		return x.PlanningUuid
	}
	return ""
}

func (x *GetDeliverableInfoResponse) GetAssignmentUuid() string {
	if x != nil {
		return x.AssignmentUuid
	}
	return ""
}

func (x *GetDeliverableInfoResponse) GetEventUuid() string {
	if x != nil {
		return x.EventUuid
	}
	return ""
}

type BulkGetDeliverableInfoRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUIDs of the deliverables to look up.
	Uuids         []string `protobuf:"bytes,1,rep,name=uuids,proto3" json:"uuids,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetDeliverableInfoRequest) Reset() {
	*x = BulkGetDeliverableInfoRequest{}
	mi := &file_repository_service_proto_msgTypes[138]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetDeliverableInfoRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetDeliverableInfoRequest) ProtoMessage() {}

func (x *BulkGetDeliverableInfoRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[138]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetDeliverableInfoRequest.ProtoReflect.Descriptor instead.
func (*BulkGetDeliverableInfoRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{138}
}

func (x *BulkGetDeliverableInfoRequest) GetUuids() []string {
	if x != nil {
		return x.Uuids
	}
	return nil
}

type BulkGetDeliverableInfoResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Items contains one entry per deliverable that has planning information.
	// Deliverables without planning information are omitted.
	Items         []*DeliverableInfo `protobuf:"bytes,1,rep,name=items,proto3" json:"items,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BulkGetDeliverableInfoResponse) Reset() {
	*x = BulkGetDeliverableInfoResponse{}
	mi := &file_repository_service_proto_msgTypes[139]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BulkGetDeliverableInfoResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BulkGetDeliverableInfoResponse) ProtoMessage() {}

func (x *BulkGetDeliverableInfoResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[139]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BulkGetDeliverableInfoResponse.ProtoReflect.Descriptor instead.
func (*BulkGetDeliverableInfoResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{139}
}

func (x *BulkGetDeliverableInfoResponse) GetItems() []*DeliverableInfo {
	if x != nil {
		return x.Items
	}
	return nil
}

type DeliverableInfo struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the deliverable document.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// UUID of the related planning item.
	PlanningUuid string `protobuf:"bytes,2,opt,name=planning_uuid,json=planningUuid,proto3" json:"planning_uuid,omitempty"`
	// UUID of the related assignment.
	AssignmentUuid string `protobuf:"bytes,3,opt,name=assignment_uuid,json=assignmentUuid,proto3" json:"assignment_uuid,omitempty"`
	// UUID of the related event.
	EventUuid     string `protobuf:"bytes,4,opt,name=event_uuid,json=eventUuid,proto3" json:"event_uuid,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeliverableInfo) Reset() {
	*x = DeliverableInfo{}
	mi := &file_repository_service_proto_msgTypes[140]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeliverableInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeliverableInfo) ProtoMessage() {}

func (x *DeliverableInfo) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[140]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeliverableInfo.ProtoReflect.Descriptor instead.
func (*DeliverableInfo) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{140}
}

func (x *DeliverableInfo) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *DeliverableInfo) GetPlanningUuid() string {
	if x != nil {
		return x.PlanningUuid
	}
	return ""
}

func (x *DeliverableInfo) GetAssignmentUuid() string {
	if x != nil {
		return x.AssignmentUuid
	}
	return ""
}

func (x *DeliverableInfo) GetEventUuid() string {
	if x != nil {
		return x.EventUuid
	}
	return ""
}

type CreateUploadRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of the uploaded file, this should be the basename of the file.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// ContentType is the type of the file as a mimetype.
	ContentType string `protobuf:"bytes,2,opt,name=content_type,json=contentType,proto3" json:"content_type,omitempty"`
	// Meta can be used to set arbitrary key-value data on an upload.
	Meta          map[string]string `protobuf:"bytes,3,rep,name=meta,proto3" json:"meta,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateUploadRequest) Reset() {
	*x = CreateUploadRequest{}
	mi := &file_repository_service_proto_msgTypes[141]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateUploadRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateUploadRequest) ProtoMessage() {}

func (x *CreateUploadRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[141]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateUploadRequest.ProtoReflect.Descriptor instead.
func (*CreateUploadRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{141}
}

func (x *CreateUploadRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateUploadRequest) GetContentType() string {
	if x != nil {
		return x.ContentType
	}
	return ""
}

func (x *CreateUploadRequest) GetMeta() map[string]string {
	if x != nil {
		return x.Meta
	}
	return nil
}

type CreateUploadResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// ID of the created upload.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// URL to PUT the object content to.
	Url           string `protobuf:"bytes,2,opt,name=url,proto3" json:"url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateUploadResponse) Reset() {
	*x = CreateUploadResponse{}
	mi := &file_repository_service_proto_msgTypes[142]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateUploadResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateUploadResponse) ProtoMessage() {}

func (x *CreateUploadResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[142]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateUploadResponse.ProtoReflect.Descriptor instead.
func (*CreateUploadResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{142}
}

func (x *CreateUploadResponse) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *CreateUploadResponse) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

type GetAttachmentsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Documents to get attachments for.
	Documents []string `protobuf:"bytes,1,rep,name=documents,proto3" json:"documents,omitempty"`
	// Attachment name to get information for.
	AttachmentName string `protobuf:"bytes,2,opt,name=attachment_name,json=attachmentName,proto3" json:"attachment_name,omitempty"`
	// DownloadLink should be set to true to generate a download link.
	DownloadLink  bool `protobuf:"varint,3,opt,name=download_link,json=downloadLink,proto3" json:"download_link,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAttachmentsRequest) Reset() {
	*x = GetAttachmentsRequest{}
	mi := &file_repository_service_proto_msgTypes[143]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAttachmentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAttachmentsRequest) ProtoMessage() {}

func (x *GetAttachmentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[143]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAttachmentsRequest.ProtoReflect.Descriptor instead.
func (*GetAttachmentsRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{143}
}

func (x *GetAttachmentsRequest) GetDocuments() []string {
	if x != nil {
		return x.Documents
	}
	return nil
}

func (x *GetAttachmentsRequest) GetAttachmentName() string {
	if x != nil {
		return x.AttachmentName
	}
	return ""
}

func (x *GetAttachmentsRequest) GetDownloadLink() bool {
	if x != nil {
		return x.DownloadLink
	}
	return false
}

type GetAttachmentsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Attachments   []*AttachmentDetails   `protobuf:"bytes,1,rep,name=attachments,proto3" json:"attachments,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAttachmentsResponse) Reset() {
	*x = GetAttachmentsResponse{}
	mi := &file_repository_service_proto_msgTypes[144]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAttachmentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAttachmentsResponse) ProtoMessage() {}

func (x *GetAttachmentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[144]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAttachmentsResponse.ProtoReflect.Descriptor instead.
func (*GetAttachmentsResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{144}
}

func (x *GetAttachmentsResponse) GetAttachments() []*AttachmentDetails {
	if x != nil {
		return x.Attachments
	}
	return nil
}

type AttachmentDetails struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Document is the UUID of the document the object is attached to.
	Document string `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
	// Name of the attachment.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Version of the attachment.
	Version int64 `protobuf:"varint,3,opt,name=version,proto3" json:"version,omitempty"`
	// DownloadLink that can be used to fetch the object contents, if requested.
	DownloadLink string `protobuf:"bytes,4,opt,name=download_link,json=downloadLink,proto3" json:"download_link,omitempty"`
	// Filename is the name of the file the object represents.
	Filename string `protobuf:"bytes,5,opt,name=filename,proto3" json:"filename,omitempty"`
	// ContentType is the MIME type of the object.
	ContentType   string `protobuf:"bytes,6,opt,name=content_type,json=contentType,proto3" json:"content_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AttachmentDetails) Reset() {
	*x = AttachmentDetails{}
	mi := &file_repository_service_proto_msgTypes[145]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AttachmentDetails) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AttachmentDetails) ProtoMessage() {}

func (x *AttachmentDetails) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[145]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AttachmentDetails.ProtoReflect.Descriptor instead.
func (*AttachmentDetails) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{145}
}

func (x *AttachmentDetails) GetDocument() string {
	if x != nil {
		return x.Document
	}
	return ""
}

func (x *AttachmentDetails) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *AttachmentDetails) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *AttachmentDetails) GetDownloadLink() string {
	if x != nil {
		return x.DownloadLink
	}
	return ""
}

func (x *AttachmentDetails) GetFilename() string {
	if x != nil {
		return x.Filename
	}
	return ""
}

func (x *AttachmentDetails) GetContentType() string {
	if x != nil {
		return x.ContentType
	}
	return ""
}

type ConfigureTypeRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type to configure.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Configuration for the type.
	Configuration *TypeConfiguration `protobuf:"bytes,2,opt,name=configuration,proto3" json:"configuration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigureTypeRequest) Reset() {
	*x = ConfigureTypeRequest{}
	mi := &file_repository_service_proto_msgTypes[146]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigureTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigureTypeRequest) ProtoMessage() {}

func (x *ConfigureTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[146]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigureTypeRequest.ProtoReflect.Descriptor instead.
func (*ConfigureTypeRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{146}
}

func (x *ConfigureTypeRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *ConfigureTypeRequest) GetConfiguration() *TypeConfiguration {
	if x != nil {
		return x.Configuration
	}
	return nil
}

type TypeConfiguration struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Set to true if this is a collection of documents that are bounded (limited
	// in size). Bounded collections can be fetched by type and language in one
	// go.
	BoundedCollection bool `protobuf:"varint,1,opt,name=bounded_collection,json=boundedCollection,proto3" json:"bounded_collection,omitempty"`
	// TimeExpressions are used for extracting time ranges from documents.
	TimeExpressions []*TypeTimeExpression `protobuf:"bytes,2,rep,name=time_expressions,json=timeExpressions,proto3" json:"time_expressions,omitempty"`
	// LabelExpressions are used to extract labels from documents.
	LabelExpressions []*LabelExpression `protobuf:"bytes,3,rep,name=label_expressions,json=labelExpressions,proto3" json:"label_expressions,omitempty"`
	// Variants are the allowed variants for the document type.
	Variants []string `protobuf:"bytes,4,rep,name=variants,proto3" json:"variants,omitempty"`
	// EvictNonCurrentAfter can be set to evict non-current versions of a
	// document from the database after they're older than the given number of
	// days. Optional.
	EvictNoncurrentAfter int64 `protobuf:"varint,5,opt,name=evict_noncurrent_after,json=evictNoncurrentAfter,proto3" json:"evict_noncurrent_after,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *TypeConfiguration) Reset() {
	*x = TypeConfiguration{}
	mi := &file_repository_service_proto_msgTypes[147]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TypeConfiguration) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TypeConfiguration) ProtoMessage() {}

func (x *TypeConfiguration) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[147]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TypeConfiguration.ProtoReflect.Descriptor instead.
func (*TypeConfiguration) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{147}
}

func (x *TypeConfiguration) GetBoundedCollection() bool {
	if x != nil {
		return x.BoundedCollection
	}
	return false
}

func (x *TypeConfiguration) GetTimeExpressions() []*TypeTimeExpression {
	if x != nil {
		return x.TimeExpressions
	}
	return nil
}

func (x *TypeConfiguration) GetLabelExpressions() []*LabelExpression {
	if x != nil {
		return x.LabelExpressions
	}
	return nil
}

func (x *TypeConfiguration) GetVariants() []string {
	if x != nil {
		return x.Variants
	}
	return nil
}

func (x *TypeConfiguration) GetEvictNoncurrentAfter() int64 {
	if x != nil {
		return x.EvictNoncurrentAfter
	}
	return 0
}

type TypeTimeExpression struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Expression is a newsdoc value extraction expression.
	Expression string `protobuf:"bytes,1,opt,name=expression,proto3" json:"expression,omitempty"`
	// Layout is the time/date format to use when parsing. Optional, defaults to
	// RFC3339 or ISO 8601 for values annotated as dates.
	Layout string `protobuf:"bytes,2,opt,name=layout,proto3" json:"layout,omitempty"`
	// Timezone is the timezone the time should be parsed in. Optional, most
	// timestamps should include timezone information, if they don't, parsing will
	// fall back to the default timezone that the repository has been configured
	// with.
	Timezone      string `protobuf:"bytes,3,opt,name=timezone,proto3" json:"timezone,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TypeTimeExpression) Reset() {
	*x = TypeTimeExpression{}
	mi := &file_repository_service_proto_msgTypes[148]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TypeTimeExpression) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TypeTimeExpression) ProtoMessage() {}

func (x *TypeTimeExpression) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[148]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TypeTimeExpression.ProtoReflect.Descriptor instead.
func (*TypeTimeExpression) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{148}
}

func (x *TypeTimeExpression) GetExpression() string {
	if x != nil {
		return x.Expression
	}
	return ""
}

func (x *TypeTimeExpression) GetLayout() string {
	if x != nil {
		return x.Layout
	}
	return ""
}

func (x *TypeTimeExpression) GetTimezone() string {
	if x != nil {
		return x.Timezone
	}
	return ""
}

type LabelExpression struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Expression is a newsdoc value extraction expression.
	Expression string `protobuf:"bytes,1,opt,name=expression,proto3" json:"expression,omitempty"`
	// Template is the template that turns the extracted values into a label.
	Template      string `protobuf:"bytes,2,opt,name=template,proto3" json:"template,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LabelExpression) Reset() {
	*x = LabelExpression{}
	mi := &file_repository_service_proto_msgTypes[149]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LabelExpression) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LabelExpression) ProtoMessage() {}

func (x *LabelExpression) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[149]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LabelExpression.ProtoReflect.Descriptor instead.
func (*LabelExpression) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{149}
}

func (x *LabelExpression) GetExpression() string {
	if x != nil {
		return x.Expression
	}
	return ""
}

func (x *LabelExpression) GetTemplate() string {
	if x != nil {
		return x.Template
	}
	return ""
}

type ConfigureTypeResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfigureTypeResponse) Reset() {
	*x = ConfigureTypeResponse{}
	mi := &file_repository_service_proto_msgTypes[150]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfigureTypeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfigureTypeResponse) ProtoMessage() {}

func (x *ConfigureTypeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[150]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfigureTypeResponse.ProtoReflect.Descriptor instead.
func (*ConfigureTypeResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{150}
}

type GetTypeConfigurationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type to get configuration for.
	Type          string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTypeConfigurationRequest) Reset() {
	*x = GetTypeConfigurationRequest{}
	mi := &file_repository_service_proto_msgTypes[151]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTypeConfigurationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTypeConfigurationRequest) ProtoMessage() {}

func (x *GetTypeConfigurationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[151]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTypeConfigurationRequest.ProtoReflect.Descriptor instead.
func (*GetTypeConfigurationRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{151}
}

func (x *GetTypeConfigurationRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

type GetTypeConfigurationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Configuration of the type.
	Configuration *TypeConfiguration `protobuf:"bytes,1,opt,name=configuration,proto3" json:"configuration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTypeConfigurationResponse) Reset() {
	*x = GetTypeConfigurationResponse{}
	mi := &file_repository_service_proto_msgTypes[152]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTypeConfigurationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTypeConfigurationResponse) ProtoMessage() {}

func (x *GetTypeConfigurationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[152]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTypeConfigurationResponse.ProtoReflect.Descriptor instead.
func (*GetTypeConfigurationResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{152}
}

func (x *GetTypeConfigurationResponse) GetConfiguration() *TypeConfiguration {
	if x != nil {
		return x.Configuration
	}
	return nil
}

type GetMatchingRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type of the documents to get.
	Type string `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	// Timespan is the inclusive time range to get documents for. Optional, this
	// is only available for document types with time expressions and/or
	// deliverables.
	Timespan *Timespan `protobuf:"bytes,2,opt,name=timespan,proto3" json:"timespan,omitempty"`
	// Labels is a list of labels that the document must have to be included.
	Labels []string `protobuf:"bytes,3,rep,name=labels,proto3" json:"labels,omitempty"`
	// Filter to apply to the documents.
	Filter *DocumentFilter `protobuf:"bytes,4,opt,name=filter,proto3" json:"filter,omitempty"`
	// IncludeDocuments should be set to true to get the document for each match.
	IncludeDocuments bool `protobuf:"varint,5,opt,name=include_documents,json=includeDocuments,proto3" json:"include_documents,omitempty"`
	// IncludeMeta should be set to true to include document meta.
	IncludeMeta bool `protobuf:"varint,6,opt,name=include_meta,json=includeMeta,proto3" json:"include_meta,omitempty"`
	// Include will include documents with an UUID that matches the uuid of the
	// newdoc extraction expression. Timespans, labels and filters will not be
	// applied to included documents.
	Include       []string `protobuf:"bytes,7,rep,name=include,proto3" json:"include,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMatchingRequest) Reset() {
	*x = GetMatchingRequest{}
	mi := &file_repository_service_proto_msgTypes[153]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMatchingRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMatchingRequest) ProtoMessage() {}

func (x *GetMatchingRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[153]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMatchingRequest.ProtoReflect.Descriptor instead.
func (*GetMatchingRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{153}
}

func (x *GetMatchingRequest) GetType() string {
	if x != nil {
		return x.Type
	}
	return ""
}

func (x *GetMatchingRequest) GetTimespan() *Timespan {
	if x != nil {
		return x.Timespan
	}
	return nil
}

func (x *GetMatchingRequest) GetLabels() []string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *GetMatchingRequest) GetFilter() *DocumentFilter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *GetMatchingRequest) GetIncludeDocuments() bool {
	if x != nil {
		return x.IncludeDocuments
	}
	return false
}

func (x *GetMatchingRequest) GetIncludeMeta() bool {
	if x != nil {
		return x.IncludeMeta
	}
	return false
}

func (x *GetMatchingRequest) GetInclude() []string {
	if x != nil {
		return x.Include
	}
	return nil
}

type GetMatchingResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Matches       []*DocumentMatch       `protobuf:"bytes,1,rep,name=matches,proto3" json:"matches,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMatchingResponse) Reset() {
	*x = GetMatchingResponse{}
	mi := &file_repository_service_proto_msgTypes[154]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMatchingResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMatchingResponse) ProtoMessage() {}

func (x *GetMatchingResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[154]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMatchingResponse.ProtoReflect.Descriptor instead.
func (*GetMatchingResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{154}
}

func (x *GetMatchingResponse) GetMatches() []*DocumentMatch {
	if x != nil {
		return x.Matches
	}
	return nil
}

type DocumentMatch struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// UUID of the document.
	Uuid string `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	// Version of the document.
	Version int64 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Document for the match.
	Document *newsdoc.Document `protobuf:"bytes,3,opt,name=document,proto3" json:"document,omitempty"`
	// Meta information for the match.
	Meta          *DocumentMeta `protobuf:"bytes,4,opt,name=meta,proto3" json:"meta,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentMatch) Reset() {
	*x = DocumentMatch{}
	mi := &file_repository_service_proto_msgTypes[155]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentMatch) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentMatch) ProtoMessage() {}

func (x *DocumentMatch) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[155]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentMatch.ProtoReflect.Descriptor instead.
func (*DocumentMatch) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{155}
}

func (x *DocumentMatch) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *DocumentMatch) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *DocumentMatch) GetDocument() *newsdoc.Document {
	if x != nil {
		return x.Document
	}
	return nil
}

func (x *DocumentMatch) GetMeta() *DocumentMeta {
	if x != nil {
		return x.Meta
	}
	return nil
}

type DocumentFilter struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Expression used to extract values from the document.
	Expression string `protobuf:"bytes,1,opt,name=expression,proto3" json:"expression,omitempty"`
	// Operator to use when comparing the provided values to the extracted values.
	Operator FilterOperator `protobuf:"varint,2,opt,name=operator,proto3,enum=elephant.repository.FilterOperator" json:"operator,omitempty"`
	// Values to compare to the extracted values.
	Values []*FilterValues `protobuf:"bytes,3,rep,name=values,proto3" json:"values,omitempty"`
	// And filters that all must be true.
	And []*DocumentFilter `protobuf:"bytes,4,rep,name=and,proto3" json:"and,omitempty"`
	// Or filters of which at least one must be true.
	Or            []*DocumentFilter `protobuf:"bytes,5,rep,name=or,proto3" json:"or,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DocumentFilter) Reset() {
	*x = DocumentFilter{}
	mi := &file_repository_service_proto_msgTypes[156]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DocumentFilter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DocumentFilter) ProtoMessage() {}

func (x *DocumentFilter) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[156]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DocumentFilter.ProtoReflect.Descriptor instead.
func (*DocumentFilter) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{156}
}

func (x *DocumentFilter) GetExpression() string {
	if x != nil {
		return x.Expression
	}
	return ""
}

func (x *DocumentFilter) GetOperator() FilterOperator {
	if x != nil {
		return x.Operator
	}
	return FilterOperator_FILTER_OP_UNKNOWN
}

func (x *DocumentFilter) GetValues() []*FilterValues {
	if x != nil {
		return x.Values
	}
	return nil
}

func (x *DocumentFilter) GetAnd() []*DocumentFilter {
	if x != nil {
		return x.And
	}
	return nil
}

func (x *DocumentFilter) GetOr() []*DocumentFilter {
	if x != nil {
		return x.Or
	}
	return nil
}

type FilterValues struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Values        map[string]string      `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FilterValues) Reset() {
	*x = FilterValues{}
	mi := &file_repository_service_proto_msgTypes[157]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FilterValues) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FilterValues) ProtoMessage() {}

func (x *FilterValues) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[157]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FilterValues.ProtoReflect.Descriptor instead.
func (*FilterValues) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{157}
}

func (x *FilterValues) GetValues() map[string]string {
	if x != nil {
		return x.Values
	}
	return nil
}

type GetSocketTokenRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSocketTokenRequest) Reset() {
	*x = GetSocketTokenRequest{}
	mi := &file_repository_service_proto_msgTypes[158]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSocketTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSocketTokenRequest) ProtoMessage() {}

func (x *GetSocketTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[158]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSocketTokenRequest.ProtoReflect.Descriptor instead.
func (*GetSocketTokenRequest) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{158}
}

type GetSocketTokenResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Token that is unique for the calling user that can be used to open
	// websocket API connections.
	Token string `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	// Expires is the expiry time of the token as a RFC3339 timestamp.
	Expires       string `protobuf:"bytes,2,opt,name=expires,proto3" json:"expires,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSocketTokenResponse) Reset() {
	*x = GetSocketTokenResponse{}
	mi := &file_repository_service_proto_msgTypes[159]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSocketTokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSocketTokenResponse) ProtoMessage() {}

func (x *GetSocketTokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_repository_service_proto_msgTypes[159]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSocketTokenResponse.ProtoReflect.Descriptor instead.
func (*GetSocketTokenResponse) Descriptor() ([]byte, []int) {
	return file_repository_service_proto_rawDescGZIP(), []int{159}
}

func (x *GetSocketTokenResponse) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *GetSocketTokenResponse) GetExpires() string {
	if x != nil {
		return x.Expires
	}
	return ""
}

var File_repository_service_proto protoreflect.FileDescriptor

const file_repository_service_proto_rawDesc = "" +
	"\n" +
	"\x18repository/service.proto\x12\x13elephant.repository\x1a\x15newsdoc/newsdoc.proto\"a\n" +
	"\fEvictRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12#\n" +
	"\runtil_version\x18\x03 \x01(\x03R\funtilVersion\"0\n" +
	"\rEvictResponse\x12\x1f\n" +
	"\vevict_count\x18\x01 \x01(\x03R\n" +
	"evictCount\"J\n" +
	"\x10GetStatusRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x0e\n" +
	"\x02id\x18\x03 \x01(\x03R\x02id\"H\n" +
	"\x11GetStatusResponse\x123\n" +
	"\x06status\x18\x01 \x01(\v2\x1b.elephant.repository.StatusR\x06status\"Y\n" +
	"\x17GetStatusHistoryRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x16\n" +
	"\x06before\x18\x03 \x01(\x03R\x06before\"R\n" +
	"\x17GetStatusHistoryReponse\x127\n" +
	"\bstatuses\x18\x01 \x03(\v2\x1b.elephant.repository.StatusR\bstatuses\"A\n" +
	"\x15GetNilStatusesRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x14\n" +
	"\x05names\x18\x02 \x03(\tR\x05names\"\xd3\x01\n" +
	"\x16GetNilStatusesResponse\x12U\n" +
	"\bstatuses\x18\x01 \x03(\v29.elephant.repository.GetNilStatusesResponse.StatusesEntryR\bstatuses\x1ab\n" +
	"\rStatusesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12;\n" +
	"\x05value\x18\x02 \x01(\v2%.elephant.repository.DocumentStatusesR\x05value:\x028\x01\"g\n" +
	"\x18GetStatusOverviewRequest\x12\x14\n" +
	"\x05uuids\x18\x01 \x03(\tR\x05uuids\x12\x1a\n" +
	"\bstatuses\x18\x02 \x03(\tR\bstatuses\x12\x19\n" +
	"\bget_meta\x18\x03 \x01(\bR\agetMeta\"Z\n" +
	"\x19GetStatusOverviewResponse\x12=\n" +
	"\x05items\x18\x01 \x03(\v2'.elephant.repository.StatusOverviewItemR\x05items\"\x99\x03\n" +
	"\x12StatusOverviewItem\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x1a\n" +
	"\bmodified\x18\x03 \x01(\tR\bmodified\x12H\n" +
	"\x05heads\x18\x04 \x03(\v22.elephant.repository.StatusOverviewItem.HeadsEntryR\x05heads\x12%\n" +
	"\x0eworkflow_state\x18\x05 \x01(\tR\rworkflowState\x12/\n" +
	"\x13workflow_checkpoint\x18\x06 \x01(\tR\x12workflowCheckpoint\x12\x1f\n" +
	"\vcreator_uri\x18\a \x01(\tR\n" +
	"creatorUri\x12\x1f\n" +
	"\vupdater_uri\x18\b \x01(\tR\n" +
	"updaterUri\x1aU\n" +
	"\n" +
	"HeadsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x121\n" +
	"\x05value\x18\x02 \x01(\v2\x1b.elephant.repository.StatusR\x05value:\x028\x01\"+\n" +
	"\x15GetPermissionsRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\"\xb8\x01\n" +
	"\x16GetPermissionsResponse\x12^\n" +
	"\vpermissions\x18\x01 \x03(\v2<.elephant.repository.GetPermissionsResponse.PermissionsEntryR\vpermissions\x1a>\n" +
	"\x10PermissionsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x86\x01\n" +
	"\x12GetEventlogRequest\x12\x14\n" +
	"\x05after\x18\x01 \x01(\x03R\x05after\x12\x17\n" +
	"\await_ms\x18\x02 \x01(\x05R\x06waitMs\x12\x1d\n" +
	"\n" +
	"batch_size\x18\x03 \x01(\x05R\tbatchSize\x12\"\n" +
	"\rbatch_wait_ms\x18\x04 \x01(\x05R\vbatchWaitMs\"N\n" +
	"\x13GetEventlogResponse\x127\n" +
	"\x05items\x18\x01 \x03(\v2!.elephant.repository.EventlogItemR\x05items\"\x8b\x01\n" +
	"\x1bGetCompactedEventlogRequest\x12\x14\n" +
	"\x05after\x18\x01 \x01(\x03R\x05after\x12\x14\n" +
	"\x05until\x18\x03 \x01(\x03R\x05until\x12\x14\n" +
	"\x05limit\x18\x04 \x01(\x05R\x05limit\x12\x16\n" +
	"\x06offset\x18\x05 \x01(\x05R\x06offset\x12\x12\n" +
	"\x04type\x18\x06 \x01(\tR\x04type\"W\n" +
	"\x1cGetCompactedEventlogResponse\x127\n" +
	"\x05items\x18\x01 \x03(\v2!.elephant.repository.EventlogItemR\x05items\"\xd1\x06\n" +
	"\fEventlogItem\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x14\n" +
	"\x05event\x18\x02 \x01(\tR\x05event\x12\x12\n" +
	"\x04uuid\x18\x03 \x01(\tR\x04uuid\x12\x1c\n" +
	"\ttimestamp\x18\x04 \x01(\tR\ttimestamp\x12\x18\n" +
	"\aversion\x18\x05 \x01(\x03R\aversion\x12\x16\n" +
	"\x06status\x18\x06 \x01(\tR\x06status\x12\x1b\n" +
	"\tstatus_id\x18\a \x01(\x03R\bstatusId\x12/\n" +
	"\x03acl\x18\b \x03(\v2\x1d.elephant.repository.ACLEntryR\x03acl\x12\x1f\n" +
	"\vupdater_uri\x18\t \x01(\tR\n" +
	"updaterUri\x12\x12\n" +
	"\x04type\x18\n" +
	" \x01(\tR\x04type\x12\x1a\n" +
	"\blanguage\x18\v \x01(\tR\blanguage\x12!\n" +
	"\fold_language\x18\f \x01(\tR\voldLanguage\x12#\n" +
	"\rmain_document\x18\r \x01(\tR\fmainDocument\x12!\n" +
	"\fsystem_state\x18\x0e \x01(\tR\vsystemState\x12%\n" +
	"\x0eworkflow_state\x18\x0f \x01(\tR\rworkflowState\x12/\n" +
	"\x13workflow_checkpoint\x18\x10 \x01(\tR\x12workflowCheckpoint\x12,\n" +
	"\x12main_document_type\x18\x11 \x01(\tR\x10mainDocumentType\x12)\n" +
	"\x10attached_objects\x18\x12 \x03(\tR\x0fattachedObjects\x12)\n" +
	"\x10detached_objects\x18\x13 \x03(\tR\x0fdetachedObjects\x12(\n" +
	"\x10delete_record_id\x18\x14 \x01(\x03R\x0edeleteRecordId\x12%\n" +
	"\x0edocument_nonce\x18\x15 \x01(\tR\rdocumentNonce\x12;\n" +
	"\ttimespans\x18\x16 \x03(\v2\x1d.elephant.repository.TimespanR\ttimespans\x12\x16\n" +
	"\x06labels\x18\x17 \x03(\tR\x06labels\x12+\n" +
	"\x11schema_generation\x18\x18 \x01(\x03R\x10schemaGeneration\".\n" +
	"\bTimespan\x12\x12\n" +
	"\x04from\x18\x01 \x01(\tR\x04from\x12\x0e\n" +
	"\x02to\x18\x02 \x01(\tR\x02to\"\x17\n" +
	"\x15GetStatusRulesRequest\"O\n" +
	"\x16GetStatusRulesResponse\x125\n" +
	"\x05rules\x18\x01 \x03(\v2\x1f.elephant.repository.StatusRuleR\x05rules\"(\n" +
	"\x12GetStatusesRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\"k\n" +
	"\x12SetWorkflowRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12A\n" +
	"\bworkflow\x18\x02 \x01(\v2%.elephant.repository.DocumentWorkflowR\bworkflow\"\x96\x01\n" +
	"\x10DocumentWorkflow\x12\x1b\n" +
	"\tstep_zero\x18\x01 \x01(\tR\bstepZero\x12\x1e\n" +
	"\n" +
	"checkpoint\x18\x02 \x01(\tR\n" +
	"checkpoint\x12/\n" +
	"\x13negative_checkpoint\x18\x03 \x01(\tR\x12negativeCheckpoint\x12\x14\n" +
	"\x05steps\x18\x04 \x03(\tR\x05steps\"\x15\n" +
	"\x13SetWorkflowResponse\"(\n" +
	"\x12GetWorkflowRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\"\x93\x01\n" +
	"\x13GetWorkflowResponse\x12A\n" +
	"\bworkflow\x18\x01 \x01(\v2%.elephant.repository.DocumentWorkflowR\bworkflow\x12\x1f\n" +
	"\vupdater_uri\x18\x02 \x01(\tR\n" +
	"updaterUri\x12\x18\n" +
	"\aupdated\x18\x03 \x01(\tR\aupdated\"+\n" +
	"\x15DeleteWorkflowRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\"\x18\n" +
	"\x16DeleteWorkflowResponse\"8\n" +
	"\x0eWorkflowStatus\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\"V\n" +
	"\x13GetStatusesResponse\x12?\n" +
	"\bstatuses\x18\x01 \x03(\v2#.elephant.repository.WorkflowStatusR\bstatuses\"Y\n" +
	"\x13UpdateStatusRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x1a\n" +
	"\bdisabled\x18\x03 \x01(\bR\bdisabled\"\x16\n" +
	"\x14UpdateStatusResponse\"N\n" +
	"\x17CreateStatusRuleRequest\x123\n" +
	"\x04rule\x18\x01 \x01(\v2\x1f.elephant.repository.StatusRuleR\x04rule\"\xb6\x01\n" +
	"\n" +
	"StatusRule\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12 \n" +
	"\vdescription\x18\x03 \x01(\tR\vdescription\x12\x1f\n" +
	"\vaccess_rule\x18\x04 \x01(\bR\n" +
	"accessRule\x12\x1d\n" +
	"\n" +
	"applies_to\x18\x05 \x03(\tR\tappliesTo\x12\x1e\n" +
	"\n" +
	"expression\x18\a \x01(\tR\n" +
	"expression\"\x1a\n" +
	"\x18CreateStatusRuleResponse\"A\n" +
	"\x17DeleteStatusRuleRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\"\x1a\n" +
	"\x18DeleteStatusRuleResponse\"\xa2\x02\n" +
	"\x12GetDocumentRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x16\n" +
	"\x06status\x18\x03 \x01(\tR\x06status\x124\n" +
	"\x04lock\x18\x04 \x01(\v2 .elephant.repository.AcquireLockR\x04lock\x12D\n" +
	"\rmeta_document\x18\x05 \x01(\x0e2\x1f.elephant.repository.GetMetaDocR\fmetaDocument\x122\n" +
	"\x15meta_document_version\x18\x06 \x01(\x03R\x13metaDocumentVersion\x12\x16\n" +
	"\x06subset\x18\a \x03(\tR\x06subset\"\x8b\x03\n" +
	"\x13GetDocumentResponse\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x123\n" +
	"\x06status\x18\x03 \x01(\v2\x1b.elephant.repository.StatusR\x06status\x125\n" +
	"\x04meta\x18\x04 \x01(\v2!.elephant.repository.MetaDocumentR\x04meta\x12(\n" +
	"\x10is_meta_document\x18\x05 \x01(\bR\x0eisMetaDocument\x12#\n" +
	"\rmain_document\x18\x06 \x01(\tR\fmainDocument\x12<\n" +
	"\x06subset\x18\a \x03(\v2$.elephant.repository.ExtractedValuesR\x06subset\x122\n" +
	"\x04lock\x18\b \x01(\v2\x1e.elephant.repository.LockGrantR\x04lock\"\xd9\x01\n" +
	"\x0fExtractedValues\x12\x1c\n" +
	"\textractor\x18\x01 \x01(\x03R\textractor\x12H\n" +
	"\x06values\x18\x02 \x03(\v20.elephant.repository.ExtractedValues.ValuesEntryR\x06values\x1a^\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x129\n" +
	"\x05value\x18\x02 \x01(\v2#.elephant.repository.ExtractedValueR\x05value:\x028\x01\"\x80\x01\n" +
	"\x0eExtractedValue\x12\x14\n" +
	"\x05value\x18\x01 \x01(\tR\x05value\x12$\n" +
	"\x05block\x18\x02 \x01(\v2\x0e.newsdoc.BlockR\x05block\x12\x1e\n" +
	"\n" +
	"annotation\x18\x03 \x01(\tR\n" +
	"annotation\x12\x12\n" +
	"\x04role\x18\x04 \x01(\tR\x04role\"m\n" +
	"\x0eBulkGetRequest\x12C\n" +
	"\tdocuments\x18\x01 \x03(\v2%.elephant.repository.BulkGetReferenceR\tdocuments\x12\x16\n" +
	"\x06subset\x18\x02 \x03(\tR\x06subset\"@\n" +
	"\x10BulkGetReference\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\"I\n" +
	"\x0fBulkGetResponse\x126\n" +
	"\x05items\x18\x01 \x03(\v2 .elephant.repository.BulkGetItemR\x05items\"\xa8\x01\n" +
	"\vBulkGetItem\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x12\n" +
	"\x04uuid\x18\x03 \x01(\tR\x04uuid\x12<\n" +
	"\x06subset\x18\x04 \x03(\v2$.elephant.repository.ExtractedValuesR\x06subset\"W\n" +
	"\fMetaDocument\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\"d\n" +
	"\x11GetHistoryRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x16\n" +
	"\x06before\x18\x02 \x01(\x03R\x06before\x12#\n" +
	"\rload_statuses\x18\x03 \x01(\bR\floadStatuses\"V\n" +
	"\x12GetHistoryResponse\x12@\n" +
	"\bversions\x18\x01 \x03(\v2$.elephant.repository.DocumentVersionR\bversions\"\x90\x03\n" +
	"\x0fDocumentVersion\x12\x18\n" +
	"\aversion\x18\x01 \x01(\x03R\aversion\x12\x18\n" +
	"\acreated\x18\x02 \x01(\tR\acreated\x12\x18\n" +
	"\acreator\x18\x03 \x01(\tR\acreator\x12B\n" +
	"\x04meta\x18\x04 \x03(\v2..elephant.repository.DocumentVersion.MetaEntryR\x04meta\x12N\n" +
	"\bstatuses\x18\x05 \x03(\v22.elephant.repository.DocumentVersion.StatusesEntryR\bstatuses\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1ab\n" +
	"\rStatusesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12;\n" +
	"\x05value\x18\x02 \x01(\v2%.elephant.repository.DocumentStatusesR\x05value:\x028\x01\"E\n" +
	"\x10DocumentStatuses\x121\n" +
	"\x05items\x18\x05 \x03(\v2\x1b.elephant.repository.StatusR\x05items\"\x89\a\n" +
	"\rUpdateRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12-\n" +
	"\bdocument\x18\x02 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12@\n" +
	"\x04meta\x18\x03 \x03(\v2,.elephant.repository.UpdateRequest.MetaEntryR\x04meta\x12\x19\n" +
	"\bif_match\x18\x04 \x01(\x03R\aifMatch\x129\n" +
	"\x06status\x18\x05 \x03(\v2!.elephant.repository.StatusUpdateR\x06status\x12/\n" +
	"\x03acl\x18\x06 \x03(\v2\x1d.elephant.repository.ACLEntryR\x03acl\x12O\n" +
	"\x10import_directive\x18\a \x01(\v2$.elephant.repository.ImportDirectiveR\x0fimportDirective\x12\x1c\n" +
	"\tlockToken\x18\b \x01(\tR\tlockToken\x120\n" +
	"\x14update_meta_document\x18\t \x01(\bR\x12updateMetaDocument\x12*\n" +
	"\x11if_workflow_state\x18\n" +
	" \x01(\tR\x0fifWorkflowState\x12]\n" +
	"\x0fif_status_heads\x18\v \x03(\v25.elephant.repository.UpdateRequest.IfStatusHeadsEntryR\rifStatusHeads\x12\\\n" +
	"\x0eattach_objects\x18\f \x03(\v25.elephant.repository.UpdateRequest.AttachObjectsEntryR\rattachObjects\x12%\n" +
	"\x0edetach_objects\x18\r \x03(\tR\rdetachObjects\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a@\n" +
	"\x12IfStatusHeadsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x03R\x05value:\x028\x01\x1a@\n" +
	"\x12AttachObjectsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"k\n" +
	"\x0fImportDirective\x12-\n" +
	"\x12originally_created\x18\x01 \x01(\tR\x11originallyCreated\x12)\n" +
	"\x10original_creator\x18\x02 \x01(\tR\x0foriginalCreator\">\n" +
	"\x0eUpdateResponse\x12\x18\n" +
	"\aversion\x18\x01 \x01(\x03R\aversion\x12\x12\n" +
	"\x04uuid\x18\x02 \x01(\tR\x04uuid\"Q\n" +
	"\x11BulkUpdateRequest\x12<\n" +
	"\aupdates\x18\x01 \x03(\v2\".elephant.repository.UpdateRequestR\aupdates\"S\n" +
	"\x12BulkUpdateResponse\x12=\n" +
	"\aupdates\x18\x01 \x03(\v2#.elephant.repository.UpdateResponseR\aupdates\"@\n" +
	"\x0fValidateRequest\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\"Q\n" +
	"\x10ValidateResponse\x12=\n" +
	"\x06errors\x18\x01 \x03(\v2%.elephant.repository.ValidationResultR\x06errors\"=\n" +
	"\fPruneRequest\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\"}\n" +
	"\rPruneResponse\x12-\n" +
	"\bdocument\x18\x01 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12=\n" +
	"\x06errors\x18\x02 \x03(\v2%.elephant.repository.ValidationResultR\x06errors\"`\n" +
	"\x10ValidationResult\x126\n" +
	"\x06entity\x18\x01 \x03(\v2\x1e.elephant.repository.EntityRefR\x06entity\x12\x14\n" +
	"\x05error\x18\x02 \x01(\tR\x05error\"\x8a\x01\n" +
	"\tEntityRef\x12\x19\n" +
	"\bref_type\x18\x01 \x01(\tR\arefType\x12\x12\n" +
	"\x04kind\x18\x02 \x01(\tR\x04kind\x12\x14\n" +
	"\x05index\x18\x03 \x01(\x03R\x05index\x12\x12\n" +
	"\x04name\x18\x04 \x01(\tR\x04name\x12\x12\n" +
	"\x04type\x18\x05 \x01(\tR\x04type\x12\x10\n" +
	"\x03rel\x18\x06 \x01(\tR\x03rel\"\xd1\x01\n" +
	"\fStatusUpdate\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12?\n" +
	"\x04meta\x18\x03 \x03(\v2+.elephant.repository.StatusUpdate.MetaEntryR\x04meta\x12\x19\n" +
	"\bif_match\x18\x04 \x01(\x03R\aifMatch\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"a\n" +
	"\x18UpdatePermissionsRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x121\n" +
	"\x04list\x18\x02 \x03(\v2\x1d.elephant.repository.ACLEntryR\x04list\"\x1b\n" +
	"\x19UpdatePermissionsResponse\"\xe7\x01\n" +
	"\x15DeleteDocumentRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12H\n" +
	"\x04meta\x18\x02 \x03(\v24.elephant.repository.DeleteDocumentRequest.MetaEntryR\x04meta\x12\x19\n" +
	"\bif_match\x18\x03 \x01(\x03R\aifMatch\x12\x1c\n" +
	"\tlockToken\x18\b \x01(\tR\tlockToken\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x18\n" +
	"\x16DeleteDocumentResponse\"\x7f\n" +
	"\x0eRestoreRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12(\n" +
	"\x10delete_record_id\x18\x02 \x01(\x03R\x0edeleteRecordId\x12/\n" +
	"\x03acl\x18\x03 \x03(\v2\x1d.elephant.repository.ACLEntryR\x03acl\"\x11\n" +
	"\x0fRestoreResponse\"L\n" +
	"\fPurgeRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12(\n" +
	"\x10delete_record_id\x18\x02 \x01(\x03R\x0edeleteRecordId\"\x0f\n" +
	"\rPurgeResponse\"\x82\x01\n" +
	"\x12ListDeletedRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x1b\n" +
	"\tbefore_id\x18\x02 \x01(\x03R\bbeforeId\x12\x1f\n" +
	"\vbefore_date\x18\x03 \x01(\tR\n" +
	"beforeDate\x12\x1a\n" +
	"\btimezone\x18\x04 \x01(\tR\btimezone\"R\n" +
	"\x13ListDeletedResponse\x12;\n" +
	"\adeletes\x18\x01 \x03(\v2!.elephant.repository.DeleteRecordR\adeletes\"\xf2\x02\n" +
	"\fDeleteRecord\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x12\n" +
	"\x04uuid\x18\x02 \x01(\tR\x04uuid\x12\x10\n" +
	"\x03uri\x18\x03 \x01(\tR\x03uri\x12\x12\n" +
	"\x04type\x18\x04 \x01(\tR\x04type\x12\x18\n" +
	"\aversion\x18\x05 \x01(\x03R\aversion\x12\x18\n" +
	"\acreated\x18\x06 \x01(\tR\acreated\x12\x18\n" +
	"\acreator\x18\a \x01(\tR\acreator\x12?\n" +
	"\x04meta\x18\b \x03(\v2+.elephant.repository.DeleteRecord.MetaEntryR\x04meta\x12\x1a\n" +
	"\blanguage\x18\t \x01(\tR\blanguage\x12\x1c\n" +
	"\tfinalised\x18\n" +
	" \x01(\tR\tfinalised\x12\x16\n" +
	"\x06purged\x18\v \x01(\tR\x06purged\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"$\n" +
	"\x0eGetMetaRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\"H\n" +
	"\x0fGetMetaResponse\x125\n" +
	"\x04meta\x18\x01 \x01(\v2!.elephant.repository.DocumentMetaR\x04meta\"\xc1\x05\n" +
	"\fDocumentMeta\x12\x18\n" +
	"\acreated\x18\x01 \x01(\tR\acreated\x12\x1a\n" +
	"\bmodified\x18\x02 \x01(\tR\bmodified\x12'\n" +
	"\x0fcurrent_version\x18\x03 \x01(\x03R\x0ecurrentVersion\x12B\n" +
	"\x05heads\x18\x04 \x03(\v2,.elephant.repository.DocumentMeta.HeadsEntryR\x05heads\x12/\n" +
	"\x03acl\x18\x05 \x03(\v2\x1d.elephant.repository.ACLEntryR\x03acl\x12-\n" +
	"\x04lock\x18\x06 \x01(\v2\x19.elephant.repository.LockR\x04lock\x12(\n" +
	"\x10is_meta_document\x18\a \x01(\bR\x0eisMetaDocument\x12#\n" +
	"\rmain_document\x18\b \x01(\tR\fmainDocument\x12%\n" +
	"\x0eworkflow_state\x18\t \x01(\tR\rworkflowState\x12/\n" +
	"\x13workflow_checkpoint\x18\n" +
	" \x01(\tR\x12workflowCheckpoint\x12\x1f\n" +
	"\vcreator_uri\x18\v \x01(\tR\n" +
	"creatorUri\x12\x1f\n" +
	"\vupdater_uri\x18\f \x01(\tR\n" +
	"updaterUri\x12D\n" +
	"\vattachments\x18\r \x03(\v2\".elephant.repository.AttachmentRefR\vattachments\x12\x14\n" +
	"\x05nonce\x18\x0e \x01(\tR\x05nonce\x12\x12\n" +
	"\x04type\x18\x0f \x01(\tR\x04type\x1aU\n" +
	"\n" +
	"HeadsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x121\n" +
	"\x05value\x18\x02 \x01(\v2\x1b.elephant.repository.StatusR\x05value:\x028\x01\"=\n" +
	"\rAttachmentRef\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\"\x84\x02\n" +
	"\x06Status\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x18\n" +
	"\acreator\x18\x03 \x01(\tR\acreator\x12\x18\n" +
	"\acreated\x18\x04 \x01(\tR\acreated\x129\n" +
	"\x04meta\x18\x05 \x03(\v2%.elephant.repository.Status.MetaEntryR\x04meta\x12(\n" +
	"\x10meta_doc_version\x18\x06 \x01(\x03R\x0emetaDocVersion\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\">\n" +
	"\bACLEntry\x12\x10\n" +
	"\x03uri\x18\x01 \x01(\tR\x03uri\x12 \n" +
	"\vpermissions\x18\x02 \x03(\tR\vpermissions\"\xd6\x01\n" +
	"\x04Lock\x12\x14\n" +
	"\x05token\x18\x01 \x01(\tR\x05token\x12\x10\n" +
	"\x03uri\x18\x02 \x01(\tR\x03uri\x12\x18\n" +
	"\acreated\x18\x03 \x01(\tR\acreated\x12\x18\n" +
	"\aexpires\x18\x04 \x01(\tR\aexpires\x12\x10\n" +
	"\x03app\x18\x05 \x01(\tR\x03app\x12\x18\n" +
	"\acomment\x18\x06 \x01(\tR\acomment\x12F\n" +
	"\vexclusivity\x18\a \x01(\x0e2$.elephant.repository.LockExclusivityR\vexclusivity\"K\n" +
	"\x17RegisterMetaTypeRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12\x1c\n" +
	"\texclusive\x18\x02 \x01(\bR\texclusive\"\x1a\n" +
	"\x18RegisterMetaTypeResponse\"V\n" +
	"\x1aRegisterMetaTypeUseRequest\x12\x1b\n" +
	"\tmain_type\x18\x01 \x01(\tR\bmainType\x12\x1b\n" +
	"\tmeta_type\x18\x02 \x01(\tR\bmetaType\"\x1d\n" +
	"\x1bRegisterMetaTypeUseResponse\"\x15\n" +
	"\x13GetMetaTypesRequest\"O\n" +
	"\x14GetMetaTypesResponse\x127\n" +
	"\x05types\x18\x01 \x03(\v2!.elephant.repository.MetaTypeInfoR\x05types\"\x19\n" +
	"\x17GetDocumentTypesRequest\"0\n" +
	"\x18GetDocumentTypesResponse\x12\x14\n" +
	"\x05types\x18\x01 \x03(\tR\x05types\";\n" +
	"\fMetaTypeInfo\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x17\n" +
	"\aused_by\x18\x02 \x03(\tR\x06usedBy\"\xca\x01\n" +
	"\x19RegisterGenerationRequest\x125\n" +
	"\aschemas\x18\x01 \x03(\v2\x1b.elephant.repository.SchemaR\aschemas\x12E\n" +
	"\n" +
	"activation\x18\x02 \x01(\x0e2%.elephant.repository.SchemaActivationR\n" +
	"activation\x12/\n" +
	"\texemplars\x18\x03 \x03(\v2\x11.newsdoc.DocumentR\texemplars\"A\n" +
	"\x1aRegisterGenerationResponse\x12#\n" +
	"\rgeneration_id\x18\x01 \x01(\x03R\fgenerationId\"\x85\x01\n" +
	"\x17SetActiveSchemasRequest\x12#\n" +
	"\rgeneration_id\x18\x01 \x01(\x03R\fgenerationId\x12E\n" +
	"\n" +
	"activation\x18\x02 \x01(\x0e2%.elephant.repository.SchemaActivationR\n" +
	"activation\"\x1a\n" +
	"\x18SetActiveSchemasResponse\"6\n" +
	"\x1cListSchemaGenerationsRequest\x12\x16\n" +
	"\x06before\x18\x01 \x01(\x03R\x06before\"\\\n" +
	"\x1dListSchemaGenerationsResponse\x12;\n" +
	"\x05items\x18\x01 \x03(\v2%.elephant.repository.SchemaGenerationR\x05items\"\xfb\x01\n" +
	"\x10SchemaGeneration\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12=\n" +
	"\x06status\x18\x02 \x01(\x0e2%.elephant.repository.SchemaActivationR\x06status\x12\x18\n" +
	"\acreated\x18\x03 \x01(\tR\acreated\x12\x1c\n" +
	"\tactivated\x18\x04 \x01(\tR\tactivated\x12 \n" +
	"\vdeactivated\x18\x05 \x01(\tR\vdeactivated\x12>\n" +
	"\aschemas\x18\x06 \x03(\v2$.elephant.repository.SchemaReferenceR\aschemas\"\xbf\x01\n" +
	"\x13GetExemplarsRequest\x12#\n" +
	"\rgeneration_id\x18\x01 \x01(\x03R\fgenerationId\x12I\n" +
	"\x05known\x18\x02 \x03(\v23.elephant.repository.GetExemplarsRequest.KnownEntryR\x05known\x1a8\n" +
	"\n" +
	"KnownEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"S\n" +
	"\x14GetExemplarsResponse\x12;\n" +
	"\texemplars\x18\x01 \x03(\v2\x1d.elephant.repository.ExemplarR\texemplars\"p\n" +
	"\bExemplar\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12-\n" +
	"\bdocument\x18\x02 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x12!\n" +
	"\fversion_hash\x18\x03 \x01(\tR\vversionHash\"@\n" +
	"\x10GetSchemaRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x02 \x01(\tR\aversion\"A\n" +
	"\x11GetSchemaResponse\x12\x18\n" +
	"\aversion\x18\x01 \x01(\tR\aversion\x12\x12\n" +
	"\x04spec\x18\x02 \x01(\fR\x04spec\"\xee\x01\n" +
	"\x1aGetAllActiveSchemasRequest\x12!\n" +
	"\fwait_seconds\x18\x01 \x01(\x03R\vwaitSeconds\x12P\n" +
	"\x05known\x18\x02 \x03(\v2:.elephant.repository.GetAllActiveSchemasRequest.KnownEntryR\x05known\x12!\n" +
	"\fonly_changed\x18\x03 \x01(\bR\vonlyChanged\x1a8\n" +
	"\n" +
	"KnownEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xb1\x01\n" +
	"\x1bGetAllActiveSchemasResponse\x125\n" +
	"\aschemas\x18\x01 \x03(\v2\x1b.elephant.repository.SchemaR\aschemas\x12\x1c\n" +
	"\tunchanged\x18\x02 \x01(\bR\tunchanged\x12\x18\n" +
	"\aremoved\x18\x03 \x03(\tR\aremoved\x12#\n" +
	"\rgeneration_id\x18\x04 \x01(\x03R\fgenerationId\"\x1a\n" +
	"\x18ListActiveSchemasRequest\"w\n" +
	"\x19ListActiveSchemasResponse\x125\n" +
	"\aschemas\x18\x01 \x03(\v2\x1b.elephant.repository.SchemaR\aschemas\x12#\n" +
	"\rgeneration_id\x18\x02 \x01(\x03R\fgenerationId\"?\n" +
	"\x0fSchemaReference\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x02 \x01(\tR\aversion\"J\n" +
	"\x06Schema\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x02 \x01(\tR\aversion\x12\x12\n" +
	"\x04spec\x18\x03 \x01(\tR\x04spec\"\x18\n" +
	"\x16GetDeprecationsRequest\"?\n" +
	"\vDeprecation\x12\x14\n" +
	"\x05label\x18\x01 \x01(\tR\x05label\x12\x1a\n" +
	"\benforced\x18\x02 \x01(\bR\benforced\"_\n" +
	"\x17GetDeprecationsResponse\x12D\n" +
	"\fdeprecations\x18\x01 \x03(\v2 .elephant.repository.DeprecationR\fdeprecations\"^\n" +
	"\x18UpdateDeprecationRequest\x12B\n" +
	"\vdeprecation\x18\x01 \x01(\v2 .elephant.repository.DeprecationR\vdeprecation\"\x1b\n" +
	"\x19UpdateDeprecationResponse\"y\n" +
	"\x19RegisterMetricKindRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12H\n" +
	"\vaggregation\x18\x02 \x01(\x0e2&.elephant.repository.MetricAggregationR\vaggregation\"\x1c\n" +
	"\x1aRegisterMetricKindResponse\"-\n" +
	"\x17DeleteMetricKindRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\"\x1a\n" +
	"\x18DeleteMetricKindResponse\"\x17\n" +
	"\x15GetMetricKindsRequest\"O\n" +
	"\x16GetMetricKindsResponse\x125\n" +
	"\x05kinds\x18\x01 \x03(\v2\x1f.elephant.repository.MetricKindR\x05kinds\"j\n" +
	"\n" +
	"MetricKind\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12H\n" +
	"\vaggregation\x18\x02 \x01(\x0e2&.elephant.repository.MetricAggregationR\vaggregation\"k\n" +
	"\x15RegisterMetricRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x12\n" +
	"\x04kind\x18\x02 \x01(\tR\x04kind\x12\x14\n" +
	"\x05label\x18\x03 \x01(\tR\x05label\x12\x14\n" +
	"\x05value\x18\x05 \x01(\x03R\x05value\"\x18\n" +
	"\x16RegisterMetricResponse\"?\n" +
	"\x11GetMetricsRequest\x12\x14\n" +
	"\x05uuids\x18\x01 \x03(\tR\x05uuids\x12\x14\n" +
	"\x05kinds\x18\x02 \x03(\tR\x05kinds\"\xce\x01\n" +
	"\x12GetMetricsResponse\x12T\n" +
	"\tdocuments\x18\x01 \x03(\v26.elephant.repository.GetMetricsResponse.DocumentsEntryR\tdocuments\x1ab\n" +
	"\x0eDocumentsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12:\n" +
	"\x05value\x18\x02 \x01(\v2$.elephant.repository.DocumentMetricsR\x05value:\x028\x01\"\\\n" +
	"\x0fDocumentMetrics\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x125\n" +
	"\ametrics\x18\x02 \x03(\v2\x1b.elephant.repository.MetricR\ametrics\"H\n" +
	"\x06Metric\x12\x12\n" +
	"\x04kind\x18\x01 \x01(\tR\x04kind\x12\x14\n" +
	"\x05label\x18\x02 \x01(\tR\x05label\x12\x14\n" +
	"\x05value\x18\x03 \x01(\x03R\x05value\"\xa7\x01\n" +
	"\vLockRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x10\n" +
	"\x03ttl\x18\x02 \x01(\x05R\x03ttl\x12\x10\n" +
	"\x03app\x18\x03 \x01(\tR\x03app\x12\x18\n" +
	"\acomment\x18\x04 \x01(\tR\acomment\x12F\n" +
	"\vexclusivity\x18\x05 \x01(\x0e2$.elephant.repository.LockExclusivityR\vexclusivity\">\n" +
	"\fLockResponse\x12\x14\n" +
	"\x05token\x18\x01 \x01(\tR\x05token\x12\x18\n" +
	"\aexpires\x18\x02 \x01(\tR\aexpires\"\x93\x01\n" +
	"\vAcquireLock\x12\x10\n" +
	"\x03ttl\x18\x01 \x01(\x05R\x03ttl\x12\x10\n" +
	"\x03app\x18\x02 \x01(\tR\x03app\x12\x18\n" +
	"\acomment\x18\x03 \x01(\tR\acomment\x12F\n" +
	"\vexclusivity\x18\x04 \x01(\x0e2$.elephant.repository.LockExclusivityR\vexclusivity\";\n" +
	"\tLockGrant\x12\x14\n" +
	"\x05token\x18\x01 \x01(\tR\x05token\x12\x18\n" +
	"\aexpires\x18\x02 \x01(\tR\aexpires\"O\n" +
	"\x11ExtendLockRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x10\n" +
	"\x03ttl\x18\x02 \x01(\x05R\x03ttl\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\"9\n" +
	"\rUnlockRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x14\n" +
	"\x05token\x18\x02 \x01(\tR\x05token\"\x10\n" +
	"\x0eUnlockResponse\"\x14\n" +
	"\x12GetWithheldRequest\"S\n" +
	"\x13GetWithheldResponse\x12<\n" +
	"\x05items\x18\x01 \x03(\v2&.elephant.repository.ScheduledDocumentR\x05items\"\x85\x02\n" +
	"\x11ScheduledDocument\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x12\n" +
	"\x04type\x18\x02 \x01(\tR\x04type\x12\x1b\n" +
	"\tstatus_id\x18\x03 \x01(\x03R\bstatusId\x12)\n" +
	"\x10document_version\x18\x04 \x01(\x03R\x0fdocumentVersion\x12#\n" +
	"\rplanning_item\x18\x05 \x01(\tR\fplanningItem\x12\x1e\n" +
	"\n" +
	"assignment\x18\x06 \x01(\tR\n" +
	"assignment\x12\x18\n" +
	"\apublish\x18\a \x01(\tR\apublish\x12!\n" +
	"\fscheduled_by\x18\b \x01(\tR\vscheduledBy\"/\n" +
	"\x19GetDeliverableInfoRequest\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\"\xb5\x01\n" +
	"\x1aGetDeliverableInfoResponse\x12*\n" +
	"\x11has_planning_info\x18\x01 \x01(\bR\x0fhasPlanningInfo\x12#\n" +
	"\rplanning_uuid\x18\x02 \x01(\tR\fplanningUuid\x12'\n" +
	"\x0fassignment_uuid\x18\x03 \x01(\tR\x0eassignmentUuid\x12\x1d\n" +
	"\n" +
	"event_uuid\x18\x04 \x01(\tR\teventUuid\"5\n" +
	"\x1dBulkGetDeliverableInfoRequest\x12\x14\n" +
	"\x05uuids\x18\x01 \x03(\tR\x05uuids\"\\\n" +
	"\x1eBulkGetDeliverableInfoResponse\x12:\n" +
	"\x05items\x18\x01 \x03(\v2$.elephant.repository.DeliverableInfoR\x05items\"\x92\x01\n" +
	"\x0fDeliverableInfo\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12#\n" +
	"\rplanning_uuid\x18\x02 \x01(\tR\fplanningUuid\x12'\n" +
	"\x0fassignment_uuid\x18\x03 \x01(\tR\x0eassignmentUuid\x12\x1d\n" +
	"\n" +
	"event_uuid\x18\x04 \x01(\tR\teventUuid\"\xcd\x01\n" +
	"\x13CreateUploadRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12!\n" +
	"\fcontent_type\x18\x02 \x01(\tR\vcontentType\x12F\n" +
	"\x04meta\x18\x03 \x03(\v22.elephant.repository.CreateUploadRequest.MetaEntryR\x04meta\x1a7\n" +
	"\tMetaEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"8\n" +
	"\x14CreateUploadResponse\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x10\n" +
	"\x03url\x18\x02 \x01(\tR\x03url\"\x83\x01\n" +
	"\x15GetAttachmentsRequest\x12\x1c\n" +
	"\tdocuments\x18\x01 \x03(\tR\tdocuments\x12'\n" +
	"\x0fattachment_name\x18\x02 \x01(\tR\x0eattachmentName\x12#\n" +
	"\rdownload_link\x18\x03 \x01(\bR\fdownloadLink\"b\n" +
	"\x16GetAttachmentsResponse\x12H\n" +
	"\vattachments\x18\x01 \x03(\v2&.elephant.repository.AttachmentDetailsR\vattachments\"\xc1\x01\n" +
	"\x11AttachmentDetails\x12\x1a\n" +
	"\bdocument\x18\x01 \x01(\tR\bdocument\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x18\n" +
	"\aversion\x18\x03 \x01(\x03R\aversion\x12#\n" +
	"\rdownload_link\x18\x04 \x01(\tR\fdownloadLink\x12\x1a\n" +
	"\bfilename\x18\x05 \x01(\tR\bfilename\x12!\n" +
	"\fcontent_type\x18\x06 \x01(\tR\vcontentType\"x\n" +
	"\x14ConfigureTypeRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x12L\n" +
	"\rconfiguration\x18\x02 \x01(\v2&.elephant.repository.TypeConfigurationR\rconfiguration\"\xbb\x02\n" +
	"\x11TypeConfiguration\x12-\n" +
	"\x12bounded_collection\x18\x01 \x01(\bR\x11boundedCollection\x12R\n" +
	"\x10time_expressions\x18\x02 \x03(\v2'.elephant.repository.TypeTimeExpressionR\x0ftimeExpressions\x12Q\n" +
	"\x11label_expressions\x18\x03 \x03(\v2$.elephant.repository.LabelExpressionR\x10labelExpressions\x12\x1a\n" +
	"\bvariants\x18\x04 \x03(\tR\bvariants\x124\n" +
	"\x16evict_noncurrent_after\x18\x05 \x01(\x03R\x14evictNoncurrentAfter\"h\n" +
	"\x12TypeTimeExpression\x12\x1e\n" +
	"\n" +
	"expression\x18\x01 \x01(\tR\n" +
	"expression\x12\x16\n" +
	"\x06layout\x18\x02 \x01(\tR\x06layout\x12\x1a\n" +
	"\btimezone\x18\x03 \x01(\tR\btimezone\"M\n" +
	"\x0fLabelExpression\x12\x1e\n" +
	"\n" +
	"expression\x18\x01 \x01(\tR\n" +
	"expression\x12\x1a\n" +
	"\btemplate\x18\x02 \x01(\tR\btemplate\"\x17\n" +
	"\x15ConfigureTypeResponse\"1\n" +
	"\x1bGetTypeConfigurationRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\"l\n" +
	"\x1cGetTypeConfigurationResponse\x12L\n" +
	"\rconfiguration\x18\x01 \x01(\v2&.elephant.repository.TypeConfigurationR\rconfiguration\"\xa2\x02\n" +
	"\x12GetMatchingRequest\x12\x12\n" +
	"\x04type\x18\x01 \x01(\tR\x04type\x129\n" +
	"\btimespan\x18\x02 \x01(\v2\x1d.elephant.repository.TimespanR\btimespan\x12\x16\n" +
	"\x06labels\x18\x03 \x03(\tR\x06labels\x12;\n" +
	"\x06filter\x18\x04 \x01(\v2#.elephant.repository.DocumentFilterR\x06filter\x12+\n" +
	"\x11include_documents\x18\x05 \x01(\bR\x10includeDocuments\x12!\n" +
	"\finclude_meta\x18\x06 \x01(\bR\vincludeMeta\x12\x18\n" +
	"\ainclude\x18\a \x03(\tR\ainclude\"S\n" +
	"\x13GetMatchingResponse\x12<\n" +
	"\amatches\x18\x01 \x03(\v2\".elephant.repository.DocumentMatchR\amatches\"\xa3\x01\n" +
	"\rDocumentMatch\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12-\n" +
	"\bdocument\x18\x03 \x01(\v2\x11.newsdoc.DocumentR\bdocument\x125\n" +
	"\x04meta\x18\x04 \x01(\v2!.elephant.repository.DocumentMetaR\x04meta\"\x98\x02\n" +
	"\x0eDocumentFilter\x12\x1e\n" +
	"\n" +
	"expression\x18\x01 \x01(\tR\n" +
	"expression\x12?\n" +
	"\boperator\x18\x02 \x01(\x0e2#.elephant.repository.FilterOperatorR\boperator\x129\n" +
	"\x06values\x18\x03 \x03(\v2!.elephant.repository.FilterValuesR\x06values\x125\n" +
	"\x03and\x18\x04 \x03(\v2#.elephant.repository.DocumentFilterR\x03and\x123\n" +
	"\x02or\x18\x05 \x03(\v2#.elephant.repository.DocumentFilterR\x02or\"\x90\x01\n" +
	"\fFilterValues\x12E\n" +
	"\x06values\x18\x01 \x03(\v2-.elephant.repository.FilterValues.ValuesEntryR\x06values\x1a9\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x17\n" +
	"\x15GetSocketTokenRequest\"H\n" +
	"\x16GetSocketTokenResponse\x12\x14\n" +
	"\x05token\x18\x01 \x01(\tR\x05token\x12\x18\n" +
	"\aexpires\x18\x02 \x01(\tR\aexpires*<\n" +
	"\n" +
	"GetMetaDoc\x12\r\n" +
	"\tMETA_NONE\x10\x00\x12\x10\n" +
	"\fMETA_INCLUDE\x10\x01\x12\r\n" +
	"\tMETA_ONLY\x10\x02*W\n" +
	"\x0fLockExclusivity\x12\x11\n" +
	"\rLOCK_DOCUMENT\x10\x00\x12\x0f\n" +
	"\vLOCK_STATUS\x10\x01\x12\f\n" +
	"\bLOCK_ACL\x10\x02\x12\x12\n" +
	"\x0eLOCK_EXCLUSIVE\x10\x03*u\n" +
	"\x10SchemaActivation\x12\x16\n" +
	"\x12ACTIVATION_UNKNOWN\x10\x00\x12\x15\n" +
	"\x11ACTIVATION_ACTIVE\x10\x01\x12\x16\n" +
	"\x12ACTIVATION_PENDING\x10\x02\x12\x1a\n" +
	"\x16ACTIVATION_DEACTIVATED\x10\x03*9\n" +
	"\x11MetricAggregation\x12\b\n" +
	"\x04NONE\x10\x00\x12\v\n" +
	"\aREPLACE\x10\x01\x12\r\n" +
	"\tINCREMENT\x10\x02*a\n" +
	"\x0eFilterOperator\x12\x15\n" +
	"\x11FILTER_OP_UNKNOWN\x10\x00\x12\x11\n" +
	"\rFILTER_OP_ANY\x10\x01\x12\x11\n" +
	"\rFILTER_OP_ALL\x10\x02\x12\x12\n" +
	"\x0eFILTER_OP_NONE\x10\x032\xe8\x16\n" +
	"\tDocuments\x12X\n" +
	"\x03Get\x12'.elephant.repository.GetDocumentRequest\x1a(.elephant.repository.GetDocumentResponse\x12T\n" +
	"\aBulkGet\x12#.elephant.repository.BulkGetRequest\x1a$.elephant.repository.BulkGetResponse\x12]\n" +
	"\n" +
	"GetHistory\x12&.elephant.repository.GetHistoryRequest\x1a'.elephant.repository.GetHistoryResponse\x12Q\n" +
	"\x06Update\x12\".elephant.repository.UpdateRequest\x1a#.elephant.repository.UpdateResponse\x12]\n" +
	"\n" +
	"BulkUpdate\x12&.elephant.repository.BulkUpdateRequest\x1a'.elephant.repository.BulkUpdateResponse\x12W\n" +
	"\bValidate\x12$.elephant.repository.ValidateRequest\x1a%.elephant.repository.ValidateResponse\x12N\n" +
	"\x05Prune\x12!.elephant.repository.PruneRequest\x1a\".elephant.repository.PruneResponse\x12N\n" +
	"\x05Evict\x12!.elephant.repository.EvictRequest\x1a\".elephant.repository.EvictResponse\x12a\n" +
	"\x06Delete\x12*.elephant.repository.DeleteDocumentRequest\x1a+.elephant.repository.DeleteDocumentResponse\x12`\n" +
	"\vListDeleted\x12'.elephant.repository.ListDeletedRequest\x1a(.elephant.repository.ListDeletedResponse\x12T\n" +
	"\aRestore\x12#.elephant.repository.RestoreRequest\x1a$.elephant.repository.RestoreResponse\x12N\n" +
	"\x05Purge\x12!.elephant.repository.PurgeRequest\x1a\".elephant.repository.PurgeResponse\x12T\n" +
	"\aGetMeta\x12#.elephant.repository.GetMetaRequest\x1a$.elephant.repository.GetMetaResponse\x12]\n" +
	"\bEventlog\x12'.elephant.repository.GetEventlogRequest\x1a(.elephant.repository.GetEventlogResponse\x12x\n" +
	"\x11CompactedEventlog\x120.elephant.repository.GetCompactedEventlogRequest\x1a1.elephant.repository.GetCompactedEventlogResponse\x12Z\n" +
	"\tGetStatus\x12%.elephant.repository.GetStatusRequest\x1a&.elephant.repository.GetStatusResponse\x12n\n" +
	"\x10GetStatusHistory\x12,.elephant.repository.GetStatusHistoryRequest\x1a,.elephant.repository.GetStatusHistoryReponse\x12i\n" +
	"\x0eGetNilStatuses\x12*.elephant.repository.GetNilStatusesRequest\x1a+.elephant.repository.GetNilStatusesResponse\x12r\n" +
	"\x11GetStatusOverview\x12-.elephant.repository.GetStatusOverviewRequest\x1a..elephant.repository.GetStatusOverviewResponse\x12i\n" +
	"\x0eGetPermissions\x12*.elephant.repository.GetPermissionsRequest\x1a+.elephant.repository.GetPermissionsResponse\x12K\n" +
	"\x04Lock\x12 .elephant.repository.LockRequest\x1a!.elephant.repository.LockResponse\x12W\n" +
	"\n" +
	"ExtendLock\x12&.elephant.repository.ExtendLockRequest\x1a!.elephant.repository.LockResponse\x12Q\n" +
	"\x06Unlock\x12\".elephant.repository.UnlockRequest\x1a#.elephant.repository.UnlockResponse\x12`\n" +
	"\vGetWithheld\x12'.elephant.repository.GetWithheldRequest\x1a(.elephant.repository.GetWithheldResponse\x12u\n" +
	"\x12GetDeliverableInfo\x12..elephant.repository.GetDeliverableInfoRequest\x1a/.elephant.repository.GetDeliverableInfoResponse\x12\x81\x01\n" +
	"\x16BulkGetDeliverableInfo\x122.elephant.repository.BulkGetDeliverableInfoRequest\x1a3.elephant.repository.BulkGetDeliverableInfoResponse\x12c\n" +
	"\fCreateUpload\x12(.elephant.repository.CreateUploadRequest\x1a).elephant.repository.CreateUploadResponse\x12i\n" +
	"\x0eGetAttachments\x12*.elephant.repository.GetAttachmentsRequest\x1a+.elephant.repository.GetAttachmentsResponse\x12`\n" +
	"\vGetMatching\x12'.elephant.repository.GetMatchingRequest\x1a(.elephant.repository.GetMatchingResponse\x12i\n" +
	"\x0eGetSocketToken\x12*.elephant.repository.GetSocketTokenRequest\x1a+.elephant.repository.GetSocketTokenResponse2\x87\r\n" +
	"\aSchemas\x12u\n" +
	"\x12RegisterGeneration\x12..elephant.repository.RegisterGenerationRequest\x1a/.elephant.repository.RegisterGenerationResponse\x12h\n" +
	"\tSetActive\x12,.elephant.repository.SetActiveSchemasRequest\x1a-.elephant.repository.SetActiveSchemasResponse\x12x\n" +
	"\x0fListGenerations\x121.elephant.repository.ListSchemaGenerationsRequest\x1a2.elephant.repository.ListSchemaGenerationsResponse\x12c\n" +
	"\fGetExemplars\x12(.elephant.repository.GetExemplarsRequest\x1a).elephant.repository.GetExemplarsResponse\x12T\n" +
	"\x03Get\x12%.elephant.repository.GetSchemaRequest\x1a&.elephant.repository.GetSchemaResponse\x12q\n" +
	"\fGetAllActive\x12/.elephant.repository.GetAllActiveSchemasRequest\x1a0.elephant.repository.GetAllActiveSchemasResponse\x12k\n" +
	"\n" +
	"ListActive\x12-.elephant.repository.ListActiveSchemasRequest\x1a..elephant.repository.ListActiveSchemasResponse\x12o\n" +
	"\x10RegisterMetaType\x12,.elephant.repository.RegisterMetaTypeRequest\x1a-.elephant.repository.RegisterMetaTypeResponse\x12x\n" +
	"\x13RegisterMetaTypeUse\x12/.elephant.repository.RegisterMetaTypeUseRequest\x1a0.elephant.repository.RegisterMetaTypeUseResponse\x12c\n" +
	"\fGetMetaTypes\x12(.elephant.repository.GetMetaTypesRequest\x1a).elephant.repository.GetMetaTypesResponse\x12l\n" +
	"\x0fGetDeprecations\x12+.elephant.repository.GetDeprecationsRequest\x1a,.elephant.repository.GetDeprecationsResponse\x12r\n" +
	"\x11UpdateDeprecation\x12-.elephant.repository.UpdateDeprecationRequest\x1a..elephant.repository.UpdateDeprecationResponse\x12o\n" +
	"\x10GetDocumentTypes\x12,.elephant.repository.GetDocumentTypesRequest\x1a-.elephant.repository.GetDocumentTypesResponse\x12f\n" +
	"\rConfigureType\x12).elephant.repository.ConfigureTypeRequest\x1a*.elephant.repository.ConfigureTypeResponse\x12{\n" +
	"\x14GetTypeConfiguration\x120.elephant.repository.GetTypeConfigurationRequest\x1a1.elephant.repository.GetTypeConfigurationResponse2\xce\x06\n" +
	"\tWorkflows\x12c\n" +
	"\fUpdateStatus\x12(.elephant.repository.UpdateStatusRequest\x1a).elephant.repository.UpdateStatusResponse\x12`\n" +
	"\vGetStatuses\x12'.elephant.repository.GetStatusesRequest\x1a(.elephant.repository.GetStatusesResponse\x12o\n" +
	"\x10CreateStatusRule\x12,.elephant.repository.CreateStatusRuleRequest\x1a-.elephant.repository.CreateStatusRuleResponse\x12o\n" +
	"\x10DeleteStatusRule\x12,.elephant.repository.DeleteStatusRuleRequest\x1a-.elephant.repository.DeleteStatusRuleResponse\x12i\n" +
	"\x0eGetStatusRules\x12*.elephant.repository.GetStatusRulesRequest\x1a+.elephant.repository.GetStatusRulesResponse\x12`\n" +
	"\vSetWorkflow\x12'.elephant.repository.SetWorkflowRequest\x1a(.elephant.repository.SetWorkflowResponse\x12`\n" +
	"\vGetWorkflow\x12'.elephant.repository.GetWorkflowRequest\x1a(.elephant.repository.GetWorkflowResponse\x12i\n" +
	"\x0eDeleteWorkflow\x12*.elephant.repository.DeleteWorkflowRequest\x1a+.elephant.repository.DeleteWorkflowResponse2\x94\x04\n" +
	"\aMetrics\x12o\n" +
	"\fRegisterKind\x12..elephant.repository.RegisterMetricKindRequest\x1a/.elephant.repository.RegisterMetricKindResponse\x12i\n" +
	"\n" +
	"DeleteKind\x12,.elephant.repository.DeleteMetricKindRequest\x1a-.elephant.repository.DeleteMetricKindResponse\x12c\n" +
	"\bGetKinds\x12*.elephant.repository.GetMetricKindsRequest\x1a+.elephant.repository.GetMetricKindsResponse\x12i\n" +
	"\x0eRegisterMetric\x12*.elephant.repository.RegisterMetricRequest\x1a+.elephant.repository.RegisterMetricResponse\x12]\n" +
	"\n" +
	"GetMetrics\x12&.elephant.repository.GetMetricsRequest\x1a'.elephant.repository.GetMetricsResponseB)Z'github.com/ttab/elephant-api/repositoryb\x06proto3"

var (
	file_repository_service_proto_rawDescOnce sync.Once
	file_repository_service_proto_rawDescData []byte
)

func file_repository_service_proto_rawDescGZIP() []byte {
	file_repository_service_proto_rawDescOnce.Do(func() {
		file_repository_service_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_repository_service_proto_rawDesc), len(file_repository_service_proto_rawDesc)))
	})
	return file_repository_service_proto_rawDescData
}

var file_repository_service_proto_enumTypes = make([]protoimpl.EnumInfo, 5)
var file_repository_service_proto_msgTypes = make([]protoimpl.MessageInfo, 179)
var file_repository_service_proto_goTypes = []any{
	(GetMetaDoc)(0),                        // 0: elephant.repository.GetMetaDoc
	(LockExclusivity)(0),                   // 1: elephant.repository.LockExclusivity
	(SchemaActivation)(0),                  // 2: elephant.repository.SchemaActivation
	(MetricAggregation)(0),                 // 3: elephant.repository.MetricAggregation
	(FilterOperator)(0),                    // 4: elephant.repository.FilterOperator
	(*EvictRequest)(nil),                   // 5: elephant.repository.EvictRequest
	(*EvictResponse)(nil),                  // 6: elephant.repository.EvictResponse
	(*GetStatusRequest)(nil),               // 7: elephant.repository.GetStatusRequest
	(*GetStatusResponse)(nil),              // 8: elephant.repository.GetStatusResponse
	(*GetStatusHistoryRequest)(nil),        // 9: elephant.repository.GetStatusHistoryRequest
	(*GetStatusHistoryReponse)(nil),        // 10: elephant.repository.GetStatusHistoryReponse
	(*GetNilStatusesRequest)(nil),          // 11: elephant.repository.GetNilStatusesRequest
	(*GetNilStatusesResponse)(nil),         // 12: elephant.repository.GetNilStatusesResponse
	(*GetStatusOverviewRequest)(nil),       // 13: elephant.repository.GetStatusOverviewRequest
	(*GetStatusOverviewResponse)(nil),      // 14: elephant.repository.GetStatusOverviewResponse
	(*StatusOverviewItem)(nil),             // 15: elephant.repository.StatusOverviewItem
	(*GetPermissionsRequest)(nil),          // 16: elephant.repository.GetPermissionsRequest
	(*GetPermissionsResponse)(nil),         // 17: elephant.repository.GetPermissionsResponse
	(*GetEventlogRequest)(nil),             // 18: elephant.repository.GetEventlogRequest
	(*GetEventlogResponse)(nil),            // 19: elephant.repository.GetEventlogResponse
	(*GetCompactedEventlogRequest)(nil),    // 20: elephant.repository.GetCompactedEventlogRequest
	(*GetCompactedEventlogResponse)(nil),   // 21: elephant.repository.GetCompactedEventlogResponse
	(*EventlogItem)(nil),                   // 22: elephant.repository.EventlogItem
	(*Timespan)(nil),                       // 23: elephant.repository.Timespan
	(*GetStatusRulesRequest)(nil),          // 24: elephant.repository.GetStatusRulesRequest
	(*GetStatusRulesResponse)(nil),         // 25: elephant.repository.GetStatusRulesResponse
	(*GetStatusesRequest)(nil),             // 26: elephant.repository.GetStatusesRequest
	(*SetWorkflowRequest)(nil),             // 27: elephant.repository.SetWorkflowRequest
	(*DocumentWorkflow)(nil),               // 28: elephant.repository.DocumentWorkflow
	(*SetWorkflowResponse)(nil),            // 29: elephant.repository.SetWorkflowResponse
	(*GetWorkflowRequest)(nil),             // 30: elephant.repository.GetWorkflowRequest
	(*GetWorkflowResponse)(nil),            // 31: elephant.repository.GetWorkflowResponse
	(*DeleteWorkflowRequest)(nil),          // 32: elephant.repository.DeleteWorkflowRequest
	(*DeleteWorkflowResponse)(nil),         // 33: elephant.repository.DeleteWorkflowResponse
	(*WorkflowStatus)(nil),                 // 34: elephant.repository.WorkflowStatus
	(*GetStatusesResponse)(nil),            // 35: elephant.repository.GetStatusesResponse
	(*UpdateStatusRequest)(nil),            // 36: elephant.repository.UpdateStatusRequest
	(*UpdateStatusResponse)(nil),           // 37: elephant.repository.UpdateStatusResponse
	(*CreateStatusRuleRequest)(nil),        // 38: elephant.repository.CreateStatusRuleRequest
	(*StatusRule)(nil),                     // 39: elephant.repository.StatusRule
	(*CreateStatusRuleResponse)(nil),       // 40: elephant.repository.CreateStatusRuleResponse
	(*DeleteStatusRuleRequest)(nil),        // 41: elephant.repository.DeleteStatusRuleRequest
	(*DeleteStatusRuleResponse)(nil),       // 42: elephant.repository.DeleteStatusRuleResponse
	(*GetDocumentRequest)(nil),             // 43: elephant.repository.GetDocumentRequest
	(*GetDocumentResponse)(nil),            // 44: elephant.repository.GetDocumentResponse
	(*ExtractedValues)(nil),                // 45: elephant.repository.ExtractedValues
	(*ExtractedValue)(nil),                 // 46: elephant.repository.ExtractedValue
	(*BulkGetRequest)(nil),                 // 47: elephant.repository.BulkGetRequest
	(*BulkGetReference)(nil),               // 48: elephant.repository.BulkGetReference
	(*BulkGetResponse)(nil),                // 49: elephant.repository.BulkGetResponse
	(*BulkGetItem)(nil),                    // 50: elephant.repository.BulkGetItem
	(*MetaDocument)(nil),                   // 51: elephant.repository.MetaDocument
	(*GetHistoryRequest)(nil),              // 52: elephant.repository.GetHistoryRequest
	(*GetHistoryResponse)(nil),             // 53: elephant.repository.GetHistoryResponse
	(*DocumentVersion)(nil),                // 54: elephant.repository.DocumentVersion
	(*DocumentStatuses)(nil),               // 55: elephant.repository.DocumentStatuses
	(*UpdateRequest)(nil),                  // 56: elephant.repository.UpdateRequest
	(*ImportDirective)(nil),                // 57: elephant.repository.ImportDirective
	(*UpdateResponse)(nil),                 // 58: elephant.repository.UpdateResponse
	(*BulkUpdateRequest)(nil),              // 59: elephant.repository.BulkUpdateRequest
	(*BulkUpdateResponse)(nil),             // 60: elephant.repository.BulkUpdateResponse
	(*ValidateRequest)(nil),                // 61: elephant.repository.ValidateRequest
	(*ValidateResponse)(nil),               // 62: elephant.repository.ValidateResponse
	(*PruneRequest)(nil),                   // 63: elephant.repository.PruneRequest
	(*PruneResponse)(nil),                  // 64: elephant.repository.PruneResponse
	(*ValidationResult)(nil),               // 65: elephant.repository.ValidationResult
	(*EntityRef)(nil),                      // 66: elephant.repository.EntityRef
	(*StatusUpdate)(nil),                   // 67: elephant.repository.StatusUpdate
	(*UpdatePermissionsRequest)(nil),       // 68: elephant.repository.UpdatePermissionsRequest
	(*UpdatePermissionsResponse)(nil),      // 69: elephant.repository.UpdatePermissionsResponse
	(*DeleteDocumentRequest)(nil),          // 70: elephant.repository.DeleteDocumentRequest
	(*DeleteDocumentResponse)(nil),         // 71: elephant.repository.DeleteDocumentResponse
	(*RestoreRequest)(nil),                 // 72: elephant.repository.RestoreRequest
	(*RestoreResponse)(nil),                // 73: elephant.repository.RestoreResponse
	(*PurgeRequest)(nil),                   // 74: elephant.repository.PurgeRequest
	(*PurgeResponse)(nil),                  // 75: elephant.repository.PurgeResponse
	(*ListDeletedRequest)(nil),             // 76: elephant.repository.ListDeletedRequest
	(*ListDeletedResponse)(nil),            // 77: elephant.repository.ListDeletedResponse
	(*DeleteRecord)(nil),                   // 78: elephant.repository.DeleteRecord
	(*GetMetaRequest)(nil),                 // 79: elephant.repository.GetMetaRequest
	(*GetMetaResponse)(nil),                // 80: elephant.repository.GetMetaResponse
	(*DocumentMeta)(nil),                   // 81: elephant.repository.DocumentMeta
	(*AttachmentRef)(nil),                  // 82: elephant.repository.AttachmentRef
	(*Status)(nil),                         // 83: elephant.repository.Status
	(*ACLEntry)(nil),                       // 84: elephant.repository.ACLEntry
	(*Lock)(nil),                           // 85: elephant.repository.Lock
	(*RegisterMetaTypeRequest)(nil),        // 86: elephant.repository.RegisterMetaTypeRequest
	(*RegisterMetaTypeResponse)(nil),       // 87: elephant.repository.RegisterMetaTypeResponse
	(*RegisterMetaTypeUseRequest)(nil),     // 88: elephant.repository.RegisterMetaTypeUseRequest
	(*RegisterMetaTypeUseResponse)(nil),    // 89: elephant.repository.RegisterMetaTypeUseResponse
	(*GetMetaTypesRequest)(nil),            // 90: elephant.repository.GetMetaTypesRequest
	(*GetMetaTypesResponse)(nil),           // 91: elephant.repository.GetMetaTypesResponse
	(*GetDocumentTypesRequest)(nil),        // 92: elephant.repository.GetDocumentTypesRequest
	(*GetDocumentTypesResponse)(nil),       // 93: elephant.repository.GetDocumentTypesResponse
	(*MetaTypeInfo)(nil),                   // 94: elephant.repository.MetaTypeInfo
	(*RegisterGenerationRequest)(nil),      // 95: elephant.repository.RegisterGenerationRequest
	(*RegisterGenerationResponse)(nil),     // 96: elephant.repository.RegisterGenerationResponse
	(*SetActiveSchemasRequest)(nil),        // 97: elephant.repository.SetActiveSchemasRequest
	(*SetActiveSchemasResponse)(nil),       // 98: elephant.repository.SetActiveSchemasResponse
	(*ListSchemaGenerationsRequest)(nil),   // 99: elephant.repository.ListSchemaGenerationsRequest
	(*ListSchemaGenerationsResponse)(nil),  // 100: elephant.repository.ListSchemaGenerationsResponse
	(*SchemaGeneration)(nil),               // 101: elephant.repository.SchemaGeneration
	(*GetExemplarsRequest)(nil),            // 102: elephant.repository.GetExemplarsRequest
	(*GetExemplarsResponse)(nil),           // 103: elephant.repository.GetExemplarsResponse
	(*Exemplar)(nil),                       // 104: elephant.repository.Exemplar
	(*GetSchemaRequest)(nil),               // 105: elephant.repository.GetSchemaRequest
	(*GetSchemaResponse)(nil),              // 106: elephant.repository.GetSchemaResponse
	(*GetAllActiveSchemasRequest)(nil),     // 107: elephant.repository.GetAllActiveSchemasRequest
	(*GetAllActiveSchemasResponse)(nil),    // 108: elephant.repository.GetAllActiveSchemasResponse
	(*ListActiveSchemasRequest)(nil),       // 109: elephant.repository.ListActiveSchemasRequest
	(*ListActiveSchemasResponse)(nil),      // 110: elephant.repository.ListActiveSchemasResponse
	(*SchemaReference)(nil),                // 111: elephant.repository.SchemaReference
	(*Schema)(nil),                         // 112: elephant.repository.Schema
	(*GetDeprecationsRequest)(nil),         // 113: elephant.repository.GetDeprecationsRequest
	(*Deprecation)(nil),                    // 114: elephant.repository.Deprecation
	(*GetDeprecationsResponse)(nil),        // 115: elephant.repository.GetDeprecationsResponse
	(*UpdateDeprecationRequest)(nil),       // 116: elephant.repository.UpdateDeprecationRequest
	(*UpdateDeprecationResponse)(nil),      // 117: elephant.repository.UpdateDeprecationResponse
	(*RegisterMetricKindRequest)(nil),      // 118: elephant.repository.RegisterMetricKindRequest
	(*RegisterMetricKindResponse)(nil),     // 119: elephant.repository.RegisterMetricKindResponse
	(*DeleteMetricKindRequest)(nil),        // 120: elephant.repository.DeleteMetricKindRequest
	(*DeleteMetricKindResponse)(nil),       // 121: elephant.repository.DeleteMetricKindResponse
	(*GetMetricKindsRequest)(nil),          // 122: elephant.repository.GetMetricKindsRequest
	(*GetMetricKindsResponse)(nil),         // 123: elephant.repository.GetMetricKindsResponse
	(*MetricKind)(nil),                     // 124: elephant.repository.MetricKind
	(*RegisterMetricRequest)(nil),          // 125: elephant.repository.RegisterMetricRequest
	(*RegisterMetricResponse)(nil),         // 126: elephant.repository.RegisterMetricResponse
	(*GetMetricsRequest)(nil),              // 127: elephant.repository.GetMetricsRequest
	(*GetMetricsResponse)(nil),             // 128: elephant.repository.GetMetricsResponse
	(*DocumentMetrics)(nil),                // 129: elephant.repository.DocumentMetrics
	(*Metric)(nil),                         // 130: elephant.repository.Metric
	(*LockRequest)(nil),                    // 131: elephant.repository.LockRequest
	(*LockResponse)(nil),                   // 132: elephant.repository.LockResponse
	(*AcquireLock)(nil),                    // 133: elephant.repository.AcquireLock
	(*LockGrant)(nil),                      // 134: elephant.repository.LockGrant
	(*ExtendLockRequest)(nil),              // 135: elephant.repository.ExtendLockRequest
	(*UnlockRequest)(nil),                  // 136: elephant.repository.UnlockRequest
	(*UnlockResponse)(nil),                 // 137: elephant.repository.UnlockResponse
	(*GetWithheldRequest)(nil),             // 138: elephant.repository.GetWithheldRequest
	(*GetWithheldResponse)(nil),            // 139: elephant.repository.GetWithheldResponse
	(*ScheduledDocument)(nil),              // 140: elephant.repository.ScheduledDocument
	(*GetDeliverableInfoRequest)(nil),      // 141: elephant.repository.GetDeliverableInfoRequest
	(*GetDeliverableInfoResponse)(nil),     // 142: elephant.repository.GetDeliverableInfoResponse
	(*BulkGetDeliverableInfoRequest)(nil),  // 143: elephant.repository.BulkGetDeliverableInfoRequest
	(*BulkGetDeliverableInfoResponse)(nil), // 144: elephant.repository.BulkGetDeliverableInfoResponse
	(*DeliverableInfo)(nil),                // 145: elephant.repository.DeliverableInfo
	(*CreateUploadRequest)(nil),            // 146: elephant.repository.CreateUploadRequest
	(*CreateUploadResponse)(nil),           // 147: elephant.repository.CreateUploadResponse
	(*GetAttachmentsRequest)(nil),          // 148: elephant.repository.GetAttachmentsRequest
	(*GetAttachmentsResponse)(nil),         // 149: elephant.repository.GetAttachmentsResponse
	(*AttachmentDetails)(nil),              // 150: elephant.repository.AttachmentDetails
	(*ConfigureTypeRequest)(nil),           // 151: elephant.repository.ConfigureTypeRequest
	(*TypeConfiguration)(nil),              // 152: elephant.repository.TypeConfiguration
	(*TypeTimeExpression)(nil),             // 153: elephant.repository.TypeTimeExpression
	(*LabelExpression)(nil),                // 154: elephant.repository.LabelExpression
	(*ConfigureTypeResponse)(nil),          // 155: elephant.repository.ConfigureTypeResponse
	(*GetTypeConfigurationRequest)(nil),    // 156: elephant.repository.GetTypeConfigurationRequest
	(*GetTypeConfigurationResponse)(nil),   // 157: elephant.repository.GetTypeConfigurationResponse
	(*GetMatchingRequest)(nil),             // 158: elephant.repository.GetMatchingRequest
	(*GetMatchingResponse)(nil),            // 159: elephant.repository.GetMatchingResponse
	(*DocumentMatch)(nil),                  // 160: elephant.repository.DocumentMatch
	(*DocumentFilter)(nil),                 // 161: elephant.repository.DocumentFilter
	(*FilterValues)(nil),                   // 162: elephant.repository.FilterValues
	(*GetSocketTokenRequest)(nil),          // 163: elephant.repository.GetSocketTokenRequest
	(*GetSocketTokenResponse)(nil),         // 164: elephant.repository.GetSocketTokenResponse
	nil,                                    // 165: elephant.repository.GetNilStatusesResponse.StatusesEntry
	nil,                                    // 166: elephant.repository.StatusOverviewItem.HeadsEntry
	nil,                                    // 167: elephant.repository.GetPermissionsResponse.PermissionsEntry
	nil,                                    // 168: elephant.repository.ExtractedValues.ValuesEntry
	nil,                                    // 169: elephant.repository.DocumentVersion.MetaEntry
	nil,                                    // 170: elephant.repository.DocumentVersion.StatusesEntry
	nil,                                    // 171: elephant.repository.UpdateRequest.MetaEntry
	nil,                                    // 172: elephant.repository.UpdateRequest.IfStatusHeadsEntry
	nil,                                    // 173: elephant.repository.UpdateRequest.AttachObjectsEntry
	nil,                                    // 174: elephant.repository.StatusUpdate.MetaEntry
	nil,                                    // 175: elephant.repository.DeleteDocumentRequest.MetaEntry
	nil,                                    // 176: elephant.repository.DeleteRecord.MetaEntry
	nil,                                    // 177: elephant.repository.DocumentMeta.HeadsEntry
	nil,                                    // 178: elephant.repository.Status.MetaEntry
	nil,                                    // 179: elephant.repository.GetExemplarsRequest.KnownEntry
	nil,                                    // 180: elephant.repository.GetAllActiveSchemasRequest.KnownEntry
	nil,                                    // 181: elephant.repository.GetMetricsResponse.DocumentsEntry
	nil,                                    // 182: elephant.repository.CreateUploadRequest.MetaEntry
	nil,                                    // 183: elephant.repository.FilterValues.ValuesEntry
	(*newsdoc.Document)(nil),               // 184: newsdoc.Document
	(*newsdoc.Block)(nil),                  // 185: newsdoc.Block
}
var file_repository_service_proto_depIdxs = []int32{
	83,  // 0: elephant.repository.GetStatusResponse.status:type_name -> elephant.repository.Status
	83,  // 1: elephant.repository.GetStatusHistoryReponse.statuses:type_name -> elephant.repository.Status
	165, // 2: elephant.repository.GetNilStatusesResponse.statuses:type_name -> elephant.repository.GetNilStatusesResponse.StatusesEntry
	15,  // 3: elephant.repository.GetStatusOverviewResponse.items:type_name -> elephant.repository.StatusOverviewItem
	166, // 4: elephant.repository.StatusOverviewItem.heads:type_name -> elephant.repository.StatusOverviewItem.HeadsEntry
	167, // 5: elephant.repository.GetPermissionsResponse.permissions:type_name -> elephant.repository.GetPermissionsResponse.PermissionsEntry
	22,  // 6: elephant.repository.GetEventlogResponse.items:type_name -> elephant.repository.EventlogItem
	22,  // 7: elephant.repository.GetCompactedEventlogResponse.items:type_name -> elephant.repository.EventlogItem
	84,  // 8: elephant.repository.EventlogItem.acl:type_name -> elephant.repository.ACLEntry
	23,  // 9: elephant.repository.EventlogItem.timespans:type_name -> elephant.repository.Timespan
	39,  // 10: elephant.repository.GetStatusRulesResponse.rules:type_name -> elephant.repository.StatusRule
	28,  // 11: elephant.repository.SetWorkflowRequest.workflow:type_name -> elephant.repository.DocumentWorkflow
	28,  // 12: elephant.repository.GetWorkflowResponse.workflow:type_name -> elephant.repository.DocumentWorkflow
	34,  // 13: elephant.repository.GetStatusesResponse.statuses:type_name -> elephant.repository.WorkflowStatus
	39,  // 14: elephant.repository.CreateStatusRuleRequest.rule:type_name -> elephant.repository.StatusRule
	133, // 15: elephant.repository.GetDocumentRequest.lock:type_name -> elephant.repository.AcquireLock
	0,   // 16: elephant.repository.GetDocumentRequest.meta_document:type_name -> elephant.repository.GetMetaDoc
	184, // 17: elephant.repository.GetDocumentResponse.document:type_name -> newsdoc.Document
	83,  // 18: elephant.repository.GetDocumentResponse.status:type_name -> elephant.repository.Status
	51,  // 19: elephant.repository.GetDocumentResponse.meta:type_name -> elephant.repository.MetaDocument
	45,  // 20: elephant.repository.GetDocumentResponse.subset:type_name -> elephant.repository.ExtractedValues
	134, // 21: elephant.repository.GetDocumentResponse.lock:type_name -> elephant.repository.LockGrant
	168, // 22: elephant.repository.ExtractedValues.values:type_name -> elephant.repository.ExtractedValues.ValuesEntry
	185, // 23: elephant.repository.ExtractedValue.block:type_name -> newsdoc.Block
	48,  // 24: elephant.repository.BulkGetRequest.documents:type_name -> elephant.repository.BulkGetReference
	50,  // 25: elephant.repository.BulkGetResponse.items:type_name -> elephant.repository.BulkGetItem
	184, // 26: elephant.repository.BulkGetItem.document:type_name -> newsdoc.Document
	45,  // 27: elephant.repository.BulkGetItem.subset:type_name -> elephant.repository.ExtractedValues
	184, // 28: elephant.repository.MetaDocument.document:type_name -> newsdoc.Document
	54,  // 29: elephant.repository.GetHistoryResponse.versions:type_name -> elephant.repository.DocumentVersion
	169, // 30: elephant.repository.DocumentVersion.meta:type_name -> elephant.repository.DocumentVersion.MetaEntry
	170, // 31: elephant.repository.DocumentVersion.statuses:type_name -> elephant.repository.DocumentVersion.StatusesEntry
	83,  // 32: elephant.repository.DocumentStatuses.items:type_name -> elephant.repository.Status
	184, // 33: elephant.repository.UpdateRequest.document:type_name -> newsdoc.Document
	171, // 34: elephant.repository.UpdateRequest.meta:type_name -> elephant.repository.UpdateRequest.MetaEntry
	67,  // 35: elephant.repository.UpdateRequest.status:type_name -> elephant.repository.StatusUpdate
	84,  // 36: elephant.repository.UpdateRequest.acl:type_name -> elephant.repository.ACLEntry
	57,  // 37: elephant.repository.UpdateRequest.import_directive:type_name -> elephant.repository.ImportDirective
	172, // 38: elephant.repository.UpdateRequest.if_status_heads:type_name -> elephant.repository.UpdateRequest.IfStatusHeadsEntry
	173, // 39: elephant.repository.UpdateRequest.attach_objects:type_name -> elephant.repository.UpdateRequest.AttachObjectsEntry
	56,  // 40: elephant.repository.BulkUpdateRequest.updates:type_name -> elephant.repository.UpdateRequest
	58,  // 41: elephant.repository.BulkUpdateResponse.updates:type_name -> elephant.repository.UpdateResponse
	184, // 42: elephant.repository.ValidateRequest.document:type_name -> newsdoc.Document
	65,  // 43: elephant.repository.ValidateResponse.errors:type_name -> elephant.repository.ValidationResult
	184, // 44: elephant.repository.PruneRequest.document:type_name -> newsdoc.Document
	184, // 45: elephant.repository.PruneResponse.document:type_name -> newsdoc.Document
	65,  // 46: elephant.repository.PruneResponse.errors:type_name -> elephant.repository.ValidationResult
	66,  // 47: elephant.repository.ValidationResult.entity:type_name -> elephant.repository.EntityRef
	174, // 48: elephant.repository.StatusUpdate.meta:type_name -> elephant.repository.StatusUpdate.MetaEntry
	84,  // 49: elephant.repository.UpdatePermissionsRequest.list:type_name -> elephant.repository.ACLEntry
	175, // 50: elephant.repository.DeleteDocumentRequest.meta:type_name -> elephant.repository.DeleteDocumentRequest.MetaEntry
	84,  // 51: elephant.repository.RestoreRequest.acl:type_name -> elephant.repository.ACLEntry
	78,  // 52: elephant.repository.ListDeletedResponse.deletes:type_name -> elephant.repository.DeleteRecord
	176, // 53: elephant.repository.DeleteRecord.meta:type_name -> elephant.repository.DeleteRecord.MetaEntry
	81,  // 54: elephant.repository.GetMetaResponse.meta:type_name -> elephant.repository.DocumentMeta
	177, // 55: elephant.repository.DocumentMeta.heads:type_name -> elephant.repository.DocumentMeta.HeadsEntry
	84,  // 56: elephant.repository.DocumentMeta.acl:type_name -> elephant.repository.ACLEntry
	85,  // 57: elephant.repository.DocumentMeta.lock:type_name -> elephant.repository.Lock
	82,  // 58: elephant.repository.DocumentMeta.attachments:type_name -> elephant.repository.AttachmentRef
	178, // 59: elephant.repository.Status.meta:type_name -> elephant.repository.Status.MetaEntry
	1,   // 60: elephant.repository.Lock.exclusivity:type_name -> elephant.repository.LockExclusivity
	94,  // 61: elephant.repository.GetMetaTypesResponse.types:type_name -> elephant.repository.MetaTypeInfo
	112, // 62: elephant.repository.RegisterGenerationRequest.schemas:type_name -> elephant.repository.Schema
	2,   // 63: elephant.repository.RegisterGenerationRequest.activation:type_name -> elephant.repository.SchemaActivation
	184, // 64: elephant.repository.RegisterGenerationRequest.exemplars:type_name -> newsdoc.Document
	2,   // 65: elephant.repository.SetActiveSchemasRequest.activation:type_name -> elephant.repository.SchemaActivation
	101, // 66: elephant.repository.ListSchemaGenerationsResponse.items:type_name -> elephant.repository.SchemaGeneration
	2,   // 67: elephant.repository.SchemaGeneration.status:type_name -> elephant.repository.SchemaActivation
	111, // 68: elephant.repository.SchemaGeneration.schemas:type_name -> elephant.repository.SchemaReference
	179, // 69: elephant.repository.GetExemplarsRequest.known:type_name -> elephant.repository.GetExemplarsRequest.KnownEntry
	104, // 70: elephant.repository.GetExemplarsResponse.exemplars:type_name -> elephant.repository.Exemplar
	184, // 71: elephant.repository.Exemplar.document:type_name -> newsdoc.Document
	180, // 72: elephant.repository.GetAllActiveSchemasRequest.known:type_name -> elephant.repository.GetAllActiveSchemasRequest.KnownEntry
	112, // 73: elephant.repository.GetAllActiveSchemasResponse.schemas:type_name -> elephant.repository.Schema
	112, // 74: elephant.repository.ListActiveSchemasResponse.schemas:type_name -> elephant.repository.Schema
	114, // 75: elephant.repository.GetDeprecationsResponse.deprecations:type_name -> elephant.repository.Deprecation
	114, // 76: elephant.repository.UpdateDeprecationRequest.deprecation:type_name -> elephant.repository.Deprecation
	3,   // 77: elephant.repository.RegisterMetricKindRequest.aggregation:type_name -> elephant.repository.MetricAggregation
	124, // 78: elephant.repository.GetMetricKindsResponse.kinds:type_name -> elephant.repository.MetricKind
	3,   // 79: elephant.repository.MetricKind.aggregation:type_name -> elephant.repository.MetricAggregation
	181, // 80: elephant.repository.GetMetricsResponse.documents:type_name -> elephant.repository.GetMetricsResponse.DocumentsEntry
	130, // 81: elephant.repository.DocumentMetrics.metrics:type_name -> elephant.repository.Metric
	1,   // 82: elephant.repository.LockRequest.exclusivity:type_name -> elephant.repository.LockExclusivity
	1,   // 83: elephant.repository.AcquireLock.exclusivity:type_name -> elephant.repository.LockExclusivity
	140, // 84: elephant.repository.GetWithheldResponse.items:type_name -> elephant.repository.ScheduledDocument
	145, // 85: elephant.repository.BulkGetDeliverableInfoResponse.items:type_name -> elephant.repository.DeliverableInfo
	182, // 86: elephant.repository.CreateUploadRequest.meta:type_name -> elephant.repository.CreateUploadRequest.MetaEntry
	150, // 87: elephant.repository.GetAttachmentsResponse.attachments:type_name -> elephant.repository.AttachmentDetails
	152, // 88: elephant.repository.ConfigureTypeRequest.configuration:type_name -> elephant.repository.TypeConfiguration
	153, // 89: elephant.repository.TypeConfiguration.time_expressions:type_name -> elephant.repository.TypeTimeExpression
	154, // 90: elephant.repository.TypeConfiguration.label_expressions:type_name -> elephant.repository.LabelExpression
	152, // 91: elephant.repository.GetTypeConfigurationResponse.configuration:type_name -> elephant.repository.TypeConfiguration
	23,  // 92: elephant.repository.GetMatchingRequest.timespan:type_name -> elephant.repository.Timespan
	161, // 93: elephant.repository.GetMatchingRequest.filter:type_name -> elephant.repository.DocumentFilter
	160, // 94: elephant.repository.GetMatchingResponse.matches:type_name -> elephant.repository.DocumentMatch
	184, // 95: elephant.repository.DocumentMatch.document:type_name -> newsdoc.Document
	81,  // 96: elephant.repository.DocumentMatch.meta:type_name -> elephant.repository.DocumentMeta
	4,   // 97: elephant.repository.DocumentFilter.operator:type_name -> elephant.repository.FilterOperator
	162, // 98: elephant.repository.DocumentFilter.values:type_name -> elephant.repository.FilterValues
	161, // 99: elephant.repository.DocumentFilter.and:type_name -> elephant.repository.DocumentFilter
	161, // 100: elephant.repository.DocumentFilter.or:type_name -> elephant.repository.DocumentFilter
	183, // 101: elephant.repository.FilterValues.values:type_name -> elephant.repository.FilterValues.ValuesEntry
	55,  // 102: elephant.repository.GetNilStatusesResponse.StatusesEntry.value:type_name -> elephant.repository.DocumentStatuses
	83,  // 103: elephant.repository.StatusOverviewItem.HeadsEntry.value:type_name -> elephant.repository.Status
	46,  // 104: elephant.repository.ExtractedValues.ValuesEntry.value:type_name -> elephant.repository.ExtractedValue
	55,  // 105: elephant.repository.DocumentVersion.StatusesEntry.value:type_name -> elephant.repository.DocumentStatuses
	83,  // 106: elephant.repository.DocumentMeta.HeadsEntry.value:type_name -> elephant.repository.Status
	129, // 107: elephant.repository.GetMetricsResponse.DocumentsEntry.value:type_name -> elephant.repository.DocumentMetrics
	43,  // 108: elephant.repository.Documents.Get:input_type -> elephant.repository.GetDocumentRequest
	47,  // 109: elephant.repository.Documents.BulkGet:input_type -> elephant.repository.BulkGetRequest
	52,  // 110: elephant.repository.Documents.GetHistory:input_type -> elephant.repository.GetHistoryRequest
	56,  // 111: elephant.repository.Documents.Update:input_type -> elephant.repository.UpdateRequest
	59,  // 112: elephant.repository.Documents.BulkUpdate:input_type -> elephant.repository.BulkUpdateRequest
	61,  // 113: elephant.repository.Documents.Validate:input_type -> elephant.repository.ValidateRequest
	63,  // 114: elephant.repository.Documents.Prune:input_type -> elephant.repository.PruneRequest
	5,   // 115: elephant.repository.Documents.Evict:input_type -> elephant.repository.EvictRequest
	70,  // 116: elephant.repository.Documents.Delete:input_type -> elephant.repository.DeleteDocumentRequest
	76,  // 117: elephant.repository.Documents.ListDeleted:input_type -> elephant.repository.ListDeletedRequest
	72,  // 118: elephant.repository.Documents.Restore:input_type -> elephant.repository.RestoreRequest
	74,  // 119: elephant.repository.Documents.Purge:input_type -> elephant.repository.PurgeRequest
	79,  // 120: elephant.repository.Documents.GetMeta:input_type -> elephant.repository.GetMetaRequest
	18,  // 121: elephant.repository.Documents.Eventlog:input_type -> elephant.repository.GetEventlogRequest
	20,  // 122: elephant.repository.Documents.CompactedEventlog:input_type -> elephant.repository.GetCompactedEventlogRequest
	7,   // 123: elephant.repository.Documents.GetStatus:input_type -> elephant.repository.GetStatusRequest
	9,   // 124: elephant.repository.Documents.GetStatusHistory:input_type -> elephant.repository.GetStatusHistoryRequest
	11,  // 125: elephant.repository.Documents.GetNilStatuses:input_type -> elephant.repository.GetNilStatusesRequest
	13,  // 126: elephant.repository.Documents.GetStatusOverview:input_type -> elephant.repository.GetStatusOverviewRequest
	16,  // 127: elephant.repository.Documents.GetPermissions:input_type -> elephant.repository.GetPermissionsRequest
	131, // 128: elephant.repository.Documents.Lock:input_type -> elephant.repository.LockRequest
	135, // 129: elephant.repository.Documents.ExtendLock:input_type -> elephant.repository.ExtendLockRequest
	136, // 130: elephant.repository.Documents.Unlock:input_type -> elephant.repository.UnlockRequest
	138, // 131: elephant.repository.Documents.GetWithheld:input_type -> elephant.repository.GetWithheldRequest
	141, // 132: elephant.repository.Documents.GetDeliverableInfo:input_type -> elephant.repository.GetDeliverableInfoRequest
	143, // 133: elephant.repository.Documents.BulkGetDeliverableInfo:input_type -> elephant.repository.BulkGetDeliverableInfoRequest
	146, // 134: elephant.repository.Documents.CreateUpload:input_type -> elephant.repository.CreateUploadRequest
	148, // 135: elephant.repository.Documents.GetAttachments:input_type -> elephant.repository.GetAttachmentsRequest
	158, // 136: elephant.repository.Documents.GetMatching:input_type -> elephant.repository.GetMatchingRequest
	163, // 137: elephant.repository.Documents.GetSocketToken:input_type -> elephant.repository.GetSocketTokenRequest
	95,  // 138: elephant.repository.Schemas.RegisterGeneration:input_type -> elephant.repository.RegisterGenerationRequest
	97,  // 139: elephant.repository.Schemas.SetActive:input_type -> elephant.repository.SetActiveSchemasRequest
	99,  // 140: elephant.repository.Schemas.ListGenerations:input_type -> elephant.repository.ListSchemaGenerationsRequest
	102, // 141: elephant.repository.Schemas.GetExemplars:input_type -> elephant.repository.GetExemplarsRequest
	105, // 142: elephant.repository.Schemas.Get:input_type -> elephant.repository.GetSchemaRequest
	107, // 143: elephant.repository.Schemas.GetAllActive:input_type -> elephant.repository.GetAllActiveSchemasRequest
	109, // 144: elephant.repository.Schemas.ListActive:input_type -> elephant.repository.ListActiveSchemasRequest
	86,  // 145: elephant.repository.Schemas.RegisterMetaType:input_type -> elephant.repository.RegisterMetaTypeRequest
	88,  // 146: elephant.repository.Schemas.RegisterMetaTypeUse:input_type -> elephant.repository.RegisterMetaTypeUseRequest
	90,  // 147: elephant.repository.Schemas.GetMetaTypes:input_type -> elephant.repository.GetMetaTypesRequest
	113, // 148: elephant.repository.Schemas.GetDeprecations:input_type -> elephant.repository.GetDeprecationsRequest
	116, // 149: elephant.repository.Schemas.UpdateDeprecation:input_type -> elephant.repository.UpdateDeprecationRequest
	92,  // 150: elephant.repository.Schemas.GetDocumentTypes:input_type -> elephant.repository.GetDocumentTypesRequest
	151, // 151: elephant.repository.Schemas.ConfigureType:input_type -> elephant.repository.ConfigureTypeRequest
	156, // 152: elephant.repository.Schemas.GetTypeConfiguration:input_type -> elephant.repository.GetTypeConfigurationRequest
	36,  // 153: elephant.repository.Workflows.UpdateStatus:input_type -> elephant.repository.UpdateStatusRequest
	26,  // 154: elephant.repository.Workflows.GetStatuses:input_type -> elephant.repository.GetStatusesRequest
	38,  // 155: elephant.repository.Workflows.CreateStatusRule:input_type -> elephant.repository.CreateStatusRuleRequest
	41,  // 156: elephant.repository.Workflows.DeleteStatusRule:input_type -> elephant.repository.DeleteStatusRuleRequest
	24,  // 157: elephant.repository.Workflows.GetStatusRules:input_type -> elephant.repository.GetStatusRulesRequest
	27,  // 158: elephant.repository.Workflows.SetWorkflow:input_type -> elephant.repository.SetWorkflowRequest
	30,  // 159: elephant.repository.Workflows.GetWorkflow:input_type -> elephant.repository.GetWorkflowRequest
	32,  // 160: elephant.repository.Workflows.DeleteWorkflow:input_type -> elephant.repository.DeleteWorkflowRequest
	118, // 161: elephant.repository.Metrics.RegisterKind:input_type -> elephant.repository.RegisterMetricKindRequest
	120, // 162: elephant.repository.Metrics.DeleteKind:input_type -> elephant.repository.DeleteMetricKindRequest
	122, // 163: elephant.repository.Metrics.GetKinds:input_type -> elephant.repository.GetMetricKindsRequest
	125, // 164: elephant.repository.Metrics.RegisterMetric:input_type -> elephant.repository.RegisterMetricRequest
	127, // 165: elephant.repository.Metrics.GetMetrics:input_type -> elephant.repository.GetMetricsRequest
	44,  // 166: elephant.repository.Documents.Get:output_type -> elephant.repository.GetDocumentResponse
	49,  // 167: elephant.repository.Documents.BulkGet:output_type -> elephant.repository.BulkGetResponse
	53,  // 168: elephant.repository.Documents.GetHistory:output_type -> elephant.repository.GetHistoryResponse
	58,  // 169: elephant.repository.Documents.Update:output_type -> elephant.repository.UpdateResponse
	60,  // 170: elephant.repository.Documents.BulkUpdate:output_type -> elephant.repository.BulkUpdateResponse
	62,  // 171: elephant.repository.Documents.Validate:output_type -> elephant.repository.ValidateResponse
	64,  // 172: elephant.repository.Documents.Prune:output_type -> elephant.repository.PruneResponse
	6,   // 173: elephant.repository.Documents.Evict:output_type -> elephant.repository.EvictResponse
	71,  // 174: elephant.repository.Documents.Delete:output_type -> elephant.repository.DeleteDocumentResponse
	77,  // 175: elephant.repository.Documents.ListDeleted:output_type -> elephant.repository.ListDeletedResponse
	73,  // 176: elephant.repository.Documents.Restore:output_type -> elephant.repository.RestoreResponse
	75,  // 177: elephant.repository.Documents.Purge:output_type -> elephant.repository.PurgeResponse
	80,  // 178: elephant.repository.Documents.GetMeta:output_type -> elephant.repository.GetMetaResponse
	19,  // 179: elephant.repository.Documents.Eventlog:output_type -> elephant.repository.GetEventlogResponse
	21,  // 180: elephant.repository.Documents.CompactedEventlog:output_type -> elephant.repository.GetCompactedEventlogResponse
	8,   // 181: elephant.repository.Documents.GetStatus:output_type -> elephant.repository.GetStatusResponse
	10,  // 182: elephant.repository.Documents.GetStatusHistory:output_type -> elephant.repository.GetStatusHistoryReponse
	12,  // 183: elephant.repository.Documents.GetNilStatuses:output_type -> elephant.repository.GetNilStatusesResponse
	14,  // 184: elephant.repository.Documents.GetStatusOverview:output_type -> elephant.repository.GetStatusOverviewResponse
	17,  // 185: elephant.repository.Documents.GetPermissions:output_type -> elephant.repository.GetPermissionsResponse
	132, // 186: elephant.repository.Documents.Lock:output_type -> elephant.repository.LockResponse
	132, // 187: elephant.repository.Documents.ExtendLock:output_type -> elephant.repository.LockResponse
	137, // 188: elephant.repository.Documents.Unlock:output_type -> elephant.repository.UnlockResponse
	139, // 189: elephant.repository.Documents.GetWithheld:output_type -> elephant.repository.GetWithheldResponse
	142, // 190: elephant.repository.Documents.GetDeliverableInfo:output_type -> elephant.repository.GetDeliverableInfoResponse
	144, // 191: elephant.repository.Documents.BulkGetDeliverableInfo:output_type -> elephant.repository.BulkGetDeliverableInfoResponse
	147, // 192: elephant.repository.Documents.CreateUpload:output_type -> elephant.repository.CreateUploadResponse
	149, // 193: elephant.repository.Documents.GetAttachments:output_type -> elephant.repository.GetAttachmentsResponse
	159, // 194: elephant.repository.Documents.GetMatching:output_type -> elephant.repository.GetMatchingResponse
	164, // 195: elephant.repository.Documents.GetSocketToken:output_type -> elephant.repository.GetSocketTokenResponse
	96,  // 196: elephant.repository.Schemas.RegisterGeneration:output_type -> elephant.repository.RegisterGenerationResponse
	98,  // 197: elephant.repository.Schemas.SetActive:output_type -> elephant.repository.SetActiveSchemasResponse
	100, // 198: elephant.repository.Schemas.ListGenerations:output_type -> elephant.repository.ListSchemaGenerationsResponse
	103, // 199: elephant.repository.Schemas.GetExemplars:output_type -> elephant.repository.GetExemplarsResponse
	106, // 200: elephant.repository.Schemas.Get:output_type -> elephant.repository.GetSchemaResponse
	108, // 201: elephant.repository.Schemas.GetAllActive:output_type -> elephant.repository.GetAllActiveSchemasResponse
	110, // 202: elephant.repository.Schemas.ListActive:output_type -> elephant.repository.ListActiveSchemasResponse
	87,  // 203: elephant.repository.Schemas.RegisterMetaType:output_type -> elephant.repository.RegisterMetaTypeResponse
	89,  // 204: elephant.repository.Schemas.RegisterMetaTypeUse:output_type -> elephant.repository.RegisterMetaTypeUseResponse
	91,  // 205: elephant.repository.Schemas.GetMetaTypes:output_type -> elephant.repository.GetMetaTypesResponse
	115, // 206: elephant.repository.Schemas.GetDeprecations:output_type -> elephant.repository.GetDeprecationsResponse
	117, // 207: elephant.repository.Schemas.UpdateDeprecation:output_type -> elephant.repository.UpdateDeprecationResponse
	93,  // 208: elephant.repository.Schemas.GetDocumentTypes:output_type -> elephant.repository.GetDocumentTypesResponse
	155, // 209: elephant.repository.Schemas.ConfigureType:output_type -> elephant.repository.ConfigureTypeResponse
	157, // 210: elephant.repository.Schemas.GetTypeConfiguration:output_type -> elephant.repository.GetTypeConfigurationResponse
	37,  // 211: elephant.repository.Workflows.UpdateStatus:output_type -> elephant.repository.UpdateStatusResponse
	35,  // 212: elephant.repository.Workflows.GetStatuses:output_type -> elephant.repository.GetStatusesResponse
	40,  // 213: elephant.repository.Workflows.CreateStatusRule:output_type -> elephant.repository.CreateStatusRuleResponse
	42,  // 214: elephant.repository.Workflows.DeleteStatusRule:output_type -> elephant.repository.DeleteStatusRuleResponse
	25,  // 215: elephant.repository.Workflows.GetStatusRules:output_type -> elephant.repository.GetStatusRulesResponse
	29,  // 216: elephant.repository.Workflows.SetWorkflow:output_type -> elephant.repository.SetWorkflowResponse
	31,  // 217: elephant.repository.Workflows.GetWorkflow:output_type -> elephant.repository.GetWorkflowResponse
	33,  // 218: elephant.repository.Workflows.DeleteWorkflow:output_type -> elephant.repository.DeleteWorkflowResponse
	119, // 219: elephant.repository.Metrics.RegisterKind:output_type -> elephant.repository.RegisterMetricKindResponse
	121, // 220: elephant.repository.Metrics.DeleteKind:output_type -> elephant.repository.DeleteMetricKindResponse
	123, // 221: elephant.repository.Metrics.GetKinds:output_type -> elephant.repository.GetMetricKindsResponse
	126, // 222: elephant.repository.Metrics.RegisterMetric:output_type -> elephant.repository.RegisterMetricResponse
	128, // 223: elephant.repository.Metrics.GetMetrics:output_type -> elephant.repository.GetMetricsResponse
	166, // [166:224] is the sub-list for method output_type
	108, // [108:166] is the sub-list for method input_type
	108, // [108:108] is the sub-list for extension type_name
	108, // [108:108] is the sub-list for extension extendee
	0,   // [0:108] is the sub-list for field type_name
}

func init() { file_repository_service_proto_init() }
func file_repository_service_proto_init() {
	if File_repository_service_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_repository_service_proto_rawDesc), len(file_repository_service_proto_rawDesc)),
			NumEnums:      5,
			NumMessages:   179,
			NumExtensions: 0,
			NumServices:   4,
		},
		GoTypes:           file_repository_service_proto_goTypes,
		DependencyIndexes: file_repository_service_proto_depIdxs,
		EnumInfos:         file_repository_service_proto_enumTypes,
		MessageInfos:      file_repository_service_proto_msgTypes,
	}.Build()
	File_repository_service_proto = out.File
	file_repository_service_proto_goTypes = nil
	file_repository_service_proto_depIdxs = nil
}
